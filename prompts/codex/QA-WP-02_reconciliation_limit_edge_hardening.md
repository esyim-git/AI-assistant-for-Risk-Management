...
- 기존 `LimitReconciliationTests` 단언 **전부 보존**. 종료부 **`Total=834 → 834+N PASS / 0 FAIL`**, `Unclassified=0`.
...
제품 코드 변경 0(테스트만) / 추가는 실제 미커버 경계 / 7상태·대사 9코드 기대값 정확 / 키스톤·`Passed`·ordinal 불변 검증 / 합성 더미(실데이터 0) / 도메인 Reconciliation·Limit·Unclassified 0 / 기존 단언 보존·감소 0 / NuGet 0 / `Total` 834 보존+신규 / Gate A.
