...
## 목표
공개 규정 catalog의 메타데이터를 **docs/17 §R3 전체 항목**으로 확장한다: 기존 7컬럼 + **버전·시행일·폐기일·파일Hash·적재일·승인상태·대체문서·라이선스 상태**(8개). 검색 답변이 이 메타를 인용에 쓸 수 있는 토대를 만든다. **공개 규정 메타만**(원문·내부규정 미포함 불변).
...
## 작업 범위 / 제외
- `RegulationCatalogEntry`에 8개 메타 필드 추가(아래 Public Interface).
...
public sealed record RegulationCatalogEntry(
    ...
    // R3-WP-01 추가(모두 string, 공개 메타):
    string Version, string EffectiveDate, string RepealDate, string FileHash,
...
## 테스트(필수)
- 확장 catalog 로드 → 8개 메타 필드 정상 채움(샘플 공개 규정 행에 version/effective_date/license 등 값).
...
8 메타필드 추가/하위호환(빈값 graceful) / 공개 메타만·원문 미포함(내부·NCR status 유지) / KbSearch 동작·점수 불변 / NuGet 0 / 기존 SmokeTest 유지 / Gate A.
