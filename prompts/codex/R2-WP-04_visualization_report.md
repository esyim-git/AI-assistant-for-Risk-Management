...
- `src/RiskManagementAI.App/MainWindow.xaml(.cs)` (`OnRunLimitMonitor` ~line 657, `OnGenerateExcelReport` ~line 700)
...
2. **집계 시각화 데이터 시트(신규)**: `ExpectedSheetNames`에 `RISK_VISUAL`(필요 시 TOPN/CONCENTRATION/HEATMAP 분리 가능, 최소 1개) 추가. 내용 = 상태분포(6상태 카운트·비율) / TopN movers(ExposureAmount 또는 UsageRatio 내림차순 상위 N, tie-break PortfolioId Ordinal) / 집중도(상위N 비중, HHI=Σ(share²)) / Heatmap 등급(UsageRatio <0.8 LOW / 0.8~1.0 MID / >1.0 HIGH). 셀은 가급적 Number/inlineStr **정적 값**으로 기록(수식 최소화 → Excel2021 게이트 위험 0). 옵션 ASCII 막대(반복문자 길이=비율) 허용.
...
- 전일 대비(Current/Prev/Δ) 산출(=R2-WP-03). `LimitAnalysisResult`/6상태/RECON_* 로직 변경. `BuildReport` 시그니처 변경.
...
- 회귀: 6상태·RECON_*·ReconciliationPassed PASS/FAIL·NO_LIMIT_ROW·NuGet 0 단언 전부 유지.

## 7. 완료조건 / 보고
- 로컬 `dotnet build RiskManagementAI.sln -c Release` 0/0, `dotnet run --project tests/RiskManagementAI.SmokeTests` → **`Total=N PASS=N FAIL=0`**(기존 646 + 신규, Unclassified=0). `git grep PackageReference` = 0.
