...
2. SQL/VBA/Excel `TextBox`에 부착 — **Ctrl+Space**: 박스 언어로 `CompletionContext` 구성 → `CompletionEngine.GetCompletions` → Popup 표시. **Enter/Tab**: 선택 항목 `InsertText`를 커서 위치에 삽입. **Esc**: 닫기.
3. **자동 삽입 없음** — 타이핑만으로 삽입 0(명시 선택 시에만). accept 시 UX-WP-01 audit 호출(해시 전용).
4. Safety finding(`SafetyHint`/`BlockedHint`)은 **기존 우측 결과 패널**(`ShowFindings`)과 연계.
...
## 테스트 (Windows 로컬)
- 자동 삽입 없음(타이핑→삽입 0; 선택 시에만 InsertText). 항목에 Source/Kind/RequiresReview 노출. Safety finding 결과패널 연계. (UI는 가능 범위에서 계약/뷰모델 단위 테스트 — SmokeTest 분류 키워드 포함.) `Total` 보존+신규.

## 완료/보고
Ctrl+Space/Enter·Tab/Esc + 자동삽입 없음 + 감사 연계. **로컬** build 0/0·SmokeTest `Total=N PASS/0 FAIL`·Gate A·NuGet 0. `docs/39` UX-WP-03 DONE 요청.

## Claude Review Checklist
외부 Editor 패키지 0(WPF 기본만) / **자동 삽입 없음** / Source·Kind·RequiresReview 노출 / 결과패널 연계 / 입력 원문 미저장 / 자동 실행 0 / NuGet 0 / 기존 테스트 불변 / Gate A.
