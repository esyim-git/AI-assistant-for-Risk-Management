...
## 1. 작업 범위
1. **Example 본문 출처 = `FeedbackLogEntry`에 `string? DraftBody = null` 추가(맨 끝 nullable, 위치 불변).** 기존 6-positional 생성자 호출·`AuditTests`(라인 ~29·68) 회귀가 깨지지 않도록 **반드시 맨 끝 nullable**. `FeedbackLogWriter`/MainWindow 입력 경로가 본문 전달. 본문 없으면 `ExampleBody=null` + 검색은 메타만(정상).
...
- 음성: audit에 본문/raw 평문 미저장 단언. `.gitignore`에 `promoted_examples` 포함 단언(Packaging/Audit 도메인).
- 기존 `AuditTests`(FeedbackLogEntry 6-positional·PromotedExample append/dedupe·원문 미저장) **전부 보존**(신규 필드 맨 끝 nullable).
