...
1. **디바운스 TextChanged 트리거 추가**: `RegisterCompletionTextBox`(MainWindow.xaml.cs:173-177)에 **debounced `TextChanged`** 핸들러를 추가해, 기존 **`ShowCompletionPopup(textBox, language)`(line 207)를 재사용**해 호출한다(엔진/팝업/안전핀/audit 경로 그대로). 디바운스 = 인박스 `DispatcherTimer`(예: 150~250ms, 마지막 입력 후 1회) — **신규 NuGet 0**.
2. **최소 prefix 게이트(필수)**: as-you-type는 **`ExtractCompletionPrefix`(308-323)·`IsCompletionPrefixChar`(325-328) 재사용**해 산출한 prefix 길이가 **임계(예 ≥2자) 이상일 때만** 트리거한다. (empty/짧은 prefix는 `MatchesPrefix`가 **전체 항목**을 반환하므로 매 키 입력마다 전체 덤프 방지.) **Ctrl+Space는 현행 유지**(임계 무관, 전체 표시 가능).
...
4. **결정 로직 = 테스트 가능한 순수 헬퍼로 추출**: "as-you-type 트리거 여부"(prefix 길이 임계·suppress·언어 지원) 판정을 **순수 static 메서드/함수**(Core 또는 App static, WPF 비의존)로 추출해 SmokeTest로 단언 가능하게 한다. 실제 팝업 렌더는 SmokeTest 불가 → **Gate B(Test PC)** 의존.
...
## 2. 제외 범위
`CompletionEngine`/providers/contracts **Core 변경**(이미 충분). 신규 provider·실시간 LLM 랭킹(R4 STOP)·자동삽입·surface audit·**팝업 표시 enrich(UX-WP-06)**·caret-relative placement(UX-WP-06). Vector/모델/NuGet.
...
- **결정 헬퍼 단언**(WPF 비의존): prefix 길이 < 임계 → as-you-type 트리거 안 함 · prefix ≥ 임계 → 트리거함 · suppress(프로그램적 편집) 중 → 트리거 안 함 · 미지원 언어/빈 텍스트 → 트리거 안 함 · Ctrl+Space 경로는 임계 무관(분리) 단언.
- **회귀 보존**: 기존 `AssistTests`(엔진 결정성·cap·dedupe·accept 엔트리·prefix 필터) **전부 PASS**. accept audit 해시 전용 불변.
- WPF 팝업 실제 표시/디바운스 타이밍 = **Gate B(Test PC)**(SmokeTest 범위 밖).
...
Core 엔진 변경 0(WPF 트리거만)·`ShowCompletionPopup` 재사용 / 디바운스=인박스 `DispatcherTimer`(NuGet 0) / **최소 prefix 게이트**(전체 덤프 방지)·`ExtractCompletionPrefix` 재사용 / **재진입 가드**(프로그램적 편집 suppress·수락 후 재트리거 0) / Ctrl+Space·팝업 네비·accept 삽입·accept audit 불변 / 자동삽입 0·surface audit 0(accept만)·입력 원문 미저장 / NoModel 보존·실시간 LLM=R4 STOP / 결정 로직 순수 헬퍼로 SmokeTest 가능 / 기존 `AssistTests` 보존 / `Total` 보존+신규·Unclassified 0 / Gate A. **B-5 종료는 본 WP(트리거)로 입력중 추천이 실제 표면화될 때**(렌더 검증=Gate B). **프롬프트**: 본 파일.
