...
현재 main = `06fb8fc`, **VERSION 0.6.0** (v0.6.0 태그 `3dfa80b`). 정본 SmokeTest = **Total=631 PASS=631 FAIL=0** (UX-WP-01 정본 602 + UX-WP-02/03 `Assist` 도메인 +29). R1(Data & Limit Foundation, v0.5.0 완료)과 R3(Regulation/NCR 구조, v0.6.0 완료 — **공개 규정 Metadata 기반 RAG 구조** + **NCR Rule Set 구조**; 규정 원문 RAG·실제 NCR 산정 아님)가 완료되어 있다. STAB v0.6.1 WP-01/02/03a/03b/04 + UX-01(Resizable Layout) + **UX-WP-01(Smart Assist 코어)·UX-WP-02(정적 Provider)·UX-WP-03(WPF Popup) 완료 — UX 트랙 완료**(#56·#57·#59·#61·#66·#68·#70·#72·#73, STAB-WP-05 코드서명 APPROVAL_REQUIRED). UX 트랙 VERIFIED 범위는 **정적·NoModel·외부 Editor 0·자동삽입 0** 한정이며 실 LLM 랭킹/학습=**R4(미구현)**·실 Test PC Gate B/C=**BLOCKED**(과대표기 금지). **NEXT UP = STAB-UX-02**(레이아웃 영속화; 대안 R2-WP-01). R2 Risk Analytics=미구현/다음 · R4 Local LLM=설계/승인대기(Runtime APPROVAL_REQUIRED, STOP) · R5 Feedback Learning=일부 구조 있음·실사용 확장 필요 · R6 Team Pilot=Gate B/C 미완료(실 Test PC 증거 BLOCKED). **완료된 MVP-1~3, R1, R3를 재구현하지 않는다.** 새 작업은 v0.6.0 다음 단계부터, **하나의 Work Package(WP) 단위**로만 수행한다.
