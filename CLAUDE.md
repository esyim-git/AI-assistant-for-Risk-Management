...
## 12. Skill 운영 원칙 (Project Skills)

반복 절차/체크리스트는 CLAUDE.md에 길게 누적하지 않고 **Skill(`.claude/skills/<name>/SKILL.md` + support)로 분리**한다. 인덱스 = root **`SKILLS.md`**. Skill 문서는 **코드 동작을 바꾸지 않으며**, 절대 원칙(§3)·STOP(§11.5)·과대표기 금지(§11.4)를 전제한다.

- Claude는 작업 시작 시 **필요 Skill을 선택**해 그 SKILL.md/support를 따른다.
- Codex에게 WP를 넘기기 전 **`risk-wp-planner`**로 WP+Codex 프롬프트를 작성한다.
- Codex 결과는 **`risk-codex-review`**(4축: 범위·보안·테스트·문서)로 검토한다.
- **릴리스 컷(REL WP)은 `risk-release-cut`**, **Release 검증 전 `risk-release-verify`**, **Gate B/C 전 `risk-gate-bc`**를 사용한다.
- **구조 리팩터(행위 불변 분해 — God-class/탭 추출)는 `risk-arch-refactor`** 없이 진행하지 않는다(행위 변경 0·계약 테스트 이전·MVVM 빅뱅 금지).
- **Team Pilot 준비/운영은 `risk-team-pilot`**을 적용한다(Gate B/C 봉인 선행·실 증거 없는 완료 표기 금지).
- **Local LLM 관련 작업은 `risk-llm-approval` 없이는 진행하지 않는다**(STOP·Model Approval 요건).
- **RAG/NCR 관련 작업은 `risk-rag-ncr-governance`**를 적용한다.
- 상태 정본화 = `risk-status-sync` → 머지 후 `risk-doc-truth-sync`(기준선 이중 표기: 코드/테스트 baseline vs current main — docs-only 머지는 baseline 미변경). 도메인별: 데이터/한도 `risk-data-limit-review` · 분석/시각화 `risk-analytics-design` · 피드백 `risk-feedback-learning` · UI/UX `risk-ui-ux-review` · 보안 `risk-security-guard` · 테스트 `risk-smoke-governance` · 브랜치/PR `risk-branch-governance`.
- 호출 순서(Release/PR/Gate/Pilot)는 `SKILLS.md §9~12` 참조.

---

## 13. 자동 Skill Preflight (Automatic Skill Preflight)

§12의 Skill을 **사용자가 매번 `/risk-*`를 수동 호출하지 않아도** Claude가 작업 유형을 인지하는 순간 **표준 절차로 스스로 적용**한다(자동 Preflight). 이는 §3 절대원칙·§11.4 과대표기 금지·§11.5 STOP을 전제로 하며 **코드 동작·테스트를 바꾸지 않는다**. path-scoped Skill(`risk-security-guard`·`risk-smoke-governance`·`risk-data-limit-review`·`risk-rag-ncr-governance`·`risk-ui-ux-review`)은 Claude Code가 해당 경로 작업 시 자동 표면화하고, 나머지는 아래 체인으로 자동 적용한다.

### 13.1 작업 유형 → 자동 Preflight 체인
| 작업 유형 | 자동 Preflight 순서 |
|---|---|
| 계획 / 다음 방향 / WP / Codex Prompt 작성 | `risk-status-sync` → `risk-doc-truth-sync` → `risk-wp-planner` |
| PR / Diff 검토 | `risk-codex-review` → `risk-security-guard` → 도메인 Skill → (테스트 변경 시) `risk-smoke-governance` |
| UI / WPF / SQL·VBA Editor / Smart Assist | `risk-ui-ux-review` → `risk-smoke-governance` → `risk-security-guard` |
| Data / Limit / Report | `risk-data-limit-review` → `risk-smoke-governance` → `risk-security-guard` |
| RAG / NCR / KB | `risk-rag-ncr-governance` → `risk-security-guard` |
| Release / Package (컷 포함) | `risk-release-cut`(REL 컷 절차) → `risk-release-verify` → `risk-security-guard` |
| 구조 리팩터(행위 불변 분해) | `risk-arch-refactor` → `risk-ui-ux-review` → `risk-smoke-governance` → `risk-security-guard` |
| Gate B/C | `risk-gate-bc`(증거 민감정보 0 — dummy·masking만) → `risk-release-verify` |
| Team Pilot 준비/운영 | `risk-gate-bc`(봉인 확인 선행) → `risk-team-pilot` → `risk-feedback-learning` |
| Local LLM / Runtime / Model / Embedding | `risk-llm-approval` → **STOP**(의존성·모델·Runtime 추가 전 승인 선행) |

### 13.2 STOP Gate (자동 실행 아님 — 승인형)
`risk-release-verify` · `risk-gate-bc` · `risk-llm-approval` · `risk-team-pilot`은 Preflight로 **참조**하되 **자동 실행·자동 PASS가 아니다.** 실 오프라인 Test PC 증거(Gate B/C) · 인증서/서명 승인(Release) · 모델/Runtime 승인(LLM) · 실 파일럿 증거(Pilot)가 없으면 **PASS 금지·진행 금지**(§11.4·§11.5). 자동 적용은 "체크리스트 적용"이지 "게이트 통과"가 아니다.

### 13.3 분류 (정본 = `SKILLS.md §6`)
- **자동 적용(path-scoped)**: `risk-security-guard`·`risk-smoke-governance`·`risk-data-limit-review`·`risk-rag-ncr-governance`·`risk-ui-ux-review`.
- **자동 Preflight(표준 절차)**: `risk-status-sync`·`risk-doc-truth-sync`·`risk-wp-planner`·`risk-codex-review`·`risk-analytics-design`·`risk-feedback-learning`·`risk-branch-governance`·`risk-release-cut`·`risk-arch-refactor`.
- **STOP Gate(승인형)**: `risk-release-verify`·`risk-gate-bc`·`risk-llm-approval`·`risk-team-pilot`.

> Codex 측 자동 참조(Automatic Skill Bridge)는 `AGENTS.md §9`. 사용자에게 매번 문서 목록을 요구하지 않는다.
