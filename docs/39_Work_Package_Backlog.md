...
- **NEXT UP (다음 WP)**: **`STAB-UX-02`(Resizable Layout 영속화)** — STAB-UX-01의 창 크기·에디터/결과 분할 비율·Safety 컬럼 너비를 `config/ui_layout.local.json`에 세션 간 영속화(순수 WPF + 인박스 `System.Text.Json`, 외부 NuGet 0·실데이터 0·자동실행 0). **무결성 제약**: 레이아웃 파일은 런타임 가변 상태이므로 manifest/`IntegrityVerifier`/패키징/git **비포함**(레이아웃 변경이 STAB-WP-03b Fail-Closed 오발 금지). **WP 상세 = `docs/39 §STAB-UX-02`, Codex 프롬프트 READY = `prompts/codex/STAB-UX-02_layout_persistence.md`** → Codex 핸드오프 가능. 대안 후보 = **R2-WP-01**(Risk Semantic Hardening — 분석 트랙). 병행: PILOT Gate B/C(BLOCKED, 실 Test PC).
- **그 다음 후보(순서, NEXT UP 아님)**: STAB-UX-02(레이아웃 영속화) → **STAB-WP-05(Authenticode 코드 서명 — APPROVAL_REQUIRED, 외부 신뢰 루트, STOP)** → R2-WP-01(Risk Semantic Hardening) → R2-WP-02~04.
...
## STAB-UX-02. Resizable Layout Persistence (세션 간 레이아웃 영속화) — **NEXT UP** (CAP-UX 사용성)
...
- **작업범위**:
  - `Core`(또는 `App`) 레이아웃 store: `config/ui_layout.local.json`에 `{ WindowWidth, WindowHeight, EditorRowStar, ResultRowStar, SafetyColumnWidth, SchemaVersion }` 직렬화. **경로 가드**는 기존 `PolicyLoader`/`LogPathResolver` 패턴 재사용(`config/`-상대 JSON만, traversal/rooted 거부).
...
- **수정예상파일**: `Core/Config/UiLayoutStore.cs`(신규) 또는 `App/` 동등 헬퍼, `App/MainWindow.xaml.cs`(load/save 연결), `.gitignore`(+`config/ui_layout.local.json`), `tests/.../`(round-trip·fallback·경로가드 SmokeTest).
...
- **보안조건(★ 무결성 제약)**: `config/ui_layout.local.json`은 **런타임 사용자 가변 상태** → **build/01 manifest·`IntegrityVerifier`(Mandatory/RequiredCritical/CriticalGlobs)·패키징 인벤토리·git 어디에도 포함 금지**(포함 시 레이아웃 변경마다 STAB-WP-03b Fail-Closed 오발). config 루트(`config/ncr/` 아님)에 두어 `CriticalGlobs(config/ncr,*.json)` 비대상 유지. `.gitignore`에 추가. 외부 NuGet 0·실데이터 0.
- **테스트**(SmokeTest, 외부 프레임워크 0): round-trip(Save→Load 동일값), 파일 부재/손상 JSON → 기본값 fallback(예외 0), 경로 가드(`config/` 밖·traversal 거부), clamp(Min/Max 범위 밖 저장값 보정), **레이아웃 파일이 무결성 critical 인벤토리에 없음**(부재/변경이 startup Fail-Closed 유발 안 함) 단언, `.gitignore` 포함 확인. 이름에 `layout`/`UiLayout` 등 → 분류(`UiContract`, Unclassified 0). 기존 `Total=631` 보존+신규.
