...
- **현재 기준선**: main `685eed6` (PR #86 truth-sync 후), VERSION **0.6.0** (**v0.6.0 정식 릴리스 태그 = `3dfa80b`**; R2 트랙 v0.7.0 진행 중·미릴리스). R1(WP-01~08)·R3(R3-WP-01~05)·REL-v0.6 가드(#54)·**STAB-WP-01~04(#56/#57/#59/#61/#66)·STAB-UX-01(#68)·UX-WP-01(#70)·UX-WP-02(#72)·UX-WP-03(#73)·STAB-UX-02(#76)·R2-WP-01(#79)·R2-WP-02(#81)·R2-WP-03(#84)** 모두 **DONE**. 추가 머지: truth-sync(#63·#67·#69·#74·#78·#80·#82·#83·#85·#86). SmokeTest **`Total=698 PASS=698 FAIL=0`**(직접 local-gate 재확인: `dotnet build` 0/0 + SmokeTest, 2026-06-30; 631 + STAB-UX-02 +15 + R2-WP-01 +25 + R2-WP-02 +9 + R2-WP-03 +18). **R2-WP-04 Codex local-gate candidate**: `feature/r2-wp-04-visualization-report`에서 정확 Exception Count(Number SoT)·`RISK_VISUAL` 시트·WPF Shapes/Canvas 화면을 구현, Codex bot P2 4건(캔버스 높이·기본폭 clipping·TopN CurrencyCode 라벨·visual finding 반환) 보정, `dotnet build RiskManagementAI.sln -c Release` 0/0, SmokeTest **`Total=714 PASS=714 FAIL=0`**(+16, Release, Unclassified=0), PackageReference 0, 변경분 secret/주민번호 0. **Claude review/merge pending**이며 실 오프라인 Test PC Gate B/C=**BLOCKED**(과대표기 금지). STAB-WP-05(코드서명)=APPROVAL_REQUIRED.
...
- **NEXT UP (다음 작업)**: **R2-WP-04 Claude review/merge**. Codex 구현 후보는 local-gate를 통과했지만 merge 전 상태는 **PARTIAL**이다. Review 기준: C-16 정확 카운트·`RISK_VISUAL` 집계 SoT·WPF 화면 한정·NuGet 0·Gate A·기존 계약 보존. 병행: PILOT Gate B/C(BLOCKED, 실 Test PC).
- **그 다음 후보(순서, NEXT UP 아님)**: R2-WP-04 review/merge 후 R2 track truth-sync/release prep(v0.7.0) → 이후 KB(원문 Clause/Chunk, Vector/Embedding STOP·승인 필요) / R5 Feedback / PILOT Gate B/C(실 Test PC 증거). **STAB-WP-05**(Authenticode 코드 서명)=**APPROVAL_REQUIRED/STOP**이라 구현 NEXT UP 부적격.
...
> 상태: **PARTIAL (Codex local-gate candidate, review/merge pending)**. `feature/r2-wp-04-visualization-report`에서 구현·검증 완료: `dotnet build RiskManagementAI.sln -c Release` 0/0, `dotnet run --project tests\RiskManagementAI.SmokeTests -c Release` → `Total=714 PASS=714 FAIL=0`, Unclassified=0, PackageReference 0, 변경분 secret/주민번호 0. Codex bot P2 4건 보정 완료. Claude review/merge와 실 Test PC Gate B/C는 아직 **BLOCKED/PENDING**.
