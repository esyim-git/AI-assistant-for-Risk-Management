# 39. Work Package Backlog and Resume Brief

> `docs/38` Release Train의 실행 단위(Work Package). Local Codex는 WP 단위로 계획·구현·검증·Draft PR을 만들고, 중요 PR은 최종 diff를 작성하지 않은 별도 Local Codex task/context가 exact-head 독립 리뷰한다.
> WP 형식: 목표·선행조건·작업범위·제외범위·읽을문서·수정예상파일·Public Interface·구현세부·보안조건·테스트·완료조건·Branch·Commit·Independent Review Checklist.
> 각 WP는 **하나의 명확한 목표**만 가진다. R1 Codex 프롬프트: `prompts/codex/WP-XX_*.md`.

---

## ★ Current Resume Brief (2026-07-13)

- **Truth-sync base**: `origin/main` `b4eafce1d0d26383081e189cc46388480a74b0f7` (PR #140, Local Codex workflow/docs migration) as observed on 2026-07-13. **Product code-test baseline**: `0a3386f5de8209ced9443d371375d63ee0309343` (PR #139, ARCH-WP-01). **Published release Build Commit** remains `fa7552567cb432ec6a4afe9900b3eca480fc5780` (PR #136). These are three distinct baselines; this planning PR may advance current main only.
...
- **Current product local gate** (2026-07-13, current main): `dotnet build RiskManagementAI.sln -c Release` warning 0/error 0; SmokeTest `Total=910 PASS=910 FAIL=0`, Unclassified 0; PackageReference 0. The +3 are ARCH-WP-01 UiContract structural guards. **Published v0.7.1 package gate** remains build/00~03 PASS and `Total=907` at `fa755256`; it is not relabeled as 910.
...
- **ARCH-WP-01 = VERIFIED** (#139, product baseline `0a3386f`): `MainWindow.xaml.cs` 1,614 -> 345 lines, six role partials, XAML and behavior contracts unchanged, additive UiContract +3, hosted `test`/`wpf-build` success.
- **NEXT UP = UI-WP-12** (existing Prior-Day Core -> Risk Dashboard/report reachability; prompt `prompts/codex/UI-WP-12_prior_day_wpf_reachability.md`). This is the only implementation WP selected by this truth-sync. **Parallel user-driven**: published v0.7.1 Gate B/C round (`docs/54`).
- **After NEXT UP**: DATA-UI-WP-01 -> KB-UI-WP-01 -> FEEDBACK-WP-03 -> RUNTIME-WP-01 -> formal Gate B/C closure -> Team Pilot. These are sequencing context, not concurrently authorized NEXT UP items.
- **Approval tracks remain STOP**: STAB-WP-05 signing, real NCR/internal Pack, Local LLM runtime/model.
- **Audit basis**: `docs/53_Repository_Audit_and_v1_Execution_Plan.md` is the 2026-07-11 pre-ARCH assessment snapshot. Current execution truth is `docs/38` + this Resume Brief; published release evidence remains `docs/52_Release_v0.7.1.md` (completed CORR/REL prompts are not rerun).

## Historical Resume Snapshot (through PR #131; retained for audit)
> 이 블록 안의 `NEXT UP`/`다음 WP` 표기는 당시 시점의 역사 기록이며 현재 실행 지시가 아니다. 현재 NEXT UP 정본은 위 Current Resume Brief의 `UI-WP-12` 하나다.
...
- **R2-WP-04 DONE (#87 merged, main `35e6f01`, VERIFIED — local-gate; Claude 4축 리뷰 APPROVE + bot P2 4건 보정)**: `RiskVisualAggregator`(신규 Core) — 결정적(Ordinal). 7상태 분포 `Enum.GetValues`로 DuplicateLimit 자동 포함, **HHI 분모=`Σ Abs(Exposure)`·분모0 graceful**(`VISUAL_CONCENTRATION_ZERO_DENOMINATOR`), TopN=`Abs(Exposure)` desc+tie-break, Heatmap 경계(<0.8 LOW/≤1.0 MID/>1.0 HIGH), `MIXED_CURRENCY`, decimal 4dp/6dp 고정. `ExcelReportBuilder`: SUMMARY `ExceptionCount`를 부정확 `=COUNTA(...)` 제거 후 **정확 Number SoT**(`CountExceptions`), 신규 `RISK_VISUAL` 시트(Number/text 정적값만→Excel2021 수식게이트 위험 0), `ExpectedSheetNames` 10→11 자동배선. `RenderRiskCharts`가 **동일 aggregator SoT**로 WPF Canvas/Shapes 화면차트(외부 charting NuGet 0). `BuildReport` 시그니처·`LimitAnalysisResult`/7상태/RECON 불변. SmokeTest **`Total=698→714 (+16)`**. **머지 전 bot P2 4건 보정(`6845884`)**: ① 시각화 caveat를 `ExcelReportResult.Findings`로 표면화(`WorkbookBuildResult.Findings`), ② TOP_EXPOSURE `CurrencyCode` 라벨 정렬, ③ Canvas MinHeight 250→360(heatmap 잘림 해소), ④ Risk Dashboard 컬럼 최소폭 722→528px(clipping 완화). NuGet 0. **잔여(기록만)**: 실 Excel 2021 수동열기·WPF 다양 창크기 렌더 = **Gate B/C(BLOCKED)**; SmokeTest는 ZIP 구조·`sheet11.xml`·셀 내용까지 검증(local-gate 범위). 실 Test PC Gate B/C=**BLOCKED**.
- **당시 다음 WP = REL-WP-071**(현재 NEXT UP 아님; v0.7.1 출하 정합 릴리스 컷 — 본 원장 항목·`prompts/codex/REL-WP-071_release_cut_v0_7_1.md`·릴리스 문서 `docs/52`): 방향 결정 대기는 제안서 채택(PR #131 `110e9ee`)으로 해소됐고 이후 완료됐다. 당시 병행 Gate/승인 후보는 역사 근거로만 보존한다.
- **WP 시퀀스(완료)**: KB-WP-01(#94) → KB-WP-02(#101) → UX-WP-04/05/06(#102~#104) → FEEDBACK-WP-01(#106) → FEEDBACK-WP-02(#108) → cleanup(R2-WP-05 #109·UX-WP-07 #110·UX-WP-08 #111). **STAB-WP-05**(코드서명 — **인증서 경로 A 확정**; ADR-012/`docs/41 §6` 나머지 항목·서명 도구는 구현 WP에서 확정, 실 인증서·서명·검증 = Windows 실 Test PC 의존)는 병행 가능. 병행: PILOT Gate B/C(BLOCKED, `docs/48` 시트 준비됨).
...
## ARCH-WP-01. MainWindow 행위 불변 partial 분해 1단계 — **VERIFIED (#139, `0a3386f`)** (Cap C-33, RR-21)

- **상태증거**: PR #139 squash merged at `0a3386f5de8209ced9443d371375d63ee0309343`. `MainWindow.xaml.cs` 1,614 -> 345 lines; `MainWindow.CompletionAssist.cs`, `MainWindow.Navigation.cs`, `MainWindow.SafetyDraftExcel.cs`, `MainWindow.KnowledgeAudit.cs`, `MainWindow.DataRiskReport.cs`, `MainWindow.Presentation.cs` six partials are present. XAML byte/handler signatures and moved method-block hashes were preserved. Additive UiContract guards +3 raised the product baseline `907 -> 910`; local build 0/0, `Total=910 PASS=910 FAIL=0`, Unclassified 0, PackageReference/Gate A 0, exact-head hosted `test`/`wpf-build` success, independent review clean.
...
- **선행조건**: GOV-WP-02 `VERIFIED`; implementation-start baseline `4efb8e6`, SmokeTest `Total=907 PASS=907 FAIL=0`; `MainWindow.xaml.cs` 1,614줄 기준.
...
- **테스트**: 기존 907개 단언과 assertion message 삭제·약화·개명 0. 정렬된 partial-source aggregation으로 기존 `MainWindow` 계약 단언을 보존하고, 본 파일 `<=600`줄·필수 partial 목록·전체 source coverage를 additive 단언했다. Release build 0/0, SmokeTest `Total=910 PASS=910 FAIL=0`, Unclassified 0, hosted `test`/`wpf-build` exact-head success.
- **완료조건**: 충족. `MainWindow.xaml.cs` 345줄, 지정 역할별 partial 파일 존재, XAML/사용자 동작/메서드 시그니처/Core/출력·audit 계약 변화 0, 기존 단언 전부 보존 + 구조 가드 3건, PackageReference 0, Gate A 0. 다음 `UI-WP-12`는 `MainWindow.DataRiskReport.cs` 중심의 좁은 변경으로 착수 가능하다.
...
- **Independent Review Checklist**: PASS — one-phase scope / verbatim method moves / XAML·event signatures·user behavior unchanged / DTO·MVVM·Core·new feature 0 / `MainWindow.xaml.cs <=600` / deterministic `MainWindow*.cs` aggregation / all prior assertions/messages preserved / Total non-decrease·Unclassified 0 / build + exact-head hosted checks / NuGet 0 / Gate A 0 / method-to-file and line-count evidence.

> Archived start prompt (재실행 금지): `prompts/codex/ARCH-WP-01_mainwindow_partial_decomposition.md`.

## UI-WP-12. Prior-Day Analytics WPF + Report Reachability — **NOT_IMPLEMENTED (NEXT UP, prompt READY)** (Cap C-34, RR-20)

- **목표**: 이미 검증된 `PriorDayAnalyzer` 결과를 사용자가 Risk Dashboard에서 명시적 Current/Prior 기준일로 실행·검토하고, 동일 결과를 Excel 2021 리포트의 `PRIOR_DAY` 시트로 내보낼 수 있게 한다.
- **선행조건**: ARCH-WP-01 `VERIFIED` (#139, `0a3386f`); product code-test baseline `Total=910 PASS=910 FAIL=0`; R2-WP-03 `PriorDayAnalyzer`/4구획 계약과 R2-WP-04 report builder는 VERIFIED. 외부 의존성 승인 불필요.
- **작업범위**: Risk 탭에 Prior BASE_DT 입력·명시 실행 버튼·요약/비교 테이블을 추가하고 기존 Current BASE_DT, exposure path, limit path를 재사용한다. `MainWindow.DataRiskReport.cs`가 기존 `PriorDayAnalyzer`를 호출해 KPI, Current/Prev/delta, movement, TopN movers, Methodology/UserValidation/HiddenRisk를 표시한다. 마지막으로 성공한 현재 입력의 prior-day 결과만 Report 생성 요청에 선택적으로 전달하고, `ExcelReportBuilder`가 정적값 전용 `PRIOR_DAY` 시트를 생성한다.
- **제외범위**: 새 분석·movement·7상태·RECON 로직, 자동 직전 영업일 계산, 파일명/경로/시트 자동 추론, 백그라운드 streaming/progress/cancel(`DATA-UI-WP-01`), 신규 차트, LLM 코멘터리, 실제 데이터, VERSION/release/Gate B/C 상태 변경.
- **읽을문서**: `AGENTS.md`, `SKILLS.md`, `CLAUDE.md`, `docs/38` C-34/RR-20, 본 항목, `docs/40` ADR-002/011, `docs/41`, `docs/28`, `docs/53 §2·§4`(pre-ARCH audit basis), `.claude/skills/risk-ui-ux-review/`, `.claude/skills/risk-data-limit-review/`, `.claude/skills/risk-smoke-governance/`, `.claude/skills/risk-security-guard/`, `.claude/skills/risk-branch-governance/`, `Core/Risk/PriorDay*`, `Core/Report/ExcelReportBuilder.cs`, `App/MainWindow.xaml`, `App/MainWindow.DataRiskReport.cs`, UiContract/Limit/Report tests.
- **수정예상파일**: `src/RiskManagementAI.App/MainWindow.xaml`, `src/RiskManagementAI.App/MainWindow.xaml.cs`(필드/생성자만), `src/RiskManagementAI.App/MainWindow.DataRiskReport.cs`, `src/RiskManagementAI.Core/Report/ExcelReportBuilder.cs`, `tests/RiskManagementAI.SmokeTests/UiContractTests.cs`, `LimitReconciliationTests.cs`, `ReportTests.cs`. 새 분석 Core 파일은 만들지 않는다.
- **Public Interface**: `ExcelReportRequest` 끝에 source-compatible optional `PriorDayAnalysisResult? PriorDayAnalysis = null`을 추가하고 `ExcelReportBuilder.ExpectedSheetNames`에 `PRIOR_DAY`를 add-only한다. 기존 7인자 호출, `PriorDayAnalyzer`, `LimitAnalysisResult`, 7상태/RECON 계약은 불변이다.
- **구현세부**: 사용자가 Current/Prior 날짜와 경로를 명시한 뒤 버튼을 눌러야만 실행한다. 동일 날짜는 기존 normalized same-day guard를 그대로 표면화한다. 성공 결과는 입력 signature와 함께 메모리에만 보관하고, 경로/날짜 입력이 바뀌면 stale result를 즉시 폐기한다. Report는 유효한 cached result만 받아 같은 `ComparisonTable`/movers/4구획을 정적 OOXML 값으로 기록하며 report-side 재계산은 하지 않는다. 표시/행 정렬은 Core 결과 순서를 보존한다.
- **보안조건**: 외부 NuGet/API/자동실행 0; NoModelMode 유지; 원문 경로·사용자 입력을 로그에 저장하지 않고 기존 `TaskLogWriter` SHA-256 audit만 사용; report 쓰기는 `reports/` 가드 유지; 실데이터/실스키마/원문/모델/인증서 0. audit 실패는 기존 `TASK_LOG_WRITE_FAILED` 비치명 정책을 유지한다.
- **테스트**: 기존 910개 단언/메시지 보존 + additive UiContract/Limit/Report 회귀. 양성: stable controls/event wiring, Current/Prior/Delta와 six movement 값, TopN/4구획, optional result가 있을 때 `PRIOR_DAY` 시트와 Core 행 수/값 일치, 없을 때 기존 report 호출/내용 호환. 음성: normalized same-day, prior 선택 행 0, duplicate key Hidden-Risk, 입력 변경 후 stale report 차단, unsafe report path. 결정성, Excel2021FunctionChecker, Unclassified 0을 검증하며 고정 최종 Total을 미리 발명하지 않는다.
- **완료조건**: Risk Dashboard에서 명시적 prior-day workflow가 도달 가능하고 Report의 `PRIOR_DAY` 값이 동일 Core result와 일치한다. 기존 단언 감소 0; build warning 0/error 0; SmokeTest `Total=N PASS=N FAIL=0`, Unclassified 0; PackageReference 0; Gate A 0; exact-head hosted `test`/`wpf-build` success. 실제 WPF 렌더와 Excel 2021 수동 열기는 `docs/54` Gate B 증거 전까지 BLOCKED로 유지한다.
- **Branch**: `feature/ui-wp-12-prior-day-wpf-reachability`.
- **Commit**: `feat: expose prior-day analytics in WPF and report (UI-WP-12)`.
- **Independent Review Checklist**: existing `PriorDayAnalyzer` single source / no report-side recomputation / explicit Current+Prior input and no auto inference / same-day and missing-prior graceful / cached-result invalidation / six movement + 4-section + Hidden-Risk visible / optional report API backward compatible / `PRIOR_DAY` static values and Excel 2021 compatible / hash-only audit / no new Core engine or STOP dependency / prior assertions preserved + additive tests / Total non-decrease·Unclassified 0 / build + exact-head hosted checks / NuGet 0 / Gate A 0 / Gate B not overclaimed.

> Codex start prompt: `prompts/codex/UI-WP-12_prior_day_wpf_reachability.md`.
...
- **목표**: 한도/대사 check가 0건일 때 Excel Report SUMMARY가 `ReconciliationPassed=PASS`를 쓰지 않고 `NOT_RUN`을 기록하도록 해 High `LIMIT_DATA_REQUIRED`와 상충하는 false-pass 표시를 제거한다.
- **선행조건**: REL-WP-071 code cut #133 merged (`abab29b`, VERSION 0.7.1). v0.7.1 tag/Release는 아직 없어 수정 후 같은 버전으로 재패키징 가능하다.
- **작업범위**: `ExcelReportBuilder`의 reconciliation display state를 `CheckCount == 0 -> NOT_RUN`, `CheckCount > 0 && Passed -> PASS`, 그 외 `FAIL`로 단일화한다. `MainWindow.BuildEmptyLimitAnalysis`는 zero-check 상태를 성공으로 해석하지 않도록 정합한다. Report/UiContract SmokeTest에 zero/pass/fail 회귀를 추가한다.
- **제외범위**: Reconciliation engine/check 9종, `ReconciliationSummary` public record 재설계, LimitMonitor 상태/대사 수 변경, Prior-Day/UI/architecture refactor, VERSION 범프, release tag/publish.
- **읽을문서**: `AGENTS.md`, `docs/38` C-31/RR-17, `docs/40` ADR-002/011, `docs/53 §3`, `docs/28`, `prompts/codex/CORR-WP-01_reconciliation_not_run.md`.
- **수정예상파일**: `src/RiskManagementAI.Core/Report/ExcelReportBuilder.cs`, `src/RiskManagementAI.App/MainWindow.xaml.cs`, `tests/RiskManagementAI.SmokeTests/ReportTests.cs`, 필요 시 `UiContractTests.cs`.
- **Public Interface**: 없음. `ReconciliationSummary` shape와 `ExcelReportBuilder.BuildReport` signature 불변.
...
## REL-v0.7.0. R2 분석 트랙 v0.7.0 정식 릴리스 컷 (버전 범프 락스텝) — **DONE (#90, VERIFIED — local-gate)**
- **상태**: **DONE (#90 머지 `30c1cfb`, VERIFIED — local-gate; Claude 4축 리뷰 APPROVE, 머지 diff 3파일/24+24 확인)**. VERSION `0.6.0→0.7.0` + 락스텝(`IntegrityVerifier.ExpectedVersion="0.7.0"`·`PackagingTests` 22리터럴), **기능 0·`Total=714 PASS=714 FAIL=0` 불변**(drift 가드 `:331` 통과). build/00~03(`-Version 0.7.0`) PASS, **v0.7.0 정식 릴리스 발행**(태그 `30c1cfb`·미서명 portable ZIP·SHA256 `42C835983127B127438AB97747B99FD0C3FA2E4363D4CB85641E45FE62E09DD5`·manifest version 0.7.0/entries 26·ReleaseNote Build Commit `30c1cfb`). 외부 NuGet/서명/인증서 0(`dotnet list package` PackageReference 0). **잔여**: 실 Excel 2021 수동열기/오프라인 기동 = Gate B/C(BLOCKED); 코드서명 = STAB-WP-05(APPROVAL_REQUIRED) 후속. (선행 #88 truth-sync 머지 완료.)
- **목표**: R2 트랙 완결(main `35e6f01`, `Total=714`)을 **v0.7.0**으로 정식 릴리스하기 위한 **버전 범프**. `VERSION` 단일원천(ADR-006)과 그에 락스텝으로 묶인 상수/테스트만 `0.6.0→0.7.0`으로 올린다. **기능 코드 변경 0.** 패키징·태깅은 Local-Gate(Windows).
- **선행조건**: R2-WP-04 truth-sync(#88) 머지(기준선 main `VERSION=0.6.0`, `Total=714`). 외부 의존성·서명 도구·인증서 **추가 0**(코드 서명 = STAB-WP-05 분리).
- **작업범위(정확히 3파일, 그 외 0)**:
  1. `VERSION`: `0.6.0` → `0.7.0`.
  2. `src/RiskManagementAI.Core/Integrity/IntegrityVerifier.cs:27` `ExpectedVersion = "0.6.0"` → `"0.7.0"`(누락 시 manifest version 불일치 → 런타임 Fail-Closed brick + drift 가드 테스트 red).
  3. `tests/RiskManagementAI.SmokeTests/PackagingTests.cs` 합성 manifest 버전 리터럴 `"0.6.0"` **22곳** → `"0.7.0"`(합성본이 `ExpectedVersion`과 일치해야 양성/음성 케이스 유효). **drift 가드 단언(`:331`, `ExpectedVersion==VERSION` 동적 비교)은 리터럴 아님 — 미수정·자동통과.**
- **제외범위**: 위 3곳 외 `0.6.0`(docs 릴리스 노트·기준선 이력 = **역사 기록, 유지**). 코드 서명/인증서/서명 도구(STAB-WP-05). 기능·계약·테스트 단언 수 변경. 신규 NuGet. `Directory.Build.props`엔 `<Version>` 리터럴 없음(어셈블리 버전은 build/01 `-p:Version` 주입 → csproj 미편집).
- **읽을문서**: `docs/47`(릴리스 노트/런북/Gate B), `docs/40` ADR-006·ADR-008·ADR-012, `docs/41 §4·§6`, `AGENTS.md §6`, `prompts/codex/REL-v0.7.0_release_cut.md`.
- **수정예상파일**: `VERSION`, `src/RiskManagementAI.Core/Integrity/IntegrityVerifier.cs`, `tests/RiskManagementAI.SmokeTests/PackagingTests.cs`(3개).
- **Public Interface**: 없음(버전 상수만). Core 계약·이벤트 시그니처 불변.
- **구현세부**: 단순 리터럴 범프. `ExpectedVersion`==`VERSION` 단일원천 불변식(STAB-WP-01·ADR-006) 유지. 런타임 Fail-Closed 의미(manifest version 매칭) 유지.
- **보안조건**: 외부 NuGet 0·서명 도구/인증서 0(STOP 준수). 실데이터/원문/토큰/키 0(Gate A). 미서명 출하 — 무결성은 manifest+Fail-Closed 앵커.
- **테스트(Local-Gate)**: `dotnet build` 0/0 → SmokeTest **`Total=714 PASS=714 FAIL=0`**(버전 범프는 단언 가감 없음 → **합계 불변**; PackagingTests 22곳 누락 시 그 도메인 FAIL = 누락 탐지기), `Unclassified=0`. build/00~03(`-Version 0.7.0`) PASS — manifest version `0.7.0`·ZIP SHA256·원문 미포함 스캔.
- **완료조건**: 3파일 범프 + build 0/0 + `Total=714` 불변 + build/03 PASS(미서명 portable ZIP/SHA256/ReleaseNote-v0.7.0/DependencyList-v0.7.0). **외부 NuGet 0 증거 = `dotnet list package` 외부 `<PackageReference>` 0**(DependencyList는 self-contained 동봉 런타임 목록이지 NuGet-0 증거 아님). 태그 `v0.7.0`은 Local(웹 프록시 태그 push 403).
- **Branch**: `feature/rel-v0.7.0-release-cut` · **Commit**: `chore: bump version to 0.7.0 for R2 analytics release (REL-v0.7.0)`
- **Claude Review Checklist**: 변경이 정확히 3파일(VERSION·ExpectedVersion·PackagingTests 22)·그 외 `0.6.0` 역사표기 미변경 / drift 가드(:331) 보존·통과 / 런타임 Fail-Closed 버전매칭 유지 / 기능·단언 수 변경 0 → `Total=714` 불변·Unclassified 0 / 외부 NuGet·서명·인증서 0 추가(STOP) / Gate A 0 / build/03 PASS(manifest 0.7.0·SHA256·원문 미포함) 증거.

## KB-WP-01. 공개 규정 Clause/Chunk Pack 계약 + 로더 + 원문 가드 (인박스, 원문 repo 미포함) — **DONE (#94, VERIFIED — local-gate)** (Cap C-28)
...
- **상태**: **DONE (#94 머지 `b7f56ce`, VERIFIED — local-gate; Claude 4축 리뷰 APPROVE + 리뷰 nit 4건·Codex bot P2 2건 보정)**. 신규 `RegulationClause`·`ClausePackLoadResult`·`ClausePackLoader` + `KbRepositoryGuard` 강화 + 합성 더미 `kb/clause_pack_sample/public_clause_pack_sample.csv`. **ChunkId=`clause-<sha12(BuildLengthPrefixedKey(SourceId,ClauseRef,PackVersion,SourceTextHash))>`**(length-prefixed 컴포넌트 키로 구분자 충돌방지)·`SourceTextHash=LogHash.Sha256Hex(ClauseText)`. **fail-closed**: 동일키+상이텍스트=DUPLICATE 거부, row-level reject/충돌/헤더누락/Pack 미존재는 **partial load 없이 catalog-only fallback**(`UsedFallback=true`·`Clauses=[]`), malformed path=`PATH_REJECTED` safe-fallback(throw 아님), 경로 가드 `IsSafeRelativeClausePackPath`+root containment. `SourceTextAllowed` false 불변·4자 가드 정합·한글 code-unit 미러. `dotnet build` 0/0 → SmokeTest **`Total=747 PASS=747 FAIL=0`**(+33 Kb), Unclassified=0, PackageReference 0, Gate A 0. **잔여**: clause keyword 검색·`ClauseSnippetAllowed` 게이트 = KB-WP-02(NEXT UP). 실 Test PC Gate B/C=BLOCKED.
...
## KB-WP-02. clause keyword 검색 + clause 인용 + ClauseSnippetAllowed 게이트 — **DONE (#101, VERIFIED — local-gate)** (Cap C-28)
> ADR-013. KB-WP-01(#94 `b7f56ce` DONE) 토대 위. 인박스 keyword-only.
- **상태**: **DONE (#101 머지 `e911855`, VERIFIED — local-gate; Claude 4축 리뷰 APPROVE 0 findings)**. `KbKeying`(신규) 단일원천 추출(`RequiresLinearContainsFallback` 포함)·clause fallback=전체 clause 선형스캔·`KbSearch.SearchClauses`+`KbClauseSearchResult`·`ClauseSnippetAllowed`==`Evaluate` 단일 게이트(`PublicStatuses`/`IsMissingGateMetadata` 재사용)·snippet 단일 결정 알고리즘(IndexOf Ordinal·m=0·cap 32·정규화)·asOf inclusivity(`Effective<=asOf<Repeal`·blank Repeal active·out-of-window 제외)·hash-only `KbClauseSearch` audit(ClauseText 평문 미저장)·`SourceTextAllowed` false 불변·합성 non-placeholder positive fixture. `Total 747→768`(+21), Unclassified 0, NuGet 0. 잔여: 실 원문 적재=Prod(APPROVAL).
...
- **설계 결정(Claude Final Gate — prompt-harden, ADR-013 §결정5 확장)**: ① asOf inclusivity = **`EffectiveDate <= asOf AND (RepealDate blank OR asOf < RepealDate)`**(시행일 당일 active·폐기일 당일 inactive·blank Repeal 무기한 active). ② out-of-window clause는 **결과에서 제외**(`(기준일 외)` 표식 방식 미채택) — top-N 슬롯 미소비. ③ non-empty invalid Effective/Repeal date → warning + clause 제외. ④ 공유 추출 대상에 `RequiresLinearContainsFallback`(>32) 포함, clause fallback 동작 = 전체 clause 선형 스캔. ⑤ snippet = 단일 결정적 알고리즘(`IndexOf` OrdinalIgnoreCase 첫매칭 m, 무매칭 m=0, 우측확장, 상한 32, control-char 정규화). ⑥ `ClauseSnippetAllowed`는 `Evaluate`와 동일 private 헬퍼 단일원천. ⑦ 본문 노출 = 경계 Snippet 단일 경로(DisclosureReason/warning/audit에 본문 0). ⑧ 기본 fixture는 전부 placeholder catalog라 `SnippetAllowed==true` 검증용 **합성 비-placeholder fixture 필수**. (상세 = 프롬프트 §1.1~1.6·§3·§4.)
...
## FEEDBACK-WP-01. 승인 Example 본문 Ingest 게이트 + 결정적 검색 + audit (RETRIEVAL, 학습 아님) — **DONE (#106, VERIFIED — local-gate)** (Cap C-20 확장)
> 권위 설계 = **ADR-014**(`docs/40`). RETRIEVAL이지 학습 아님 — 모델 가중치 학습/모델파일 쓰기 0. 인박스, Vector/Embedding STOP.
- **상태**: **DONE (#106 머지 `f8b330a`, VERIFIED — local-gate; Claude 5-lens review APPROVE)**. 본문 non-log `FeedbackDraftBodyInput` 분리(`FeedbackLogEntry` 6-positional·해시 스키마 불변)·`ExamplePromotion.ResolveBody` ingest 게이트(SQL/VBA Blocker 0 AND `ForbiddenTermScanner` 0 → 본문 store, 실패→null+warning)·신규 `ForbiddenTermScanner.ScanText`(단일 토큰 원천, 가드 string 미재사용)·`PromotedExampleRetriever.Search` 결정적 검색(`OrderByDescending(Score).ThenBy(ExampleId,Ordinal)`)·hash-only `PromotedExampleSearch` audit(`TaskLogWriter`)·`.gitignore` promoted_examples·Feedback Center UI 배선. build 0/0, **`Total 792→807 (+15)`**(Audit 도메인, Unclassified 0), Gate A 0, PackageReference 0. bot P2 1건(AuditTests description "metadata"→Kb 오분류) `f51e237` 보정 후 머지.
- **목표**: 승인 Example 본문을 안전 적재(ingest 게이트)·영속하고 결정적으로 검색, 검색 행위 해시 audit. (Prompt 반영은 FEEDBACK-WP-02.) **완료됨**: FEEDBACK-WP-01로 본문 저장·검색 retrieval 경로까지 구현, 현재 남은 R5 범위는 review 경유 Prompt 반영(FEEDBACK-WP-02).
...
## FEEDBACK-WP-02. 검색 결과 Prompt 반영 (review 경유 read-only 주입) — **DONE (#108, VERIFIED — local-gate)** (Cap C-20 확장)
> ADR-014 §결정5. FEEDBACK-WP-01(DONE #106) 토대 위. RETRIEVAL 주입이지 학습 아님.
- **상태**: **DONE (#108 머지 `4e82e8f`, VERIFIED — local-gate; Claude adversarial 4축 review(각 finding 독립 검증) APPROVE·0 confirmed findings)**. 신규 `DraftReferenceExample`(non-log DTO)·`DraftReferenceComposer`(결정적·`MaxReferenceCount=5`·`MaxReferenceTextChars=2000`·truncation marker·`| ` fenced block·`ReferenceBlockHeader`·원 Context 부분문자열 보존·유효 0건 항등)·`DraftPipeline` 가산 필드(`ReferenceExamples`/`ReferencesReviewed` 기본값 → 3/4-인자 호출부 무변경)·`effectiveContext`를 `DraftRequest`와 audit hash(`Sha256(Kind|Prompt|effectiveContext)`) 양쪽에 사용(무참고 경로 RequestHash 불변)·hash-only 2차 audit(`PromotedExampleReflection`, selected ExampleId join hash·평문 0)·`PromotedExampleRetriever` 미호출. build 0/0, **`Total 807→820 (+13)`**(Audit 도메인, Unclassified 0), Gate A 0, PackageReference 0. capture `CapturingDraftService`로 결합 Context 관측.
- **목표**: 검색된 승인 Example을 `DraftRequest.Context`에 **read-only 참고 블록으로 결합**(원 Context 보존), **review/approval 경유**(자동 무검토 주입 금지), 산출=검토용 초안, 주입 행위 해시 audit.
- **선행조건**: FEEDBACK-WP-01(검색).
- **작업범위**: ① `DraftPipeline`/`DraftRequest`에 참고 Example 결합 옵션(원 Context 보존) ② review 게이트(승인된 Example만·사용자 확인) ③ 주입 audit(`TaskLogWriter` 해시) ④ 관측용 capture `ILocalDraftService`(테스트, Context 노출).
- **제외범위**: 모델 추론(R4)·가중치 학습. 자동 무검토 주입. Vector/모델.
- **읽을문서**: ADR-014, `Core/Generation/{DraftPipeline,ILocalDraftService,NoModelDraftService}.cs`, FEEDBACK-WP-01 산출.
- **수정예상파일**: `Core/Generation/DraftPipeline.cs`(Context 결합), `AuditTests.cs`(capture fake).
- **Public Interface**: `DraftRequest`/`DraftPipeline` 참고 결합 옵션(additive). 기존 NoModel 경로 불변.
- **구현세부**: read-only 결합(원 Context 보존). `NoModelDraftService`는 Context 결합 직접 관측 불가 → 테스트는 capture fake로 관측. 결정적.
- **보안조건**: 학습/모델파일 0 · 자동주입 금지(review) · 해시 audit(원문 미저장) · NuGet 0.
- **테스트**(도메인 `Audit`): 참고 블록 결합·원 Context 보존(capture fake 관측 또는 RequestHash 단언)·review 없으면 주입 0·주입 audit 1건. `Total`+N, Unclassified 0.
- **완료조건**: Context 결합+review+audit + build 0/0 + 기존 보존 + Gate A.
...
- **Claude Review Checklist**: read-only 결합·원 Context 보존 / review 게이트(자동주입 0) / 학습/모델파일 0 / capture fake 관측 / 해시 audit / 도메인 Audit·Unclassified 0 / NuGet 0 / `Total` 보존+신규 / Gate A. **프롬프트**: `prompts/codex/FEEDBACK-WP-02_prompt_reflection.md`.
...
## R2-WP-05. R2 잔여 데드코드 하이진 (Welford 미사용 필드 제거, 자기검증형·동작 불변) — **DONE (#109, VERIFIED — local-gate)** (Cap C-14 하이진)
> **상태**: **DONE (#109 머지 `e55f00d`, VERIFIED — local-gate; Claude adversarial review APPROVE·0 confirmed)**. `NumericAccumulator.mean`/`m2`(write-only·read 0 확정) + Welford 갱신 블록 제거(-10 lines, 단일 파일 `DataProfiler.cs`). `OutlierWork`는 독립(`legacyDoubleSum/count`)·OutlierCount·`DuplicateRowCount`·프로파일 수치·streaming 상한·CP949 **바이트 동일**. **`Total` 델타 0**(테스트 무변경·동작 불변; 현 정본 합계는 `829`).
...
## UX-WP-09. 동일 finding 이중 핀 정리 (A-4 — BlockedHint+SafetyHint 중복 표면 축소) — **DONE (#113, VERIFIED — local-gate)** (Cap C-22 하이진)
> UX-WP-07(#110)이 A-4를 범위 밖(의도된 이중목적)으로 뒀던 항목을 사용자 요청으로 **reviewable 변경**으로 승격 → Claude 승인.
> **상태**: **DONE (#113 머지 `0f6e1d7`, VERIFIED — local-gate; Claude adversarial 2-lens review(correctness+governance, findings 독립 검증) APPROVE·0 confirmed)**. `CollapseDuplicateFindingPins` — BlockedHint 보유 finding의 SafetyHint만 제거(2-pass·정렬 순서 보존·`HashSet` membership 전용=결정적), `IsExplicitFinding`으로 `COMPLETION_FINDING_REQUIRED` fallback 그룹핑 제외(무관 fallback 핀 오축소 방지·전용 음성 테스트), **`Findings` 배열은 collapse 이전 산출 → finding 미손실 구조 보장**, 실 default provider 조합 `SQL_DML_DELETE` 핀 1행 회귀(main에서 실패하는 진짜 회귀) 포함. build 0/0, **`Total 829→834 (+5)`**(Assist), Unclassified 0, Gate A 0, PackageReference 0.
- **목표**: 동일 finding에 BlockedHint(언어 provider)+SafetyHint(SafetyHintProvider)가 둘 다 핀되어 2줄 표면되는 것을 1줄(BlockedHint 유지)로 축소. **finding 미손실**(`CompletionResult.Findings` 전부 보존; 이미 `.Distinct()`)·삽입/트리거/accept audit·정적·NoModel 불변.
- **수정예상파일**: `Core/Assist/CompletionEngine.cs`, `AssistTests.cs`.
- **테스트**(도메인 `Assist`): 동일 finding BlockedHint+SafetyHint→핀 1개(BlockedHint)·SafetyHint-only 보존·null Finding 제외·결정적·불변 회귀(pinned 우선·cap 비핀만·findings 절단 안전)·기존 `AssistTests`/UX-WP-07 보존. `Total 829`+N·Unclassified 0.
- **Branch**: `feature/ux-wp-09-same-finding-pin-dedup` · **프롬프트**: `prompts/codex/UX-WP-09_completion_same_finding_pin_dedup.md`.
...
### Wave 1 — DONE
## QA-WP-01. Safety Checker 음성/경계 SmokeTest 하드닝 (SQL·VBA·Excel2021) — **DONE (#115, VERIFIED — local-gate)** (품질)
- **상태**: **DONE (#115 머지 `07bad83`)**. SafetyTests +15(Safety 95→110): SQL deny(대소문자·주석은닉·다중문)·false-positive 가드(`UPDATED_AT`/`CALL_AMOUNT` word-boundary)·VBA 금지 API·Excel `\s*\(` 게이팅·empty graceful. 제품 코드 0·기존 단언 보존. `Total 834→849`. adversarial 4축 0 confirmed.
...
### Wave 2 — DONE (#119~#122)
## QA-WP-03. Kb/Citation SmokeTest 하드닝 — **DONE (#119, VERIFIED — local-gate)** (품질)
- **상태**: **DONE (#119 머지 `b988fa0`)**. KbTests +4(도메인 Kb): `SourceTextAllowed=false` 전 status 잠금·clause-search audit raw locator/clause text 미포함(hash-only)·원문 가드 samples//data_sources/ 확장. 합성 더미(실 원문 0)·제품 코드 0. `Total 861→865`. adversarial 4축 0 confirmed.
- **Branch**: `feature/qa-wp-03-kb-citation-hardening` · **프롬프트**: `prompts/codex/QA-WP-03_kb_citation_hardening.md`.

## QA-WP-04. Report/ExcelReport SmokeTest 하드닝 — **DONE (#120, VERIFIED — local-gate)** (품질)
- **상태**: **DONE (#120 머지 `a7dc659`)**. ReportTests +5(도메인 Report): OOXML 시트 인벤토리·app.xml count/RISK_VISUAL title·정확 Exception Count(B20 static Number, COUNTA 아님)·RISK_VISUAL 무수식(formula-injection 0)·TopN=0 경계. 합성 더미·제품 코드 0. `Total 861→866`. adversarial 4축 0 confirmed.
- **Branch**: `feature/qa-wp-04-report-hardening` · **프롬프트**: `prompts/codex/QA-WP-04_report_hardening.md`.

## QA-WP-05. Csv/Xlsx/DataProfile SmokeTest 하드닝 — **DONE (#121, VERIFIED — local-gate)** (품질)
- **상태**: **DONE (#121 머지 `94cf4cb`)**. CsvTests+2·DataProfileTests+1·XlsxTests+1(=+4): quoted-comma(full+streaming)·CP949 `DecodeLines` byte parity·XlsxReader unsafe rel target fail-closed·DataProfiler worksheet ProfileTable(dup/numeric). in-memory 스트림·제품 코드 0·대용량 바이너리 0(-1=닫는 괄호 이동, 단언 약화 아님). `Total 861→865`. adversarial 4축 0 confirmed.
- **Branch**: `feature/qa-wp-05-csv-xlsx-profile-hardening` · **프롬프트**: `prompts/codex/QA-WP-05_csv_xlsx_profile_hardening.md`.

## UX-WP-11. Excel 2021 Function Helper 카탈로그 확장 — **DONE (#122, VERIFIED — local-gate)** (콘텐츠)
- **상태**: **DONE (#122 머지 `693488c`)**. `excel_function_help.json` +5 함수(AVERAGEIFS·MAXIFS·MINIFS·SUMPRODUCT·IFERROR — 전부 2021 호환·§6 차단셋 0)·SafetyTests +3(Is365Only=false·차단셋 미포함·checker 호환·결정적 검색). helper/checker/UI 구조 불변·resource critical glob 미사용. `Total 861→864`. adversarial 4축 0 confirmed(nit F1 REFUTED — Recommended/Is365Only는 RuleSet 파생이라 JSON로 차단함수 추천 불가).
- **Branch**: `feature/ux-wp-11-excel-function-catalog` · **프롬프트**: `prompts/codex/UX-WP-11_excel_function_catalog_expansion.md`.

### Wave 3 — DONE (#124~#127)
> 인박스 SmokeTest 도메인 중 미하드닝 잔여(Ncr·UiContract·Audit·Generation·Mapping·Packaging)를 회귀로 고정. 전부 순수 additive 테스트(제품 코드 0)·합성 더미·파일 겹침 0. **Wave 3로 인박스 테스트 하드닝 스윕 완결**(누적 `Total 877→900`, 전부 adversarial 리뷰 0 confirmed 후 머지).
## QA-WP-06. Ncr Rule Set 구조 SmokeTest 하드닝 — **DONE (#124, VERIFIED — local-gate)** (품질)
- **상태**: **DONE (#124 머지 `5b458e6`)**. `NcrTests` +4(도메인 Ncr, 49→53): `ComponentMap`이 선언 컴포넌트만 참조·`ValuePolicy=="APPROVAL_REQUIRED_NO_REAL_COEFFICIENT"`(실 계수 0 placeholder)·`ValidationSqlTemplates` SELECT 전용 + `SqlSafetyChecker` <Medium·필수필드 누락 fallback(`NCR_RULESET_REQUIRED_FIELD_MISSING`→`UNAPPROVED-PLACEHOLDER`). 합성 JSON 더미(실 NCR 공식본/계수 0)·제품 코드 0. `Total 877→881`(누적). adversarial 리뷰 0 confirmed.
- **목표(원)**: NCR Rule Set 8요소 **구조**(SCAFFOLD_ONLY)·조회전용 SQL·검토용 초안·placeholder 계수(실 계수 0) 경계 회귀. **제품 코드 0·테스트만.**
- **파일/브랜치**: `tests/RiskManagementAI.SmokeTests/NcrTests.cs`(도메인 `Ncr`) · `feature/qa-wp-06-ncr-hardening` · `prompts/codex/QA-WP-06_ncr_hardening.md`.

## QA-WP-07. UiContract SmokeTest 하드닝 (XAML Contract·레이아웃·스냅샷) — **DONE (#125, VERIFIED — local-gate; 실 렌더=Gate B)** (품질)
- **상태**: **DONE (#125 머지 `c38a333`)**. `UiContractTests` +4(도메인 UiContract, 70→74): `DASHBOARD_AUDIT_LOG_WARNING` 노출·`ResizeMode="CanResize"`+`SizeToContent="Manual"`·`DraftRequestBox` Stretch·Safety 패널 `Width="340"`+Min/Max(-1은 기존 단언 강화)·비유한 `UiLayout`→`UiLayoutStore.Default`·traversal 경로 거부. XAML/제품 코드 0(실 렌더=Gate B). `Total 877→881`(누적). adversarial 리뷰 0 confirmed.
- **목표(원)**: MainWindow XAML Contract·레이아웃 영속(`UiLayoutStore` round-trip/fallback/clamp)·읽기전용 스냅샷·탭 내비게이션의 **미커버 경계** 회귀. **제품 코드 0·테스트만.**
- **파일/브랜치**: `tests/RiskManagementAI.SmokeTests/UiContractTests.cs`(도메인 `UiContract`) · `feature/qa-wp-07-uicontract-hardening` · `prompts/codex/QA-WP-07_uicontract_hardening.md`.

## QA-WP-08. Audit/Generation SmokeTest 하드닝 (해시 전용 로그·NoModel draft) — **DONE (#126, VERIFIED — local-gate)** (품질)
- **상태**: **DONE (#126 머지 `18d97c5`)**. `AuditTests`+`GenerationTests` +8(Audit 69→74·Generation 17→20): TaskLog null OutputHash 허용(raw output 미저장)·`FeedbackLogEntry` 6-필드 스키마(reflection)·`SuggestionLogWriter` accepted 항목 hash 전용·비해시/ SafetyHint kind 거부·NoModel null 요청→unavailable/DraftText null·`DraftPipeline` REVIEW_REQUIRED 경로(High finding→REVIEW_REQUIRED·DraftText null·audit 기록). 합성 더미(실 user/raw prompt/원문 0)·제품 코드 0. `Total 877→885`(누적). adversarial 리뷰 0 confirmed.
- **목표(원)**: `TaskLog`/`FeedbackLog`/`SuggestionLog` 해시 전용(원문/user 미저장·`IsSha256Hex` 강제)·`DraftPipeline`/`NoModelDraftService` 경로·6-positional 로그 스키마의 **미커버 경계** 회귀. **제품 코드 0·테스트만.**
- **파일/브랜치**: `tests/RiskManagementAI.SmokeTests/{AuditTests,GenerationTests}.cs`(도메인 `Audit`/`Generation`) · `feature/qa-wp-08-audit-generation-hardening` · `prompts/codex/QA-WP-08_audit_generation_hardening.md`.

## QA-WP-09. Mapping/Packaging SmokeTest 하드닝 (Column Mapping·Integrity Manifest) — **DONE (#127, VERIFIED — local-gate)** (품질)
- **상태**: **DONE (#127 머지 `7094d91`)**. `MappingTests`+`PackagingTests` +7(Mapping 31→34·Packaging 92→96): ColumnMapping fallback(config 부재/미지 논리컬럼/비문자 물리값)·금지 확장자 lock-step(`.gguf/.safetensors/.onnx/.pt/.pem/.key/.pfx/.env`를 build/01 AND build/03 양쪽에)·backslash traversal fail-closed·`IntegrityResult.ShortHash` audit-safe·`RequiredCriticalEntries`가 `config/ncr/ncr_ruleset_sample.json`+`kb/clause_pack_sample/public_clause_pack_sample.csv` 포함·`.local.json` 제외. 합성 더미(실 컬럼명/모델파일/원문 0)·무결성 로직/build 스크립트 0·제품 코드 0·인벤토리 정합(신규 critical asset 0). `Total 877→884`(누적). adversarial 리뷰 0 confirmed.
- **목표(원)**: Risk Column Mapping·Integrity Manifest/`IntegrityVerifier`(critical glob·required·co-deletion·fail-closed)·build/03 스캔의 **미커버 경계** 회귀. **제품 코드 0·테스트만**(무결성 로직 변경 0).
- **파일/브랜치**: `tests/RiskManagementAI.SmokeTests/{MappingTests,PackagingTests}.cs`(도메인 `Mapping`/`Packaging`) · `feature/qa-wp-09-mapping-packaging-hardening` · `prompts/codex/QA-WP-09_mapping_packaging_hardening.md`.
...
## UX-WP-04. Excel Function Helper (함수 상세설명·예시·Excel 2021 대체식 — 정적·NoModel) — **DONE (#102, VERIFIED — local-gate)** (Cap C-29)
> **상태**: **DONE (#102 머지 `73ca99e`, VERIFIED — local-gate; Claude 4축 리뷰 APPROVE 0 findings)**. `ExcelFunctionHelper` + Core embedded resource(`Excel/Resources/excel_function_help.json`, critical glob 미사용)·`SafetyRuleSet` 단일원천 재사용(함수목록 재선언 0)·WPF Excel 탭 helper view 배선(B-5 UI 갭 종료)·수식 삽입 사용자선택만(자동삽입 0)·검색어 평문 미로그·공개/더미 데이터만·NuGet 0. `Total 768→778`(+10). 실 UI 렌더/Excel 열기=Gate B/C BLOCKED.
> Gate B(2026-06-30) **B-5 PARTIAL** 후속(`docs/48 §B′`). 정적·NoModel·외부 API 0·NuGet 0. 기존 함수 차단 룰을 helper로 확장(중복정의 금지).
- **목표**: Excel 2021 함수 **검색 + 상세설명 + 인수 설명 + 리스크관리 예시 + 수식 예시 + Microsoft 365 전용 여부 + Excel 2021 대체식 + 사용 가능 함수 추천**. 수식 삽입은 **사용자 선택 시에만**.
- **선행조건**: 없음(안정 기준선 `3e82cc0`). 기존 `Core/Excel/Excel2021FunctionChecker.cs`·`Core/Safety/RuleLoader.cs`(차단/365/대체 단일원천) 재사용. 외부 의존성 0.
- **작업범위**: ① 함수 메타 **단일원천 = 기존 `Excel2021FunctionChecker`/RuleSet 재사용**(차단/365전용/대체식 목록 중복정의 금지) + 정적 helper 카탈로그(설명·인수·리스크관리 예시·수식 예시) = **Core embedded resource**(`Core/Excel/Resources/*`, csproj `<EmbeddedResource>`·cp949 map 동형·항상 동봉; **`rules/`/critical glob 옵션파일+누락 fallback 금지** — `rules/*`=`RequiredCriticalEntries`/critical glob라 누락=런타임 fail-closed, fallback은 파싱실패에만), 공개/더미만·실데이터 0 ② 신규 `ExcelFunctionHelper`(Core: name/부분일치 검색·조회 API) + `ExcelFunctionInfo` record ③ 사용가능 함수 추천(차단 함수 → 대체식·대체 함수 매핑) ④ **수식 삽입 = 사용자 명시 선택 시에만**(자동삽입 0) ⑤ **입력 원문(검색어) 로그 미저장**(평문 미기록; 감사 필요 시 해시만) ⑥ **WPF 읽기전용 helper view 배선(필수 — B-5 UI 갭 종료조건; Core-only는 B-5 미종료)**, 기존 패널 재사용·UI 렌더 검증=Gate B.
- **제외범위**: 실시간 LLM 추천/랭킹(R4). Smart Assist 입력중 추천(UX-WP-05/06). 함수 자동실행/계산. Vector/모델/NuGet. 외부 함수 DB/API.
- **읽을문서**: 본 WP, `docs/48 §B′`(B-5), `Core/Excel/Excel2021FunctionChecker.cs`, `Core/Safety/RuleLoader.cs`, `Core/Assist/*`(표면화), `CLAUDE.md §6`(Excel 함수 제한 정본).
- **수정예상파일**: `Core/Excel/ExcelFunctionHelper.cs`·`ExcelFunctionInfo.cs`(신규), 함수정보 **embedded resource**(`Core/Excel/Resources/excel_function_help.*` + csproj `<EmbeddedResource>`; critical glob 미사용), `App/`(helper view 배선), `tests/RiskManagementAI.SmokeTests/`(회귀).
- **Public Interface**: `ExcelFunctionHelper.Search(query)`/`Lookup(name)` → `ExcelFunctionInfo`(Name·Description·Args·RiskMgmtExample·FormulaExample·Is365Only·Excel2021Alternative·Recommended). 기존 `Excel2021FunctionChecker` 시그니처 불변.
- **구현세부**: 결정적·정적. 차단/365/대체 판정은 기존 checker 재사용(단일원천). 데이터=공개 함수 일반지식·더미 예시만, 실 업무데이터 0.
- **보안조건**: 외부 API 0·NuGet 0·자동실행 0·자동삽입 0·**입력 원문(검색어) 로그 미저장**·실데이터/실 수식 0·`CLAUDE.md §6` 금지함수 목록과 정합.
- **테스트**(도메인 적정): 함수 검색 hit·상세 완비(설명·인수·예시·365여부·대체식)·차단함수→대체식 추천·M365 전용 표식·삽입은 명시 선택 시만(자동삽입 0 회귀)·검색어 평문 미기록 회귀·기존 `Excel2021FunctionChecker` 회귀 보존. `Total`+N, Unclassified 0.
- **완료조건**: helper 검색/상세/대체식/추천 + 사용자선택 삽입 + **읽기전용 helper view 배선(B-5 종료)** + build 0/0 + 기존 보존 + Gate A. (UI 렌더 자체 검증 = Gate B Test PC.)
...
- **Claude Review Checklist**: 함수 메타 단일원천(기존 checker/RuleSet 재사용·중복 0) / 상세 완비(설명·인수·리스크예시·수식예시·365여부·대체식·추천) / 수식 삽입 사용자선택만(자동삽입 0) / 입력 원문 로그 미저장(해시 전용) / **helper 데이터=Core embedded(critical glob 미사용·파싱실패만 fallback)** / **읽기전용 helper view 배선(B-5 종료 필수; Core-only=B-5 미종료)** / 외부 API·NuGet 0·실데이터 0 / `CLAUDE.md §6` 정합 / 기존 checker 회귀 / `Total` 보존+신규 / Gate A. **프롬프트**: `prompts/codex/UX-WP-04_excel_function_helper.md`.

## UX-WP-05. Smart Assist 입력중(as-you-type) 추천 표면화 (정적·NoModel, WPF 트리거만) — **DONE (#103, VERIFIED — local-gate)** (Cap C-30)
> **상태**: **DONE (#103 머지 `01715f7`, VERIFIED — local-gate; Claude 4축 리뷰 APPROVE 0 findings)**. Core `CompletionTriggerPolicy`(net8.0 비-WPF, 순수 테스트 가능)·debounced `TextChanged`(인박스 `DispatcherTimer`, NuGet 0)·focus-preserving show(`grabFocus:false`, `CompletionList.Focus()`는 Ctrl+Space 전용)·TextBox `MoveSelection` Up/Down·최소 prefix 게이트·stale close·재진입 가드·Core 엔진/providers 불변·surface audit 0(accept만)·자동삽입 0·NoModel. `Total 778→788`(+10). 실 UI 렌더/focus 동작=Gate B BLOCKED.
> Gate B B-5 후속 UX Enhancement. ⚠️ **기존 완료분 확장(재구현 아님)**: Smart Assist Core(#70)·정적 Provider(#72)·WPF Popup(#73)은 이미 **VERIFIED(local-gate)**. **스코프 확정(2026-06-30 코드 조사)**: 현 Smart Assist는 **Ctrl+Space 명시 전용**(`RegisterCompletionTextBox`가 `PreviewKeyDown`만 연결, `TextChanged` 없음) → B-5 "입력중 추천 없음"은 **설계상 정상(by-design)·회귀 아님**. `CompletionEngine.GetCompletions`는 이미 stateless·prefix-filtered·NoModel이라 **Core 변경 0** — 본 WP = **WPF 트리거 배선만**(as-you-type 신규 추가).
- **목표**: 정적 범위 내 **입력중(debounced as-you-type) 추천**을 실행 Release UI에 표면화. **실시간 LLM 랭킹/학습 = R4(STOP, 미구현)**.
- **선행조건**: 없음(안정 기준선 `e6fba98`). UX-WP-04와 독립. Core 변경 0.
- **작업범위**: ① `RegisterCompletionTextBox`에 **debounced `TextChanged`**(인박스 `DispatcherTimer`, NuGet 0) 추가 → 기존 `ShowCompletionPopup(textBox,language)` 재사용 ② **최소 prefix 게이트 + stale 닫기**(`ExtractCompletionPrefix` 재사용, 길이 ≥ 임계 + 매칭일 때만 트리거; 빈 prefix 전체덤프 방지; **임계 미만/매칭 0 시 열린 팝업 Close**; Ctrl+Space는 현행 유지) ③ **재진입 가드**(suppress 플래그 — `InsertCompletionText`/`AcceptCompletionItem`의 프로그램적 `Text` 세팅이 재트리거 안 하게) ④ **트리거 결정 로직 = `Core`/net8.0(비-WPF) 순수 static 헬퍼로 추출**(App 아님 — SmokeTest=net8.0이 App 참조 불가; should-show bool) ⑤ Ctrl+Space·팝업 네비·accept 삽입·accept 해시 audit 불변 ⑥ surface만으로 audit 0(accept만)·자동삽입 0·NoModel 보존 ⑦ **focus 불탈취 — focus-preserving show path 신규**: 현 `CompletionPopup.Show`는 `CompletionList.Focus()`로 ListBox focus를 잡으므로(입력 끊김 원인), `Show`에 `grabFocus` 파라미터/오버로드 추가 → as-you-type는 focus·캐럿을 TextBox에 유지(focus 이동은 Ctrl+Space 전용). as-you-type 네비는 TextBox `OnCompletionTextBoxPreviewKeyDown`에서 Up/Down=선택 이동(`MoveSelection` 추가)·Enter/Tab/Escape 현행.
- **제외범위**: Core 엔진/provider 변경. 실시간 LLM 랭킹(R4 STOP)·자동삽입·surface audit·팝업 표시 enrich(UX-WP-06)·caret placement(UX-WP-06). Vector/모델/NuGet.
- **읽을문서**: `docs/48 §B′`, `App/MainWindow.xaml.cs`(`RegisterCompletionTextBox`·`ShowCompletionPopup`·`ExtractCompletionPrefix`·`AcceptCompletionItem`·`InsertCompletionText`), `Core/Assist/{CompletionEngine,CompletionContracts}.cs`, `AssistTests.cs`.
- **수정예상파일**: `App/MainWindow.xaml.cs`(TextChanged 배선·디바운스·suppress·Up/Down nav·stale Close), `App/Controls/CompletionPopup.xaml.cs`(focus-preserving `Show` grabFocus·`MoveSelection`), **`Core/Assist/` 트리거 결정 순수 헬퍼(신규, net8.0 비-WPF)**, `tests/RiskManagementAI.SmokeTests/AssistTests.cs`(회귀).
- **Public Interface**: **`Core`(net8.0) 트리거 결정 순수 static 메서드**(prefix 길이/suppress/언어/매칭 → should-show bool). 기존 `CompletionEngine`/`CompletionPopup`/`ShowCompletionPopup` 계약 불변.
- **구현세부**: 디바운스 150~250ms·결정적. prefix 임계(예 ≥2)·suppress 플래그·언어별 동일. WPF 렌더·타이밍 = Gate B.
- **보안조건**: NoModel·외부 API·NuGet·모델 0·자동삽입 0·자동학습 0·입력 원문 미저장(accept 해시 audit 재사용)·입력 지연/멈춤 0.
- **테스트**(도메인 `UiContract` 또는 적정): Core 결정 헬퍼 — prefix<임계 should-show=false·≥임계+매칭 true·매칭 0 false(팝업 Close)·suppress 중 false·미지원/빈 텍스트 false·Ctrl+Space 분리. 기존 `AssistTests` 보존. `Total`+N, Unclassified 0. (WPF 렌더·focus 불탈취·stale Close 타이밍 = Gate B.)
- **완료조건**: 트리거 배선 + 결정 헬퍼 + build 0/0 + 기존 보존 + Gate A. **B-5 종료 = 입력중 추천이 실제 표면화될 때**(렌더 검증 Gate B).
...
- **Claude Review Checklist**: Core 엔진 변경 0(트리거만)·`ShowCompletionPopup` 재사용 / 디바운스=인박스 `DispatcherTimer`(NuGet 0) / 최소 prefix 게이트·**임계 미만/매칭 0 시 stale 팝업 Close**·재진입 가드(수락 후 재트리거 0) / **focus 불탈취 = as-you-type show가 `CompletionList.Focus()` 미호출**(focus 이동 Ctrl+Space 전용·Up/Down은 TextBox에서 `MoveSelection`) / Ctrl+Space·accept·audit 불변 / 자동삽입 0·surface audit 0·입력 원문 미저장 / NoModel·실시간 LLM=R4 STOP / **결정 헬퍼는 `Core`/net8.0(비-WPF, App 아님)**→ SmokeTest 단언 / `AssistTests` 보존 / `Total` 보존+신규 / Gate A. **프롬프트**: `prompts/codex/UX-WP-05_smart_assist_as_you_type.md`.

## UX-WP-06. Smart Assist Completion Popup 표시 확장 (Snippet·SafetyNote·Kind, 자동삽입 0) — **DONE (#104, VERIFIED — local-gate)** (Cap C-30)
> **상태**: **DONE (#104 머지 `e69a1ae`, VERIFIED — local-gate; Claude 4축 리뷰 APPROVE 0 findings + Codex bot P2 1건 보정: finding code/message를 SafetyNote보다 먼저 표시 + 회귀 1)**. Core `CompletionDisplayFormatter`/`CompletionDisplayInfo`·팝업 `KindLabel`/snippet preview/`SafetyText`/insertability 표시·`SelectedCompletion` 원 `CompletionItem` 유지·`TryAcceptSelected`/`ItemAccepted`/`Insertable` 게이트/accept audit 불변·provider/matching/Core 변경 0·자동삽입 0·NoModel. `Total 788→792`(+4). 실 UI 렌더=Gate B BLOCKED.
> Gate B 후속 UX Enhancement. ⚠️ **기존 WPF Completion Popup(#73) 표시 확장(재구현 아님)**. **선행 = UX-WP-05**(트리거). **스코프 확정**: `CompletionPopup.xaml:24-47`은 Label·Source·Kind·RequiresReview+"검토용 초안" badge만 렌더 — **Snippet 미리보기·`SafetyNote`(CompletionItem에 있으나 UI 미사용)·Kind 표식 없음**.
- **목표**: 팝업 **표시 확장**(Snippet 구분·`SafetyNote`/findings 표면화·Kind 라벨/아이콘). **삽입/트리거 동작 불변**(자동삽입 0, 선택 시에만; non-insertable 삽입 0).
- **선행조건**: UX-WP-05 머지(트리거).
- **작업범위**: ① `CompletionItemKind.Snippet` 구분 표시 + `InsertText` 읽기전용 미리보기 ② `CompletionItem.SafetyNote`/`Finding` 렌더(SafetyHint/BlockedHint=findings-only) ③ Kind 라벨/아이콘·기존 정렬 표시 불변 ④ `Show/Close/TryAcceptSelected/IsCompletionOpen/ItemAccepted`·`AcceptCompletionItem` 삽입·accept audit 불변·**`Insertable` 게이트 보존** ⑤ (선택) caret-relative placement.
- **제외범위**: as-you-type 트리거(UX-WP-05). 신규 provider/추천 알고리즘·실시간 LLM(R4). 자동삽입. 매칭 로직(substring/fuzzy=Core 변경). Vector/모델/NuGet.
- **읽을문서**: `Controls/CompletionPopup.xaml(.cs)`, `Core/Assist/CompletionContracts.cs`(`CompletionItem`), `App/MainWindow.xaml.cs`(`AcceptCompletionItem`), `AssistTests.cs`.
- **수정예상파일**: `App/Controls/CompletionPopup.xaml(.cs)`(표시), (필요 시) 표시 헬퍼/VM(신규), `AssistTests.cs`.
- **보안조건**: NoModel·외부 API·NuGet·모델 0·자동삽입 0·non-insertable 삽입 0·표시 텍스트=정적 seed만(실데이터 0)·입력 원문 미저장.
- **테스트**: 표시 로직(Kind→라벨·SafetyNote 노출·Insertable 게이트) 순수 헬퍼/VM 단언·non-insertable 삽입 0 회귀·`CompletionItem` 계약 불변. 기존 `AssistTests` 보존. `Total`+N. (렌더=Gate B.)
- **완료조건**: 표시 확장 + build 0/0 + 삽입/트리거 불변 + 기존 보존 + Gate A.
...
- **Claude Review Checklist**: 표시만·트리거/삽입 불변 / Snippet·SafetyNote·Kind 표면화 / 자동삽입 0·non-insertable 삽입 0(`Insertable` 게이트) / 팝업 계약·accept audit 불변 / Core 변경 0 / NoModel·NuGet 0·실데이터 0 / 표시 로직 순수 헬퍼 SmokeTest·`AssistTests` 보존 / `Total` 보존+신규 / Gate A. **프롬프트**: `prompts/codex/UX-WP-06_smart_assist_popup_display.md`.
...
- **상태**: **APPROVAL_REQUIRED — 인증서 경로 = A(사내 Enterprise CA) 확정(2026-06-30, 사용자 승인); 구현 WP는 본 KB→R5 시퀀스 뒤**. STAB-WP-03b interim이 닫지 못한 **manifest 독립 신뢰 앵커**(쓰기 가능 폴더에서 파일+manifest를 lock-step 동시 변조하는 co-tamper) + **self-contained 런타임 DLL(~150개) 미해시** + 폴더 동반 변조를 닫는다. **인증서 경로 결정 완료(A)** → STOP의 핵심 결정은 해소. 다만 나머지 §6.2 항목(인증서 보관·반입 절차·런타임 검증 정책·오프라인 폐기 처리·Rollback)과 **서명 도구**(인박스 `Set-AuthenticodeSignature` 1순위, signtool=Windows SDK면 별도 승인)는 **구현 WP에서 확정**하고, 실 인증서·서명·검증은 **Windows 실 Test PC/운영 환경 의존** → 실 증거 전 VERIFIED 금지(§11.4).
...
- **선행조건**: STAB-WP-03b 머지(완료). **승인 문서 = `docs/40` ADR-012 + `docs/41 §6`(작성 완료)**; 게이트 해제 = 사용자 **인증서 경로 결정**(발급 주체·반입 절차·검증 정책·Rollback 포함). 인증서 결정 전 = 구현 NEXT UP 부적격.
- **제외범위(STOP 전)**: 자동 서명 파이프라인, 인증서 저장·반입 자동화.
- **테스트(승인 후, Windows 실 Test PC) — 잔여 3건 폐쇄 매핑(`docs/41 §6.3`/ADR-012 §검증)**: 정상 서명=기동, 미서명/불일치=차단; **①콘텐츠 co-tamper=서명검증 실패로 차단**, **②런타임 DLL 변조=서명 카탈로그/게시자 검증으로 차단**(미적용 범위는 OPEN 표기), **③폴더 동반 변조=서명앵커+manifest 인벤토리 교차 차단** — 셋 다 03b에서 "미탐지=양성"으로 고정된 케이스를 회귀로 **PASS 전환**. 회귀 PASS 전까지 VERIFIED 금지·Gate B/C BLOCKED 유지(과대표기 금지).
...
## R2-WP-04. Visualization / Report 강화 (인박스 차트·Heatmap·TopN·집중도 + 정확 Exception Count) — 상태: DONE (#87 merged `35e6f01`, VERIFIED — local-gate; Claude 4축 리뷰 APPROVE)

> 상태: **VERIFIED**(local-gate, #87 `35e6f01`; Claude 4축 리뷰 APPROVE + Codex bot P2 4건 보정). `dotnet build RiskManagementAI.sln -c Release` 0/0, `dotnet run --project tests\RiskManagementAI.SmokeTests -c Release` → `Total=714 PASS=714 FAIL=0`, Unclassified=0, PackageReference 0, Gate A 0. `RiskVisualAggregator`+`RISK_VISUAL` 시트(정확 Exception Count·7상태·TopN Abs·HHI·Heatmap·MIXED_CURRENCY/0분모 graceful)·WPF Shapes 화면(동일 SoT)·시각화 caveat `ExcelReportResult.Findings` 표면화. 외부 charting NuGet 0·결정적·`BuildReport`/7상태/RECON 계약 불변. **→ R2 분석 트랙(v0.7.0) 완결.** 실 Test PC Gate B/C(Excel 2021 수동열기·WPF 렌더 포함)=**BLOCKED**.
...
> **Codex local-gate 결과(2026-06-30, #87 merged `35e6f01`, Claude review APPROVE)**: `dotnet build RiskManagementAI.sln -c Release` → 0 warnings / 0 errors. `dotnet run --project tests\RiskManagementAI.SmokeTests -c Release` → **`Total=714 PASS=714 FAIL=0`**, Unclassified=0. 신규 단언 +16(`ReportTests`): 정확 ExceptionCount(헤더/NO_EXCEPTION 제외), 0건 카운트, 신규 `RISK_VISUAL` sheet 배선, 7상태 분포(DUPLICATE_LIMIT 포함), TopN Abs Exposure 정렬+Ordinal tie-break+CurrencyCode 라벨, 집중도 `Abs(ExposureAmount)` 분모+HHI, Heatmap 경계(0.8/1.0), `MIXED_CURRENCY`, visual finding 반환, 0분모 graceful. Codex bot P2 4건 보정: 캔버스 높이 여유, 기본 창 폭 clipping 완화, TopN CurrencyCode 라벨 정렬, visual findings를 `ExcelReportResult.Findings`에 포함. Core/App/SmokeTest csproj `PackageReference` 0. Gate A: `git add -A --dry-run` 추적 대상 의도 파일, 변경분 secret/주민번호 0, 금지 확장자 0. Excel 2021 수동 열기·실 Test PC Gate B/C는 미수행(**BLOCKED**).
