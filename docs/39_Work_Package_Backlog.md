...
- **수정예상파일**: `App/MainWindow.xaml.cs`(TextChanged 배선·디바운스·suppress·focus 불탈취·stale Close), **`Core/Assist/` 트리거 결정 순수 헬퍼(신규, net8.0 비-WPF)**, `tests/RiskManagementAI.SmokeTests/AssistTests.cs`(회귀).
...
- **Claude Review Checklist**: Core 엔진 변경 0(트리거만)·`ShowCompletionPopup` 재사용 / 디바운스=인박스 `DispatcherTimer`(NuGet 0) / 최소 prefix 게이트·**임계 미만/매칭 0 시 stale 팝업 Close**·재진입 가드(수락 후 재트리거 0) / **팝업 TextBox focus·캐럿 미탈취**(키 입력 끊김 0) / Ctrl+Space·네비·accept·audit 불변 / 자동삽입 0·surface audit 0·입력 원문 미저장 / NoModel·실시간 LLM=R4 STOP / **결정 헬퍼는 `Core`/net8.0(비-WPF, App 아님)**→ SmokeTest 단언 / `AssistTests` 보존 / `Total` 보존+신규 / Gate A. **프롬프트**: `prompts/codex/UX-WP-05_smart_assist_as_you_type.md`.
