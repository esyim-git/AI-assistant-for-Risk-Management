...
- **현재 기준선**: main `d8cb415` (확장 트랙 Wave 1 #115~#117 머지 후), VERSION **0.7.0** (**v0.7.0 정식 릴리스 태그 = `30c1cfb`**, ZIP SHA256 `42C835…`·미서명; 직전 v0.6.0 태그 `3dfa80b`). R1(WP-01~08)·R3(R3-WP-01~05)·REL-v0.6 가드(#54)·**STAB-WP-01~04·STAB-UX-01/02·UX-WP-01~10·R2-WP-01~05·REL-v0.7.0·KB-WP-01/02·FEEDBACK-WP-01/02·QA-WP-01/02**(#56~#117 계열) 모두 **DONE**. 추가 머지: truth-sync/계획/Skills(#63~#114 계열 다수). SmokeTest **`Total=861 PASS=861 FAIL=0`**(local-gate; … → 807(FEEDBACK-WP-01 +15) → 829(FEEDBACK-WP-02 #108 +13·UX-WP-07 #110 +6·UX-WP-08 #111 +3·R2-WP-05 #109 +0) → 834(UX-WP-09 #113 +5) → 861(QA-WP-01 #115 +15·QA-WP-02 #116 +6·UX-WP-10 #117 +6)). **FEEDBACK-WP-02(검색 Example → `DraftRequest.Context` review 경유 read-only 반영) 완료**: `DraftReferenceComposer` 결정적(개수/문자 상한·fencing)·원 Context 보존·`ReferencesReviewed` 게이트(자동주입 0)·`effectiveContext`를 `DraftRequest`·audit hash 양쪽에 사용(무참고 경로 RequestHash 불변)·hash-only reflection audit(`PromotedExampleReflection`)·`PromotedExampleRetriever` 미호출 — **RETRIEVAL·학습 아님**. **cleanup 완료**: UX-WP-07(#110 Smart Assist 표면화 하이진)·R2-WP-05(#109 dead Welford 필드 제거·동작 불변)·UX-WP-08(#111 팝업 Esc/Close 포커스 복원). **전부 Claude adversarial 4축 review(각 findings 독립 검증) APPROVE·0 confirmed findings 후 머지.** VERIFIED 범위는 local-gate 한정이며 실 오프라인 Test PC Gate B/C=**BLOCKED**(과대표기 금지; 2026-06-30 부분검증 `docs/48 §B′`, UX-WP-08 실 포커스 렌더=Gate B). STAB-WP-05(코드서명)=APPROVAL_REQUIRED(**인증서 경로 A 확정**). **UX-WP-09 완료**(#113: A-4 이중 핀 축소·finding 미손실). **확장 트랙 Wave 1 완료**(#115~#117: QA-WP-01 Safety 음성/경계 +15·QA-WP-02 Recon/Limit 경계 +6·UX-WP-10 SQL/VBA/RiskPhrase seed 확장(전 seed checker 안전 가드) +6 — 전부 adversarial 4축 review·**0 confirmed findings** 후 머지). **NEXT UP = 확장 트랙 Wave 2**(**QA-WP-03**(Kb/citation)·**QA-WP-04**(Report/ExcelReport)·**QA-WP-05**(Csv/Xlsx/DataProfile)·**UX-WP-11**(Excel 함수 카탈로그) — 4 WP 병렬·독립 브랜치·파일 겹침 0, 프롬프트 READY; 본 문서 확장 트랙 참조). 그 외 후보(신규 기능 트랙 설계·STAB-WP-05 승인·**Gate B/C 실 Test PC=user-driven**, `docs/48 §B″` 실행 런북)는 후속. 인박스·Vector/Embedding STOP.
...
- **NEXT UP (다음 WP) = 확장 트랙 Wave 2 (QA-WP-03·QA-WP-04·QA-WP-05·UX-WP-11 — 4 WP 병렬)**: Wave 1(#115~#117) 완료. Wave 2도 파일 겹침 0(`KbTests` / `ReportTests` / `CsvTests`·`XlsxTests`·`DataProfileTests` / Excel embedded resource+`AssistTests`)이라 **각각 독립 브랜치로 병렬 진행**하고 개별 PR로 열어둔다(Claude 배치 리뷰). 프롬프트 전부 READY(본 문서 확장 트랙 참조). 그 외 후보는 후속: **신규 기능 트랙 설계**(Claude 프롬프트 선행) · **STAB-WP-05 코드서명**(APPROVAL_REQUIRED, 인증서 경로 A 확정) · **Gate B/C 실 Test PC 증거**(user-driven, BLOCKED — `docs/48 §B″` 실행 런북 준비됨).
...
## ★ QA-WP-03. Kb/Citation SmokeTest 하드닝 (공개 규정 검색·인용·원문 가드) — **READY** (품질)
- **목표**: KB Metadata/Keyword·Inverted Index 검색·인용(문서명·버전·시행일·조항·출처·검색기준일)·`ClauseSnippetAllowed`/`SourceTextAllowed` 게이트·`KbRepositoryGuard` 원문 차단의 **미커버 경계**를 회귀로 고정. **제품 코드 0·테스트만 additive**(합성 더미 catalog/pack, 실 원문 0). ⚠️ 도메인 `Kb`(설명에 `검색`/`원문`/`공개`/`인용`/`Regulation`/`citation` 토큰 사용 — 이건 Kb 분류에 필요).
- **수정예상파일**: `tests/RiskManagementAI.SmokeTests/KbTests.cs`. **도메인 `Kb`**.
- **Branch**: `feature/qa-wp-03-kb-citation-hardening` · **프롬프트**: `prompts/codex/QA-WP-03_kb_citation_hardening.md`.

## ★ QA-WP-04. Report/ExcelReport SmokeTest 하드닝 (RISK_VISUAL·정확 Exception Count·Dashboard=Report) — **READY** (품질)
- **목표**: 인박스 OOXML Report·`RISK_VISUAL`(7상태 분포·TopN·HHI·Heatmap·MIXED_CURRENCY)·정확 Exception Count(Number SoT)·Dashboard=Report 일원화의 **미커버 경계**(0분모 graceful·시트 배선·수치 일치)를 회귀로 고정. **제품 코드 0·테스트만 additive**(합성 더미). 외부 charting NuGet 0.
- **수정예상파일**: `tests/RiskManagementAI.SmokeTests/ReportTests.cs`. **도메인 `Report`**.
- **Branch**: `feature/qa-wp-04-report-hardening` · **프롬프트**: `prompts/codex/QA-WP-04_report_hardening.md`.

## ★ QA-WP-05. Csv/Xlsx/DataProfile SmokeTest 하드닝 (CP949·streaming 상한·프로파일 경계) — **READY** (품질)
- **목표**: CP949(UHC)·UTF-8·XLSX 입력·streaming 행/바이트 상한(`InvalidDataException`)·Welford/Outlier parity·`DuplicateRowCount`·DataProfile 경계의 **미커버 케이스**를 회귀로 고정. **제품 코드 0·테스트만 additive**(합성 더미·인박스 매핑표 재사용, 실데이터 0). 파일 분리로 3 test 파일 동시 편집이나 QA-WP-03/04와 겹침 0.
- **수정예상파일**: `tests/RiskManagementAI.SmokeTests/{CsvTests,XlsxTests,DataProfileTests}.cs`. **도메인 `Csv`/`Xlsx`/`DataProfile`**.
