...
- **현재 기준선**: main `dd286fa`, VERSION **0.6.0** (**v0.6.0 정식 릴리스 태그 = `3dfa80b`**). R1(WP-01~08)·R3(R3-WP-01~05)·REL-v0.6 가드(#54)·**STAB-WP-01(#56)·STAB-WP-02(#57)·STAB-WP-03a(#59)·STAB-WP-03b(#61)·STAB-WP-04(#66)·STAB-UX-01(#68)** 모두 **DONE**. 추가 머지: truth-sync(#63·#67)·Smart Assist 설계(#62, UX 트랙)·STAB-WP-04+UX-WP-01 kickoff(#65). SmokeTest **`Total=579 PASS=579 FAIL=0`**(local-gate; STAB-WP-04 정본 572 + STAB-UX-01 XAML Contract +7, 전부 `UiContract` 도메인 분류).
...
- **NEXT UP (다음 WP)**: **`UX-WP-01`(Smart Assist Completion Core — 정적·NoModel)** — `CompletionEngine`·`CompletionContext`·`CompletionItem`·`ICompletionProvider`·Registry 계약 + NoModelMode + accept 해시 audit(`Core/Assist`, **WPF/모델 0**). **Codex 프롬프트 READY**: `prompts/codex/UX-WP-01_completion_core.md`(설계 #62 머지, 7+4 P2 반영 완료) → **Codex 핸드오프**(구현은 로컬 SDK). 병행: PILOT Gate B/C(BLOCKED, 실 Test PC).
- **그 다음 후보(순서, NEXT UP 아님)**: UX-WP-01 → UX-WP-02(정적 Provider)·UX-WP-03(WPF Popup) → STAB-UX-02(레이아웃 영속화) → **STAB-WP-05(Authenticode 코드 서명 — APPROVAL_REQUIRED, 외부 신뢰 루트, STOP)** → R2-WP-01(Risk Semantic Hardening) → R2-WP-02~04.
