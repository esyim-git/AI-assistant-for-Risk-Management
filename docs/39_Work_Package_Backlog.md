...
- **현재 기준선**: main `35e6f01` (PR #87 머지 후), VERSION **0.6.0** (**v0.6.0 정식 릴리스 태그 = `3dfa80b`**; **R2 분석 트랙(v0.7.0) 완결**, 정식 v0.7.0 태깅 미수행). R1(WP-01~08)·R3(R3-WP-01~05)·REL-v0.6 가드(#54)·**STAB-WP-01~04(#56/#57/#59/#61/#66)·STAB-UX-01(#68)·UX-WP-01(#70)·UX-WP-02(#72)·UX-WP-03(#73)·STAB-UX-02(#76)·R2-WP-01(#79)·R2-WP-02(#81)·R2-WP-03(#84)·R2-WP-04(#87)** 모두 **DONE**. 추가 머지: truth-sync(#63·#67·#69·#74·#78·#80·#82·#83·#85·#86). SmokeTest **`Total=714 PASS=714 FAIL=0`**(직접 local-gate 재확인: `dotnet build` 0/0 + SmokeTest, 2026-06-30; 631 + STAB-UX-02 +15 + R2-WP-01 +25 + R2-WP-02 +9 + R2-WP-03 +18 + R2-WP-04 +16). **R2-WP-04(Visualization/Report) 완료**: 정확 Exception Count(Number SoT)·`RISK_VISUAL` 인박스 시트·WPF Shapes/Canvas 화면·시각화 caveat `ExcelReportResult.Findings` 표면화, Codex bot P2 4건 보정 후 Claude APPROVE 머지. **UX/STAB + R2 트랙 완료** — VERIFIED 범위는 local-gate 한정이며 실 오프라인 Test PC Gate B/C=**BLOCKED**(과대표기 금지). STAB-WP-05(코드서명)=APPROVAL_REQUIRED. **NEXT UP = REL-v0.7.0**(v0.7.0 정식 릴리스 컷 — 버전 범프 락스텝; 코드서명=STAB-WP-05 APPROVAL_REQUIRED·인증서 결정 대기) — 아래 Resume Brief 및 `## ★ REL-v0.7.0` 참조.
...
- **NEXT UP (다음 WP) = REL-v0.7.0 (R2 분석 트랙 v0.7.0 정식 릴리스 컷)**: 사용자 트랙 결정 = **"v0.7.0 릴리스 준비 / STAB-WP-05 코드서명"**. **REL-v0.7.0**(구현 가능)을 NEXT UP으로 집고, **STAB-WP-05 코드서명**은 **승인 문서 READY**(`docs/40` ADR-012 + `docs/41 §6`) — **사용자 인증서 경로 결정 대기**(APPROVAL_REQUIRED, 구현 NEXT UP 아님). REL-v0.7.0 = **버전 범프만**(`VERSION 0.6.0→0.7.0` + 락스텝 `IntegrityVerifier.ExpectedVersion`·`PackagingTests` 22리터럴), **기능 코드 0·`Total=714` 불변**(drift 가드 `PackagingTests.cs:331` 보존), build/00~03 Local-Gate + 미서명 portable ZIP/SHA256/ReleaseNote-v0.7.0. 프롬프트 `prompts/codex/REL-v0.7.0_release_cut.md` READY, 릴리스 노트/런북 `docs/47`. **선행**: R2-WP-04 truth-sync(#88) 머지 후 main 기준.
- **그 다음 후보(순서, REL-v0.7.0 후)**: ① **STAB-WP-05 Authenticode 코드서명**(인증서 경로 승인 시 — `docs/41 §6` 게이트 해제 후 구현) · ② **KB 원문 Clause/Chunk 검색**(v0.8 — 인박스 keyword-only 가능, **Vector/Embedding/모델은 STOP·승인 선행** `docs/40·41`) · ③ **R5 Feedback Learning 확장**(`Core/Feedback` 영속/검색/재사용/Audit) · ④ **실 Test PC Gate B/C 증거**(BLOCKED 해소, `docs/45` 양식 재사용; R2-WP-04 Excel 수동열기 포함). **STAB-WP-05**=인증서 결정 전 **APPROVAL_REQUIRED/STOP**이라 구현 NEXT UP 부적격. 병행: PILOT Gate B/C(BLOCKED, 실 Test PC).
...
## ★ REL-v0.7.0. R2 분석 트랙 v0.7.0 정식 릴리스 컷 (버전 범프 락스텝) — **NEXT UP (구현 가능)**
- **상태**: **READY (NEXT UP)**. 사용자 트랙 결정 = "v0.7.0 릴리스 준비 / STAB-WP-05 코드서명" 중 **구현 가능한 릴리스 컷**. (STAB-WP-05 코드서명은 `docs/41 §6` 인증서 결정 대기 = 별도·APPROVAL_REQUIRED.)
...
- **상태**: **APPROVAL_REQUIRED — 승인 문서 READY, 사용자 인증서 경로 결정 대기**. STAB-WP-03b interim이 닫지 못한 **manifest 독립 신뢰 앵커**(쓰기 가능 폴더에서 파일+manifest를 lock-step 동시 변조하는 co-tamper) + **self-contained 런타임 DLL(~150개) 미해시** + 폴더 동반 변조를 닫는다. 외부 신뢰 루트(인증서·서명 도구)가 필요하므로 **STOP 규칙(§11.5)** — 승인 게이트(`docs/41 §6` / ADR-012 = ADR-008 §결정5 보강) 통과 전 진행 금지. **승인 문서 작성 완료**(ADR-012 승인 요건 + `docs/41 §6` 인증서 경로 4안 A~D·결정 체크리스트); **남은 것 = 사용자/문서오너의 인증서 경로(A 사내CA/B OV/C EV/D self-signed) 결정** 1건.
