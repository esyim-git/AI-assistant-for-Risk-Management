...
- **현재 기준선**: main `59a752f` (R2-WP-01 머지 후), VERSION **0.6.0** (**v0.6.0 정식 릴리스 태그 = `3dfa80b`**; R2 트랙 v0.7.0 진행 중·미릴리스). R1(WP-01~08)·R3(R3-WP-01~05)·REL-v0.6 가드(#54)·**STAB-WP-01~04(#56/#57/#59/#61/#66)·STAB-UX-01(#68)·UX-WP-01(#70)·UX-WP-02(#72)·UX-WP-03(#73)·STAB-UX-02(#76)·R2-WP-01(#79)** 모두 **DONE**. 추가 머지: truth-sync(#63·#67·#69·#74·#78). SmokeTest **`Total=671 PASS=671 FAIL=0`**(local-gate; UX 트랙 정본 631 + STAB-UX-02 레이아웃 영속화 +15 + R2-WP-01 Risk Semantic Hardening +25). **UX/STAB 트랙 완료 + R2-WP-01 완료** — VERIFIED 범위는 **정적·NoModel·외부 Editor 0·자동삽입 0** 한정이며 실 LLM 랭킹/학습=**R4(NOT_IMPLEMENTED)**, 실 오프라인 Test PC Gate B/C=**BLOCKED**(과대표기 금지). STAB-WP-05(코드서명)=APPROVAL_REQUIRED.
...
- **NEXT UP (다음 WP)**: **`R2-WP-02`(Streaming/Perf — 대용량 streaming·행/바이트 상한 `InvalidDataException`·Welford 온라인 분산·중복 해시·결정적 프로파일)**. **인박스·NuGet 0·기존 결정성/`DuplicateRowCount` 불변**. **WP 상세 = `docs/39 §R2-WP-02`, Codex 프롬프트 = `prompts/codex/R2-WP-02_streaming_performance.md`**(P2 반영본, main 적재). Data/ 한정이라 R2-WP-03/04와 파일 충돌 적음. 병행: PILOT Gate B/C(BLOCKED, 실 Test PC).
- **그 다음 후보(순서, NEXT UP 아님)**: R2-WP-02 → R2-WP-03 → R2-WP-04 (cross-WP: R2-WP-01이 `LimitAnalysisMetadata` ctor를 이미 확장했고 #79로 머지됨 → R2-WP-03 BASE_DT 선행 충족). **STAB-WP-05**(Authenticode 코드 서명)=**APPROVAL_REQUIRED/STOP**이라 구현 NEXT UP 부적격.
