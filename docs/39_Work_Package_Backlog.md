...
- **Audit input `origin/main` / code-test baseline**: `abab29b3cb5f8436c846d5f478cd80cd1dbaea22` (PR #133). REL-WP-071 changed VERSION/IntegrityVerifier/PackagingTests, so the baseline advanced from `7094d91`. This audit's docs/workflow-only merge will advance current main but not the product-code/test baseline.
- **VERSION**: `0.7.1`. Latest published release remains `v0.7.0` (`30c1cfb`, unsigned). Remote `v0.7.1` tag/Release does not exist.
- **Local gate**: `dotnet build -c Release` warning 0/error 0; SmokeTest `Total=900 PASS=900 FAIL=0`, Unclassified 0.
- **Merged-main v0.7.1 candidate (`abab29b`)**: build/00~03 PASS, manifest version 0.7.1/27 entries, ZIP SHA256 `A70D0B37AD92344A2ECFBE0D4D96360F56CBAFFF94363249F0BD1A20ADC1ECDC`. This is a candidate only; any later merge requires rebuild.
...
- **Confirmed release blocker**: no-limit report path can produce High `LIMIT_DATA_REQUIRED` while SUMMARY says `ReconciliationPassed=PASS` because zero checks are vacuously true. See `docs/53 §3`.
- **NEXT UP = CORR-WP-01 only**: zero reconciliation checks render `NOT_RUN`, never `PASS`.
- **After NEXT UP**: rebuild/tag/publish v0.7.1 -> GOV-WP-02(restored PR CI evidence + protection/security settings) -> ARCH-WP-01 -> UI-WP-12 -> remaining Core-to-WPF reachability -> .NET 10 -> formal Gate B/C -> Team Pilot.
- **Approval tracks remain STOP**: STAB-WP-05 signing, real NCR/internal Pack, Local LLM runtime/model.
- **Current assessment**: `docs/53_Repository_Audit_and_v1_Execution_Plan.md`; current prompt: `prompts/codex/CORR-WP-01_reconciliation_not_run.md`.
...
- **NEXT UP (다음 WP) = REL-WP-071**(v0.7.1 출하 정합 릴리스 컷 — 본 원장 항목·`prompts/codex/REL-WP-071_release_cut_v0_7_1.md` READY·릴리스 문서 `docs/52`): 방향 결정 대기(decision point)는 **제안서 채택(PR #131 `110e9ee`)으로 해소** — 채택 로드맵 = `docs/proposals/FABLE5_REPO_ASSESSMENT_PROPOSAL_20260706.md`(§8 로드맵·§10 WP-A~K·§14 실행순서). **병행(user-driven)**: Gate B/C Round 1(`docs/48 §B″` 런북 — published v0.7.0 ZIP로 B-6·B-8 + user-reported 봉인, 증거 민감정보 0). 승인 게이트 3건(STAB-WP-05 서명·NCR 실 Pack·R4 LLM)은 `docs/51` 결정 대기(부분 승인 가능). v0.8 후속 후보(REL-WP-071 뒤) = ARCH-WP-01(MainWindow 행위 불변 분해)·UI-WP-12(Prior-Day UI 배선) — 제안서 §9.3 순서. 인박스·Vector/Embedding STOP.
...
- **운영 모델(Local-Gate)**: GitHub Actions 2,000분/월 소진 대응 — **리셋 예정일(~2026-06-30) 경과**했으나 자동 트리거 복원 여부는 사용자 결정 대기(제안서 §6.3-4; 복원 전까지 local-gate 정본 유지). build/test/packaging는 **전부 로컬 실행**. 머지 게이트 = 로컬 `Total=N PASS/0 FAIL` 증거 + Claude 코드리뷰(GitHub CI green 전제 아님). `ci.yml`·`governance-soft-guard.yml`는 `workflow_dispatch` 수동(분 가용 시), `ci.yml` test=ubuntu·wpf=windows. (`CLAUDE.md §11.6`)
...
## CORR-WP-01. Report Reconciliation `NOT_RUN` truth-state — **NEXT UP / NOT_IMPLEMENTED** (Cap C-31, RR-17)
...
- **테스트**: (a) zero checks + `Passed=true` synthetic input -> SUMMARY `NOT_RUN`, `PASS` 아님, (b) nonzero all-pass -> PASS, (c) nonzero fail -> FAIL, (d) missing limit finding remains in VALIDATION/EXCEPTION_LIST. 기존 900 단언 삭제·약화 0, 신규는 Report/UiContract domain, Unclassified=0.
- **완료조건**: build warning 0/error 0; SmokeTest `Total>900 PASS=Total FAIL=0`; PackageReference 0; Gate A 0; false PASS 문자열 경로 0. 머지 후 latest main에서 v0.7.1 package를 재생성한다.
...
- **상태**: **VERIFIED code cut** — PR #133 squash merged at `abab29b`; 정확히 3파일을 0.7.1로 락스텝. build warning 0/error 0, SmokeTest `Total=900 PASS=900 FAIL=0`, post-merge build/00~03 PASS, manifest 27 entries. Candidate SHA256 `A70D0B37AD92344A2ECFBE0D4D96360F56CBAFFF94363249F0BD1A20ADC1ECDC`. Remote tag/Release는 없음. **CORR-WP-01 merge 뒤 candidate를 재생성하고 tag/publish한다.**
...
- **Claude Review Checklist**: 정확히 3파일·역사 표기 미변경 / 음성 케이스 상이-버전 유효성 유지 / drift 가드 보존·통과 / 단언 가감 0→`Total=900` 불변 / NuGet·서명·인증서 0(STOP) / Gate A 0 / build/03 PASS 증거 / current main·binary-impact 두 SHA 구분 보고.
