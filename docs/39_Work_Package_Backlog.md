...
- **STAB-WP-03b 구현완료·검토중 (IN REVIEW, branch `feature/stab-wp-03b-runtime-integrity`, base local-gate PASS; 봇 P2 수정 델타 재검증 대기 = 상태 PARTIAL)**: runtime Fail-Closed(Design 3 interim). 신규 `Core/Integrity/`(IntegrityStatus·IntegrityResult·IntegrityVerifier·IntegrityGate)가 build/03 §4를 in-process 포팅(SHA256 전용, NuGet 0, build/01·03 미변경, 513 불변). `App.OnStartup` 최상단에서 검증→FailClosed=Shutdown(2). 데이터/자산 변조 + manifest 부재/축소/버전불일치 차단. dev 스위치 강화(`RMAI_DEV_ALLOW_UNVERIFIED=1` + `Debugger.IsAttached`). **잔여 위험(미탐지, 명시)**: manifest 독립 앵커 부재(co-tamper)·self-contained 런타임 DLL 미해시·폴더 동반 변조 → **코드 서명(APPROVAL_REQUIRED)** 후속(`STAB-WP-05`). Codex local-gate(2026-06-28, base `0b10607`): `dotnet build` 0 warning/0 error, SmokeTest `Total=555 PASS=555 FAIL=0`, build/00~03 `-Version 0.6.0` PASS(manifest 25 entries, ZIP SHA256 `D591541A251B6E250C99FC49BB0D1D9BFEA2A5E37F026839131C46463155F075`). **이후 PR #61 봇 P2 4건 수정(`99fc508`): IntegrityVerifier 하드닝(① null 엔트리 → FailClosed ② malformed path try/catch ③ mandatory는 manifest `required` 플래그와 무관하게 path로 강제 ④ 플랫폼 무관 rooted/UNC 거부) + 회귀 +5 → 예상 `Total=560`. 이 델타는 로컬 재검증 대기(증거 없으면 PASS 금지).** 머지 전제 = 로컬 `Total=560 PASS/0 FAIL` 재확인 + Claude 리뷰.
