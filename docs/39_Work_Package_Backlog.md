...
- **현재 기준선**: main `600d687`, VERSION **0.6.0** (**v0.6.0 정식 릴리스 태그 = `3dfa80b`**). R1(WP-01~08)·R3(R3-WP-01~05)·REL-v0.6 가드(#54)·**STAB-WP-01(#56)·STAB-WP-02(#57)·STAB-WP-03a(#59)·STAB-WP-03b(#61)·STAB-WP-04(#66)·STAB-UX-01(#68)·UX-WP-01(#70)** 모두 **DONE**. 추가 머지: truth-sync(#63·#67·#69)·Smart Assist 설계(#62, UX 트랙)·STAB-WP-04+UX-WP-01 kickoff(#65). SmokeTest **`Total=602 PASS=602 FAIL=0`**(local-gate; STAB-UX-01 정본 579 + UX-WP-01 `Assist` 도메인 +23).
...
- **NEXT UP (다음 WP)**: **`UX-WP-02`(Static SQL/VBA/Excel/Risk Providers)** — UX-WP-01 계약 위에 정적 provider 5종(SQL keyword/snippet·VBA 안전 snippet·Excel2021 함수·Excel365 차단+대체 힌트·SafetyHint(기존 Checker 재사용)·Risk phrase). 차단셋은 **기존 `SqlSafetyChecker`/`VbaSafetyChecker`/`Excel2021FunctionChecker`+RuleSet 단일 원천 재사용**(중복 정의 금지), 실데이터/실 테이블명/원문 0. WP 상세 = `docs/39 §UX-WP-02`. **Codex 프롬프트 READY**: `prompts/codex/UX-WP-02_static_providers.md`(UX-WP-03 popup 프롬프트도 작성 완료) → Codex 핸드오프 가능. 병행: PILOT Gate B/C(BLOCKED, 실 Test PC).
- **그 다음 후보(순서, NEXT UP 아님)**: UX-WP-02(정적 Provider) → UX-WP-03(WPF Popup) → STAB-UX-02(레이아웃 영속화) → **STAB-WP-05(Authenticode 코드 서명 — APPROVAL_REQUIRED, 외부 신뢰 루트, STOP)** → R2-WP-01(Risk Semantic Hardening) → R2-WP-02~04.
