...
- **NEXT UP (다음 WP)**: **`R2-WP-01`(Risk Semantic Hardening — RR-15)** — 중복 Limit Key `group.Last()` 임의선택 제거→`DUPLICATE_LIMIT` 상태화, 통화·단위 ColumnMapping, 휴면 `RECON_UNIT_MISMATCH` 활성, BASE_DT 검증/정규화, Join 규칙 Audit. **인박스·NuGet 0·R1 계약(6상태 ADD-ONLY·대사 9코드·일원화) 보존.** **WP 상세 = `docs/39 §R2-WP-01`, Codex 프롬프트 = `prompts/codex/R2-WP-01_semantic_hardening.md`** — 둘 다 **PR #77(R2 트랙 설계)에 있으며, #77 머지 후 핸드오프 활성**. 병행: PILOT Gate B/C(BLOCKED, 실 Test PC).
...
## R2-WP-01. Risk Semantic Hardening (RR-15) — 상태: NOT_IMPLEMENTED (설계)
