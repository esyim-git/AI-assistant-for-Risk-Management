...
- **NEXT UP (다음 WP) = (트랙 결정 대기)**: **R2 분석 트랙(v0.7.0) 완결**. 사용자 트랙 결정 전까지 신규 구현 WP를 임의 착수하지 않는다(특히 KB는 STOP 규칙). 후보 트랙: ① **KB 원문 Clause/Chunk 검색**(v0.8 — 인박스 keyword-only는 가능, **Vector/Embedding/모델은 STOP·승인 문서 선행** `docs/40·41`) · ② **R5 Feedback Learning 확장**(`Core/Feedback` 승격 구조 영속/검색/재사용/Audit) · ③ **실 Test PC Gate B/C 증거**(BLOCKED 해소, `docs/45`; R2-WP-04 Excel 수동열기 검증 포함) · ④ **STAB-WP-05 Authenticode 코드서명**(APPROVAL_REQUIRED). 병행: PILOT Gate B/C(BLOCKED, 실 Test PC).
- **그 다음 후보(순서)**: 위 ①~④ 중 사용자 지정 트랙 → 해당 트랙 ADR(`docs/40`)·WP(`docs/39`)·프롬프트 선행 후 1 WP씩. **STAB-WP-05**=**APPROVAL_REQUIRED/STOP**이라 구현 NEXT UP 부적격. (선택) v0.7.0 정식 릴리스 태깅/Release ZIP은 별도 REL WP.
