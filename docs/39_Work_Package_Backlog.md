...
- **작업범위**: ① `RegisterCompletionTextBox`에 **debounced `TextChanged`**(인박스 `DispatcherTimer`, NuGet 0) 추가 → 기존 `ShowCompletionPopup(textBox,language)` 재사용 ② **최소 prefix 게이트**(`ExtractCompletionPrefix` 재사용, 길이 ≥ 임계일 때만 트리거; 빈 prefix 전체덤프 방지; Ctrl+Space는 현행 유지) ③ **재진입 가드**(suppress 플래그 — `InsertCompletionText`/`AcceptCompletionItem`의 프로그램적 `Text` 세팅이 재트리거 안 하게) ④ **트리거 결정 로직 = 순수 static 헬퍼로 추출**(SmokeTest 가능, WPF 비의존) ⑤ Ctrl+Space·팝업 네비·accept 삽입·accept 해시 audit 불변 ⑥ surface만으로 audit 0(accept만)·자동삽입 0·NoModel 보존.
...
- **수정예상파일**: `App/MainWindow.xaml.cs`(TextChanged 배선·디바운스·suppress), 트리거 결정 순수 헬퍼(신규, Core 또는 App static), `tests/RiskManagementAI.SmokeTests/AssistTests.cs`(회귀).
- **Public Interface**: 트리거 결정 순수 static 메서드(prefix 길이/suppress/언어 → trigger bool). 기존 `CompletionEngine`/`CompletionPopup`/`ShowCompletionPopup` 계약 불변.
...
- **테스트**(도메인 `UiContract` 또는 적정): 결정 헬퍼 — prefix<임계 미트리거·≥임계 트리거·suppress 중 미트리거·미지원/빈 텍스트 미트리거·Ctrl+Space 분리. 기존 `AssistTests` 보존. `Total`+N, Unclassified 0. (WPF 렌더 = Gate B.)
...
- **Claude Review Checklist**: Core 변경 0(트리거만)·`ShowCompletionPopup` 재사용 / 디바운스=인박스 `DispatcherTimer`(NuGet 0) / 최소 prefix 게이트·재진입 가드(수락 후 재트리거 0) / Ctrl+Space·네비·accept·audit 불변 / 자동삽입 0·surface audit 0·입력 원문 미저장 / NoModel·실시간 LLM=R4 STOP / 결정 로직 순수 헬퍼 SmokeTest / `AssistTests` 보존 / `Total` 보존+신규 / Gate A. **프롬프트**: `prompts/codex/UX-WP-05_smart_assist_as_you_type.md`.
