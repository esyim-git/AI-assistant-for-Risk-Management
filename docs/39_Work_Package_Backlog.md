...
- **NEXT UP (다음 WP) = 방향 결정 대기(decision point)**: 확장 트랙 Wave 1~3(#115~#127, QA-WP-01~09 + UX-WP-10/11) 완료로 **인박스 SmokeTest 도메인 하드닝 스윕이 완결**됨(Ncr·UiContract·Audit·Generation·Mapping·Packaging까지 회귀 고정). 비게이트 저위험 테스트 큐 소진. 이후 작업은 다음 3택 중 사용자 지정 필요(과대표기 금지): (a) **신규 기능 트랙 설계**(Claude 프롬프트 선행) · (b) **STOP·승인 게이트 착수** — STAB-WP-05 코드서명(인증서, APPROVAL_REQUIRED)·NCR 실 Rule Pack(APPROVAL_REQUIRED)·R4 Local LLM Runtime(STOP)은 전부 승인 선행 → **승인 결정 패킷 `docs/51`**(3게이트 결정 요청·권고·옵션·승인란; 승인 후 Claude가 후속 구현 WP+프롬프트 작성) · (c) **Gate B/C 실 오프라인 Test PC 증거**(user-driven, BLOCKED — `docs/48 §B″` 런북 준비됨). 인박스·Vector/Embedding STOP.
