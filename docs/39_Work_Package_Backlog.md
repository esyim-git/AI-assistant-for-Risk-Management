...
- **UX Enhancement 트랙(Gate B 2026-06-30 후속, 별도 큐)**: **UX-WP-04 Excel Function Helper**(프롬프트 READY `prompts/codex/UX-WP-04_excel_function_helper.md`) → UX-WP-05/06(Smart Assist 입력중 추천 표면화·Popup 확장, 스코프 확정 대기). **사용자 제안 UX-WP-01/02/03은 완료분과 ID 충돌이라 UX-WP-04~06으로 부여**(**ID 확정 2026-06-30**). **시퀀싱 확정(사용자 2026-06-30)**: **KB-WP-02 먼저**(현 NEXT UP 유지) → 그 뒤 UX-WP-04(Excel Helper).
...
> ⚠️ **ID 충돌 주의**: 사용자 제안 "UX-WP-01/02/03"은 **이미 완료·머지된** UX-WP-01(Smart Assist Core #70)·UX-WP-02(정적 Provider #72)·UX-WP-03(WPF Completion Popup #73)과 충돌한다(§0 재구현 금지·traceability). 충돌 방지를 위해 본 신규 트랙은 **UX-WP-04부터** 부여하고 사용자 제안명을 매핑한다: **UX-WP-04 = Excel Function Helper**(제안 "UX-WP-01"), **UX-WP-05 = Smart Assist 입력중 추천 표면화**(제안 "UX-WP-02 Smart Assist Core"), **UX-WP-06 = Smart Assist Popup 확장**(제안 "UX-WP-03 Smart Assist Popup"). **번호 체계 = UX-WP-04~06 확정**(사용자 2026-06-30). **시퀀스 = KB-WP-02(NEXT UP) → UX-WP-04 → UX-WP-05/06(스코프 확정 후).**
...
## UX-WP-05. Smart Assist 입력중 추천 표면화 (정적 Core 확장) (Cap C-30) — 스코프 확정 대기
> Gate B B-5 후속 UX Enhancement. ⚠️ **기존 완료분 확장(재구현 아님)**: Smart Assist Core(#70)·정적 Provider(#72)·WPF Popup(#73)은 이미 **VERIFIED(local-gate)**. 본 WP = **입력중(as-you-type) 추천 표면화** 델타.
- **목표**: Ctrl+Space 수동 트리거를 넘어 **정적 범위 내 입력중 추천**(prefix 기반·NoModel)을 실행 Release UI에 표면화. **실시간 LLM 랭킹/학습 = R4(STOP, 미구현)**.
- **상태**: **PLACEHOLDER — 스코프 확정 대기.** Gate B "Smart Assist 미표시"가 (a) 정적 코어 미배선(표면화 갭)인지 (b) Ctrl+Space-only 설계상 정상(입력중 추천=애초 범위 밖)인지 **실행 Release UI 조사 후 확정**. 기능 코드 미변경.
- **선행조건**: `Core/Assist/{CompletionEngine,CompletionProviderRegistry,Providers/StaticCompletionProviders}.cs` 조사.
- **작업범위(잠정)**: 입력중 추천 트리거(정적·결정적)·자동삽입 0·기존 `SuggestionLog` 해시 audit 재사용. 실제 범위는 조사 후 확정.
- **보안조건**: 외부 API·NuGet·모델 0·자동삽입 0·자동학습 0·입력 원문 미저장(해시).
- **Branch**: `feature/ux-wp-05-smart-assist-surfacing` · **프롬프트**: 스코프 확정 후 작성.

## UX-WP-06. Smart Assist Popup 확장 (Cap C-30) — 스코프 확정 대기
> Gate B 후속 UX Enhancement. ⚠️ **기존 WPF Completion Popup(#73) 확장(재구현 아님)**.
- **목표**: 기존 Ctrl+Space 팝업 UX 개선(추천 문구·snippet 표시·선택 삽입). 자동삽입 0 유지.
- **상태**: **PLACEHOLDER — UX-WP-05 스코프 확정에 종속.** 기능 코드 미변경.
- **보안조건**: 외부 API·NuGet·모델 0·자동삽입 0·입력 원문 미저장.
- **Branch**: `feature/ux-wp-06-smart-assist-popup` · **프롬프트**: UX-WP-05 확정 후 작성.
