...
- **UX Enhancement 트랙(Gate B 2026-06-30 후속, 별도 큐)**: **UX-WP-04 Excel Function Helper**(프롬프트 READY `prompts/codex/UX-WP-04_excel_function_helper.md`) → UX-WP-05/06(Smart Assist 입력중 추천 표면화·Popup 확장, 스코프 확정 대기). **사용자 제안 UX-WP-01/02/03은 완료분과 ID 충돌이라 UX-WP-04~06으로 부여**. **시퀀싱 결정 필요**: KB-WP-02(현 NEXT UP) 먼저 vs UX-WP-04(실 사용자 Gate B 갭) 먼저 — 사용자 지정 대기(기본값: KB-WP-02 유지).
