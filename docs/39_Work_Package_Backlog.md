...
- **현재 기준선**: main `e69a1ae` (KB-WP-02·UX-WP-04~06 #101~#104 머지 후), VERSION **0.7.0** (**v0.7.0 정식 릴리스 태그 = `30c1cfb`**, ZIP SHA256 `42C835…`·미서명; 직전 v0.6.0 태그 `3dfa80b`). R1(WP-01~08)·R3(R3-WP-01~05)·REL-v0.6 가드(#54)·**STAB-WP-01~04(#56/#57/#59/#61/#66)·STAB-UX-01(#68)·UX-WP-01(#70)·UX-WP-02(#72)·UX-WP-03(#73)·STAB-UX-02(#76)·R2-WP-01~04(#79/#81/#84/#87)·REL-v0.7.0(#90)·KB-WP-01(#94)·KB-WP-02(#101)·UX-WP-04(#102)·UX-WP-05(#103)·UX-WP-06(#104)** 모두 **DONE**. 추가 머지: truth-sync/계획/Skills(#63~#100 계열 다수). SmokeTest **`Total=792 PASS=792 FAIL=0`**(local-gate; 747 → 768(KB-WP-02 +21) → 778(UX-WP-04 +10) → 788(UX-WP-05 +10) → 792(UX-WP-06 +4)). **R2-WP-04(Visualization/Report) 완료**: 정확 Exception Count(Number SoT)·`RISK_VISUAL` 인박스 시트·WPF Shapes/Canvas 화면·시각화 caveat `ExcelReportResult.Findings` 표면화, Codex bot P2 4건 보정 후 Claude APPROVE 머지. **UX/STAB + R2 트랙 완료** — VERIFIED 범위는 local-gate 한정이며 실 오프라인 Test PC Gate B/C=**BLOCKED**(과대표기 금지; **2026-06-30 사용자 수동 부분검증** — B-1~4·B-7·B-9·B-10 **user-reported PASS(증거 미첨부)**·**B-5 PARTIAL**·B-6/B-8 PENDING, `docs/48 §B′`; 전체 BLOCKED 유지). STAB-WP-05(코드서명)=APPROVAL_REQUIRED(**인증서 경로 A 확정**). **UX Enhancement 트랙 완료**: UX-WP-04(Excel Function Helper #102)·UX-WP-05(Smart Assist as-you-type #103)·UX-WP-06(팝업 표시 확장 #104) 모두 **DONE**(local-gate; 실 UI 렌더=Gate B). **NEXT UP = FEEDBACK-WP-01**(승인 Example ingest 게이트+영속+결정적 검색+audit — ADR-014, **RETRIEVAL·학습 아님**; WP 시퀀스 KB-WP-01/02(DONE)→UX-WP-04/05/06(DONE)→FEEDBACK-WP-01→02). 인박스·Vector/Embedding STOP — 아래 Resume Brief 및 `## ★ FEEDBACK-WP-01` 참조.
...
- **NEXT UP (다음 WP) = FEEDBACK-WP-01 (승인 Example 본문 Ingest 게이트 + 결정적 검색 + audit)**: KB-WP-01/02·UX-WP-04/05/06 모두 DONE(#94/#101/#102/#103/#104, local-gate). 그 뒤 R5 Feedback 트랙 첫 WP를 집는다. ADR-014 + 프롬프트 `prompts/codex/FEEDBACK-WP-01_example_retrieval.md` READY. **RETRIEVAL·학습 아님(모델 가중치 쓰기 0) · 본문은 non-log DTO(`FeedbackDraftBodyInput`)로만(로그 직렬화 0) · ingest 게이트(SafetyChecker + 신규 `ForbiddenTermScanner`) · 인박스·Vector/Embedding STOP**. 기준선 main `e69a1ae`, `Total=792` → +N.
- **WP 시퀀스(사용자 승인)**: ① **KB-WP-01**(DONE #94) → ② **KB-WP-02**(DONE #101) → **UX-WP-04/05/06**(DONE #102~#104) → ③ **FEEDBACK-WP-01**(NEXT UP — 승인 Example ingest 게이트+영속+결정적 검색+audit) → ④ **FEEDBACK-WP-02**(검색결과 Prompt 반영, review 경유). **STAB-WP-05**(코드서명 — **인증서 경로 A 확정**; ADR-012/`docs/41 §6` 나머지 항목·서명 도구는 구현 WP에서 확정, 실 인증서·서명·검증 = Windows 실 Test PC 의존)는 병행 가능하나 본 시퀀스 뒤. 병행: PILOT Gate B/C(BLOCKED, `docs/48` 시트 준비됨 — 2026-06-30 부분 검증 반영).
...
## ★ FEEDBACK-WP-01. 승인 Example 본문 Ingest 게이트 + 결정적 검색 + audit (RETRIEVAL, 학습 아님) — **NEXT UP** (Cap C-20 확장)
...
- **Claude Review Checklist**: read-only 결합·원 Context 보존 / review 게이트(자동주입 0) / 학습/모델파일 0 / capture fake 관측 / 해시 audit / 도메인 Audit·Unclassified 0 / NuGet 0 / `Total` 보존+신규 / Gate A. **프롬프트**: FEEDBACK-WP-01 머지 후 작성.
