...
- **NEXT UP (다음 WP) = (트랙 결정 대기)**: REL-v0.7.0(#90)로 **v0.7.0 정식 릴리스 완료**(태그 `30c1cfb`·ZIP SHA256 `42C835…`·미서명). 사용자 트랙 결정 전 신규 구현 WP 임의 착수 금지. 후보·순서는 아래 줄.
- **후보 트랙(사용자 결정 — 순서 가이드)**: ① **STAB-WP-05 Authenticode 코드서명**(승인 문서 ADR-012/`docs/41 §6` READY — **사용자 인증서 경로 A~D 결정 선행**, 결정 전 `APPROVAL_REQUIRED/STOP`이라 구현 NEXT UP 부적격) · ② **KB 원문 Clause/Chunk 검색**(v0.8 — 인박스 keyword-only 가능, **Vector/Embedding/모델은 STOP·승인 선행** `docs/40·41`) · ③ **R5 Feedback Learning 확장**(`Core/Feedback` 영속/검색/재사용/Audit) · ④ **실 Test PC Gate B/C 증거**(BLOCKED 해소, `docs/45` 양식 재사용; R2-WP-04 Excel 수동열기 포함). 병행: PILOT Gate B/C(BLOCKED, 실 Test PC).
