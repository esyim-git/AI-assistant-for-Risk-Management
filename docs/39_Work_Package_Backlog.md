...
- **현재 기준선**: main `30c1cfb` (REL-v0.7.0 #90 머지 후), VERSION **0.7.0** (**v0.7.0 정식 릴리스 태그 = `30c1cfb`**, ZIP SHA256 `42C835…`·미서명; 직전 v0.6.0 태그 `3dfa80b`). R1(WP-01~08)·R3(R3-WP-01~05)·REL-v0.6 가드(#54)·**STAB-WP-01~04(#56/#57/#59/#61/#66)·STAB-UX-01(#68)·UX-WP-01(#70)·UX-WP-02(#72)·UX-WP-03(#73)·STAB-UX-02(#76)·R2-WP-01(#79)·R2-WP-02(#81)·R2-WP-03(#84)·R2-WP-04(#87)·REL-v0.7.0(#90)** 모두 **DONE**. 추가 머지: truth-sync(#63·#67·#69·#74·#78·#80·#82·#83·#85·#86). SmokeTest **`Total=714 PASS=714 FAIL=0`**(직접 local-gate 재확인: `dotnet build` 0/0 + SmokeTest, 2026-06-30; 631 + STAB-UX-02 +15 + R2-WP-01 +25 + R2-WP-02 +9 + R2-WP-03 +18 + R2-WP-04 +16). **R2-WP-04(Visualization/Report) 완료**: 정확 Exception Count(Number SoT)·`RISK_VISUAL` 인박스 시트·WPF Shapes/Canvas 화면·시각화 caveat `ExcelReportResult.Findings` 표면화, Codex bot P2 4건 보정 후 Claude APPROVE 머지. **UX/STAB + R2 트랙 완료** — VERIFIED 범위는 local-gate 한정이며 실 오프라인 Test PC Gate B/C=**BLOCKED**(과대표기 금지). STAB-WP-05(코드서명)=APPROVAL_REQUIRED(**인증서 경로 A 확정**). **NEXT UP = KB-WP-01**(공개 규정 Clause/Chunk Pack — ADR-013; WP 시퀀스 KB-WP-01→02→FEEDBACK-WP-01→02). 인박스 keyword-only·원문 repo 미포함·Vector/Embedding STOP — 아래 Resume Brief 및 `## ★ KB-WP-01` 참조.
...
- **NEXT UP (다음 WP) = KB-WP-01 (공개 규정 Clause/Chunk Pack 계약 + 로더 + 원문 가드)**: 사용자 트랙 결정 = **KB(②)→R5(③) 진행** + **STAB-WP-05 인증서 경로 = A(사내 Enterprise CA) 확정**. KB 트랙 먼저 — **KB-WP-01**을 집는다. ADR-013 + 프롬프트 `prompts/codex/KB-WP-01_clause_pack_contract.md` READY. **인박스 keyword-only · 규정 원문 repo 미포함(합성 더미만) · Vector/Embedding STOP**. 기준선 main `30c1cfb`, `Total=714` → +N.
- **WP 시퀀스(사용자 승인)**: ① **KB-WP-01**(NEXT UP — Clause Pack 계약/로더/가드) → ② **KB-WP-02**(clause keyword 검색 + `ClauseSnippetAllowed` 게이트 + clause 인용) → ③ **FEEDBACK-WP-01**(승인 Example ingest 게이트+영속+결정적 검색+audit) → ④ **FEEDBACK-WP-02**(검색결과 Prompt 반영, review 경유). **STAB-WP-05**(코드서명 — **인증서 경로 A 확정**; ADR-012/`docs/41 §6` 나머지 항목·서명 도구는 구현 WP에서 확정, 실 인증서·서명·검증 = Windows 실 Test PC 의존)는 병행 가능하나 본 시퀀스 뒤. 병행: PILOT Gate B/C(BLOCKED, `docs/48` 시트 준비됨).
...
## ★ KB-WP-01. 공개 규정 Clause/Chunk Pack 계약 + 로더 + 원문 가드 (인박스, 원문 repo 미포함) — **NEXT UP** (Cap C-28)
