...
- **NEXT UP (다음 WP)**: **`STAB-UX-02`(Resizable Layout 영속화)** — STAB-UX-01의 GridSplitter/창 크기·열·행 비율을 세션 간 영속화(순수 WPF/로컬 설정 파일, 외부 NuGet 0·실데이터 0·자동실행 0). UX 트랙 마지막 정리이며 R2 진입 전 STAB 마감 성격. 대안 후보 = **R2-WP-01**(Risk Semantic Hardening). WP 상세 작성 필요(현재 미작성) → 다음 Planning에서 `docs/39 §STAB-UX-02` + `prompts/codex/STAB-UX-02_*.md`. 병행: PILOT Gate B/C(BLOCKED, 실 Test PC).
