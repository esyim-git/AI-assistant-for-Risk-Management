...
- **현재 기준선**: main `693488c` (확장 트랙 Wave 2 #119~#122 머지 후), VERSION **0.7.0** (**v0.7.0 정식 릴리스 태그 = `30c1cfb`**, ZIP SHA256 `42C835…`·미서명; 직전 v0.6.0 태그 `3dfa80b`). R1(WP-01~08)·R3(R3-WP-01~05)·REL-v0.6 가드(#54)·**STAB-WP-01~04·STAB-UX-01/02·UX-WP-01~11·R2-WP-01~05·REL-v0.7.0·KB-WP-01/02·FEEDBACK-WP-01/02·QA-WP-01~05**(#56~#122 계열) 모두 **DONE**. 추가 머지: truth-sync/계획/Skills(#63~#118 계열 다수). SmokeTest **`Total=877 PASS=877 FAIL=0`**(local-gate; … → 834(UX-WP-09 #113 +5) → 861(QA-WP-01 #115 +15·QA-WP-02 #116 +6·UX-WP-10 #117 +6) → 877(QA-WP-03 #119 +4·QA-WP-04 #120 +5·QA-WP-05 #121 +4·UX-WP-11 #122 +3)). **FEEDBACK-WP-02(검색 Example → `DraftRequest.Context` review 경유 read-only 반영) 완료**: `DraftReferenceComposer` 결정적(개수/문자 상한·fencing)·원 Context 보존·`ReferencesReviewed` 게이트(자동주입 0)·`effectiveContext`를 `DraftRequest`·audit hash 양쪽에 사용(무참고 경로 RequestHash 불변)·hash-only reflection audit(`PromotedExampleReflection`)·`PromotedExampleRetriever` 미호출 — **RETRIEVAL·학습 아님**. **cleanup 완료**: UX-WP-07(#110 Smart Assist 표면화 하이진)·R2-WP-05(#109 dead Welford 필드 제거·동작 불변)·UX-WP-08(#111 팝업 Esc/Close 포커스 복원). **전부 Claude adversarial 4축 review(각 findings 독립 검증) APPROVE·0 confirmed findings 후 머지.** VERIFIED 범위는 local-gate 한정이며 실 오프라인 Test PC Gate B/C=**BLOCKED**(과대표기 금지; 2026-06-30 부분검증 `docs/48 §B′`, UX-WP-08 실 포커스 렌더=Gate B). STAB-WP-05(코드서명)=APPROVAL_REQUIRED(**인증서 경로 A 확정**). **UX-WP-09 완료**(#113: A-4 이중 핀 축소·finding 미손실). **확장 트랙 Wave 1 완료**(#115~#117: QA-WP-01 +15·QA-WP-02 +6·UX-WP-10 +6). **확장 트랙 Wave 2 완료**(#119~#122: QA-WP-03 Kb/citation +4·QA-WP-04 Report/RISK_VISUAL +5·QA-WP-05 Csv/Xlsx/DataProfile +4·UX-WP-11 Excel 카탈로그(차단 함수 추천 0 checker 가드) +3 — 전부 adversarial 4축 review·**0 confirmed findings** 후 머지). **NEXT UP = 확장 트랙 Wave 3**(잔여 테스트 도메인 하드닝 완결 — **QA-WP-06**(Ncr Rule Set 구조)·**QA-WP-07**(UiContract)·**QA-WP-08**(Audit/Generation)·**QA-WP-09**(Mapping/Packaging) — 4 WP 병렬·독립 브랜치·파일 겹침 0, 프롬프트 READY; 본 문서 확장 트랙 참조). 그 외 후보(신규 기능 트랙 설계·STAB-WP-05 승인·**Gate B/C 실 Test PC=user-driven**, `docs/48 §B″` 실행 런북)는 후속. 인박스·Vector/Embedding STOP.
...
- **NEXT UP (다음 WP) = 확장 트랙 Wave 3 (QA-WP-06·QA-WP-07·QA-WP-08·QA-WP-09 — 4 WP 병렬, 잔여 테스트 도메인 완결)**: Wave 2(#119~#122) 완료. Wave 3도 파일 겹침 0(`NcrTests` / `UiContractTests` / `AuditTests`·`GenerationTests` / `MappingTests`·`PackagingTests`)이라 **각각 독립 브랜치로 병렬 진행**하고 개별 PR로 열어둔다(Claude 배치 리뷰). 프롬프트 전부 READY(본 문서 확장 트랙 참조). **Wave 3로 인박스 테스트 도메인 하드닝 스윕이 사실상 완결** — 이후 비게이트 작업은 신규 기능 설계 또는 STOP-게이트 승인 필요(과대표기 금지). 그 외 후보: **신규 기능 트랙 설계**(Claude 프롬프트 선행) · **STAB-WP-05 코드서명**(APPROVAL_REQUIRED) · **Gate B/C 실 Test PC 증거**(user-driven, BLOCKED — `docs/48 §B″` 런북 준비됨).
...
### Wave 3 — READY (NEXT UP, 4 WP 병렬 · 잔여 테스트 도메인 완결)
> 인박스 SmokeTest 도메인 중 미하드닝 잔여(Ncr·UiContract·Audit·Generation·Mapping·Packaging)를 회귀로 고정. 전부 순수 additive 테스트(제품 코드 0)·합성 더미·파일 겹침 0. **Wave 3로 테스트 하드닝 스윕 사실상 완결.**
## ★ QA-WP-06. Ncr Rule Set 구조 SmokeTest 하드닝 — **READY** (품질)
- **목표**: NCR Rule Set 8요소 **구조**(SCAFFOLD_ONLY)·조회전용 SQL·검토용 초안·placeholder 계수(실 계수 0) 경계 회귀. **제품 코드 0·테스트만.** ⚠️ 도메인 `Ncr`(설명에 `NCR Rule`/`Rule Set` 토큰 — Ncr 분류에 필요; Kb 토큰 회피).
- **수정예상파일**: `tests/RiskManagementAI.SmokeTests/NcrTests.cs`. **도메인 `Ncr`**.
- **Branch**: `feature/qa-wp-06-ncr-hardening` · **프롬프트**: `prompts/codex/QA-WP-06_ncr_hardening.md`.

## ★ QA-WP-07. UiContract SmokeTest 하드닝 (XAML Contract·레이아웃·스냅샷) — **READY** (품질)
- **목표**: MainWindow XAML Contract(GridSplitter·Min·Stretch)·레이아웃 영속(`UiLayoutStore` round-trip/fallback/clamp)·읽기전용 스냅샷·탭 내비게이션의 **미커버 경계** 회귀. **제품 코드 0·테스트만.**
- **수정예상파일**: `tests/RiskManagementAI.SmokeTests/UiContractTests.cs`. **도메인 `UiContract`**.
- **Branch**: `feature/qa-wp-07-uicontract-hardening` · **프롬프트**: `prompts/codex/QA-WP-07_uicontract_hardening.md`.

## ★ QA-WP-08. Audit/Generation SmokeTest 하드닝 (해시 전용 로그·NoModel draft) — **READY** (품질)
- **목표**: `TaskLog`/`FeedbackLog`/`SuggestionLog` 해시 전용(원문/user 미저장·`IsSha256Hex` 강제)·`DraftPipeline`/`NoModelDraftService` 경로·6-positional 로그 스키마의 **미커버 경계** 회귀. **제품 코드 0·테스트만.**
- **수정예상파일**: `tests/RiskManagementAI.SmokeTests/{AuditTests,GenerationTests}.cs`. **도메인 `Audit`/`Generation`**.
- **Branch**: `feature/qa-wp-08-audit-generation-hardening` · **프롬프트**: `prompts/codex/QA-WP-08_audit_generation_hardening.md`.

## ★ QA-WP-09. Mapping/Packaging SmokeTest 하드닝 (Column Mapping·Integrity Manifest) — **READY** (품질)
- **목표**: Risk Column Mapping(설정·safe fallback·물리컬럼)·Integrity Manifest/`IntegrityVerifier`(critical glob·required·co-deletion·fail-closed)·build/03 스캔의 **미커버 경계** 회귀. **제품 코드 0·테스트만**(무결성 로직 변경 0). ⚠️ Packaging 단언은 `IntegrityVerifier.RequiredCriticalEntries`/`PackagingTests` 인벤토리와 정합.
- **수정예상파일**: `tests/RiskManagementAI.SmokeTests/{MappingTests,PackagingTests}.cs`. **도메인 `Mapping`/`Packaging`**.
- **Branch**: `feature/qa-wp-09-mapping-packaging-hardening` · **프롬프트**: `prompts/codex/QA-WP-09_mapping_packaging_hardening.md`.
