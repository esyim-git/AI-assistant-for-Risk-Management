...
- **현재 기준선**: main `f8b330a` (FEEDBACK-WP-01 #106 머지 후), VERSION **0.7.0** (**v0.7.0 정식 릴리스 태그 = `30c1cfb`**, ZIP SHA256 `42C835…`·미서명; 직전 v0.6.0 태그 `3dfa80b`). R1(WP-01~08)·R3(R3-WP-01~05)·REL-v0.6 가드(#54)·**STAB-WP-01~04(#56/#57/#59/#61/#66)·STAB-UX-01(#68)·UX-WP-01(#70)·UX-WP-02(#72)·UX-WP-03(#73)·STAB-UX-02(#76)·R2-WP-01~04(#79/#81/#84/#87)·REL-v0.7.0(#90)·KB-WP-01(#94)·KB-WP-02(#101)·UX-WP-04(#102)·UX-WP-05(#103)·UX-WP-06(#104)·FEEDBACK-WP-01(#106)** 모두 **DONE**. 추가 머지: truth-sync/계획/Skills(#63~#105 계열 다수). SmokeTest **`Total=807 PASS=807 FAIL=0`**(local-gate; 747 → 768(KB-WP-02 +21) → 778(UX-WP-04 +10) → 788(UX-WP-05 +10) → 792(UX-WP-06 +4) → 807(FEEDBACK-WP-01 +15)). **FEEDBACK-WP-01(승인 Example ingest 게이트·결정적 검색·audit) 완료**: 본문 non-log `FeedbackDraftBodyInput` 분리·ingest 게이트(SQL/VBA Blocker 0 AND `ForbiddenTermScanner` 0)·metadata-only fallback·`PromotedExampleRetriever` 결정적 검색(`Score` desc·`ExampleId` Ordinal)·hash-only `PromotedExampleSearch` audit — **RETRIEVAL·학습 아님**, Claude 5-lens review APPROVE 후 머지. **UX/STAB + R2 + FEEDBACK-WP-01 트랙 완료** — VERIFIED 범위는 local-gate 한정이며 실 오프라인 Test PC Gate B/C=**BLOCKED**(과대표기 금지; **2026-06-30 사용자 수동 부분검증** — B-1~4·B-7·B-9·B-10 **user-reported PASS(증거 미첨부)**·**B-5 PARTIAL**·B-6/B-8 PENDING, `docs/48 §B′`; 전체 BLOCKED 유지). STAB-WP-05(코드서명)=APPROVAL_REQUIRED(**인증서 경로 A 확정**). **UX Enhancement 트랙 완료**: UX-WP-04(Excel Function Helper #102)·UX-WP-05(Smart Assist as-you-type #103)·UX-WP-06(팝업 표시 확장 #104) 모두 **DONE**(local-gate; 실 UI 렌더=Gate B). **NEXT UP = FEEDBACK-WP-02**(검색 Example의 review 경유 Draft Prompt 반영 — ADR-014 §결정5, **RETRIEVAL·학습 아님·자동주입 0**; WP 시퀀스 KB-WP-01/02(DONE)→UX-WP-04/05/06(DONE)→FEEDBACK-WP-01(DONE)→FEEDBACK-WP-02). 인박스·Vector/Embedding STOP — 아래 Resume Brief 및 `## ★ FEEDBACK-WP-02` 참조.
...
- **NEXT UP (다음 WP) = FEEDBACK-WP-02 (검색된 승인 Example의 review 경유 Draft Prompt 반영)**: FEEDBACK-WP-01 DONE(#106, local-gate). 그 뒤 R5 Feedback 트랙 두 번째 WP를 집는다. ADR-014 §결정5 + 프롬프트 `prompts/codex/FEEDBACK-WP-02_prompt_reflection.md` READY. **RETRIEVAL·학습 아님(모델 가중치 쓰기 0) · `PromotedExampleRetriever` 검색결과를 `DraftRequest.Context`에 read-only reference block으로 review 경유 반영(자동주입 0·원본 Context 보존) · 기존 `DraftPipeline` hash audit 재사용(Context 이미 해시) · 결정적 · 인박스·Vector/Embedding STOP**. 기준선 main `f8b330a`, `Total=807` → +N.
- **WP 시퀀스(사용자 승인)**: ① **KB-WP-01**(DONE #94) → ② **KB-WP-02**(DONE #101) → **UX-WP-04/05/06**(DONE #102~#104) → ③ **FEEDBACK-WP-01**(DONE #106 — 승인 Example ingest 게이트+결정적 검색+audit) → ④ **FEEDBACK-WP-02**(NEXT UP — 검색결과 Prompt 반영, review 경유). **STAB-WP-05**(코드서명 — **인증서 경로 A 확정**; ADR-012/`docs/41 §6` 나머지 항목·서명 도구는 구현 WP에서 확정, 실 인증서·서명·검증 = Windows 실 Test PC 의존)는 병행 가능하나 본 시퀀스 뒤. 병행: PILOT Gate B/C(BLOCKED, `docs/48` 시트 준비됨 — 2026-06-30 부분 검증 반영).
...
## ★ FEEDBACK-WP-02. 검색 결과 Prompt 반영 (review 경유 read-only 주입) — **NEXT UP** (Cap C-20 확장)
