...
- **현재 기준선**: main `61cf782`, VERSION **0.6.0** (**v0.6.0 정식 릴리스 태그 = `3dfa80b`**). R1(WP-01~08)·R3(R3-WP-01~05)·REL-v0.6 가드(#54)·**STAB-WP-01~04(#56/#57/#59/#61/#66)·STAB-UX-01(#68)·UX-WP-01(#70)·UX-WP-02(#72)·UX-WP-03(#73)·STAB-UX-02(#76)** 모두 **DONE**. 추가 머지: truth-sync(#63·#67·#69·#74). SmokeTest **`Total=646 PASS=646 FAIL=0`**(local-gate; UX 트랙 정본 631 + STAB-UX-02 레이아웃 영속화 +15(UiContract +13·Packaging +2)). **UX/STAB 트랙 완료** — VERIFIED 범위는 **정적·NoModel·외부 Editor 0·자동삽입 0** 한정이며 실 LLM 랭킹/학습=**R4(NOT_IMPLEMENTED)**, 실 오프라인 Test PC Gate B/C=**BLOCKED**(과대표기 금지). STAB-WP-05(코드서명)=APPROVAL_REQUIRED.
...
- **NEXT UP (다음 WP)**: **`R2-WP-01` Claude review/merge** — Codex 구현 branch `feature/r2-wp-01-semantic-hardening` local-gate PASS(`dotnet build` 0/0, SmokeTest `Total=666 PASS=666 FAIL=0`, NuGet 0). 병합 전까지 상태는 **PARTIAL**이며, Claude review/merge 후 다음 구현 후보는 R2-WP-02. 병행: PILOT Gate B/C(BLOCKED, 실 Test PC).
- **그 다음 후보(순서, NEXT UP 아님)**: R2-WP-01 → (R2-WP-02 병행 가능, Data/ 한정) → R2-WP-03 → R2-WP-04 (cross-WP: R2-WP-01이 `LimitAnalysisMetadata` ctor 손대므로 R2-WP-03보다 선행·직렬화). **STAB-WP-05**(Authenticode 코드 서명)=**APPROVAL_REQUIRED/STOP**이라 구현 NEXT UP 부적격.
...
## R2-WP-01. Risk Semantic Hardening (RR-15) — 상태: PARTIAL (Codex local-gate PASS, Claude review/merge pending)
...
- **수정예상파일**: `src/RiskManagementAI.Core/Mapping/ColumnMapping.cs`(enum+SafeDefaults), `src/RiskManagementAI.Core/Mapping/ColumnMappingLoader.cs`(필수/선택 분리), `config/column_mapping.json`(더미 키 추가), `src/RiskManagementAI.Core/Risk/LimitMonitor.cs`, `src/RiskManagementAI.Core/Risk/LimitAnalysisResult.cs`, `tests/RiskManagementAI.SmokeTests/LimitReconciliationTests.cs`(+신규 회귀). Dashboard/Report 소비부는 컴파일 호환만 확인(기능변경 0).
- **Public Interface**:
...
- **Codex local-gate 결과(2026-06-29, branch `feature/r2-wp-01-semantic-hardening`)**: 구현 완료, Claude review/merge pending. `dotnet build RiskManagementAI.sln` 0 warnings / 0 errors. `dotnet run --project tests/RiskManagementAI.SmokeTests` → `Total=666 PASS=666 FAIL=0`, Unclassified=0. 신규 단언은 DuplicateLimit 차단·Optional Currency/Unit mapping·`RECON_UNIT_MISMATCH` 활성/비활성·BASE_DT 정상화/invalid graceful·JoinAudit 결정성. 외부 NuGet 0 / 실데이터 0 / 자동실행 0.
...
- **상태**: **NOT_IMPLEMENTED**(설계만). 본 Linux 환경엔 .NET SDK 없음 → 구현·build·SmokeTest는 추후 Codex 로컬(Local-Gate).
...
- SmokeTest 정본 Total 보존(현 기준 646; STAB-UX-02 #76 머지와 독립). 도메인 분류기 Unclassified=0 유지.
...
- 로컬 `dotnet build RiskManagementAI.sln -c Release` 0 error / 0 warning, `dotnet run --project tests/RiskManagementAI.SmokeTests` → `Total=N PASS=N FAIL=0`(기존 646 + 신규, 도메인 Unclassified=0).
...
- 커밋 본문에 NuGet 0·실데이터 0·외부 charting 미도입·SmokeTest Total 변화(646→N) 명시.
