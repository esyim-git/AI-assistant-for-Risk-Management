...
- **현재 기준선**: main `b7f56ce` (KB-WP-01 #94 머지 후), VERSION **0.7.0** (**v0.7.0 정식 릴리스 태그 = `30c1cfb`**, ZIP SHA256 `42C835…`·미서명; 직전 v0.6.0 태그 `3dfa80b`). R1(WP-01~08)·R3(R3-WP-01~05)·REL-v0.6 가드(#54)·**STAB-WP-01~04(#56/#57/#59/#61/#66)·STAB-UX-01(#68)·UX-WP-01(#70)·UX-WP-02(#72)·UX-WP-03(#73)·STAB-UX-02(#76)·R2-WP-01(#79)·R2-WP-02(#81)·R2-WP-03(#84)·R2-WP-04(#87)·REL-v0.7.0(#90)·KB-WP-01(#94)** 모두 **DONE**. 추가 머지: truth-sync(#63·#67·#69·#74·#78·#80·#82·#83·#85·#86·#91·#92), KB/R5 트랙 계획(#93), Project Skills(#95). SmokeTest **`Total=747 PASS=747 FAIL=0`**(직접 local-gate 재확인: `dotnet build` 0/0 + SmokeTest, 2026-06-30; 631 + STAB-UX-02 +15 + R2-WP-01 +25 + R2-WP-02 +9 + R2-WP-03 +18 + R2-WP-04 +16 + KB-WP-01 +33). **R2-WP-04(Visualization/Report) 완료**: 정확 Exception Count(Number SoT)·`RISK_VISUAL` 인박스 시트·WPF Shapes/Canvas 화면·시각화 caveat `ExcelReportResult.Findings` 표면화, Codex bot P2 4건 보정 후 Claude APPROVE 머지. **UX/STAB + R2 트랙 완료** — VERIFIED 범위는 local-gate 한정이며 실 오프라인 Test PC Gate B/C=**BLOCKED**(과대표기 금지; **2026-06-30 사용자 수동 부분검증** — B-1~4·B-7·B-9·B-10 **user-reported PASS(증거 미첨부)**·**B-5 PARTIAL**·B-6/B-8 PENDING, `docs/48 §B′`; 전체 BLOCKED 유지). STAB-WP-05(코드서명)=APPROVAL_REQUIRED(**인증서 경로 A 확정**). **신규 UX Enhancement 트랙(Gate B B-5 후속)**: UX-WP-04(Excel Function Helper) + UX-WP-05/06(Smart Assist as-you-type 표면화·Popup 표시 확장, **스코프 확정·프롬프트 READY**) 등록 — 기능 코드 미변경, `## UX Enhancement 트랙` 참조. **NEXT UP = KB-WP-02**(clause keyword 검색 + ClauseSnippetAllowed 게이트 + clause 인용 — ADR-013; WP 시퀀스 KB-WP-01(DONE)→02→FEEDBACK-WP-01→02). 인박스 keyword-only·원문 repo 미포함·Vector/Embedding STOP — 아래 Resume Brief 및 `## ★ KB-WP-02` 참조.
...
- **NEXT UP (다음 WP) = KB-WP-02 (clause keyword 검색 + clause 인용 + ClauseSnippetAllowed 게이트)**: KB-WP-01(계약/로더/가드)이 **#94 `b7f56ce` DONE**(fail-closed catalog-only fallback·length-prefixed ChunkId·합성 더미·`SourceTextAllowed` false 불변). 그 토대 위에서 **KB-WP-02**를 집는다. ADR-013 + 프롬프트 `prompts/codex/KB-WP-02_clause_search.md` READY. **인박스 keyword-only · 규정 원문 repo 미포함(합성 더미만) · `SourceTextAllowed`/원문 미노출 불변 · 발췌는 `ClauseSnippetAllowed`만 게이트 · Vector/Embedding STOP**. 기준선 main `b7f56ce`, `Total=747` → +N.
- **WP 시퀀스(사용자 승인)**: ① **KB-WP-01**(DONE #94 — Clause Pack 계약/로더/가드) → ② **KB-WP-02**(NEXT UP — clause keyword 검색 + `ClauseSnippetAllowed` 게이트 + clause 인용) → ③ **FEEDBACK-WP-01**(승인 Example ingest 게이트+영속+결정적 검색+audit) → ④ **FEEDBACK-WP-02**(검색결과 Prompt 반영, review 경유). **STAB-WP-05**(코드서명 — **인증서 경로 A 확정**; ADR-012/`docs/41 §6` 나머지 항목·서명 도구는 구현 WP에서 확정, 실 인증서·서명·검증 = Windows 실 Test PC 의존)는 병행 가능하나 본 시퀀스 뒤. 병행: PILOT Gate B/C(BLOCKED, `docs/48` 시트 준비됨 — 2026-06-30 부분 검증 반영).
- **UX Enhancement 트랙(Gate B 2026-06-30 후속, 별도 큐)**: **UX-WP-04 Excel Function Helper**(프롬프트 READY `prompts/codex/UX-WP-04_excel_function_helper.md`) → **UX-WP-05**(Smart Assist as-you-type 표면화, 프롬프트 READY `prompts/codex/UX-WP-05_smart_assist_as_you_type.md`) → **UX-WP-06**(Popup 표시 확장, 프롬프트 READY `prompts/codex/UX-WP-06_smart_assist_popup_display.md`). **사용자 제안 UX-WP-01/02/03은 완료분과 ID 충돌이라 UX-WP-04~06으로 부여**(**ID 확정 2026-06-30**). **시퀀싱 확정(사용자 2026-06-30)**: **KB-WP-02 먼저**(현 NEXT UP 유지) → 그 뒤 UX-WP-04 → UX-WP-05 → UX-WP-06. (UX-WP-05/06 스코프 = 2026-06-30 코드 조사 확정: Smart Assist는 Ctrl+Space-only by-design이라 B-5는 회귀 아님 — UX-WP-05가 as-you-type 신규 추가.)
...
## ★ KB-WP-02. clause keyword 검색 + clause 인용 + ClauseSnippetAllowed 게이트 — **NEXT UP** (Cap C-28)
...
## ★ UX-WP-04. Excel Function Helper (함수 상세설명·예시·Excel 2021 대체식 — 정적·NoModel) (Cap C-29)
...
## ★ UX-WP-05. Smart Assist 입력중(as-you-type) 추천 표면화 (정적·NoModel, WPF 트리거만) (Cap C-30)
...
## UX-WP-06. Smart Assist Completion Popup 표시 확장 (Snippet·SafetyNote·Kind, 자동삽입 0) (Cap C-30)
