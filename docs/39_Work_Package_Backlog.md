...
- **작업범위**: ① 함수 메타 **단일원천 = 기존 `Excel2021FunctionChecker`/RuleSet 재사용**(차단/365전용/대체식 목록 중복정의 금지) + 정적 helper 카탈로그(설명·인수·리스크관리 예시·수식 예시) = repo 내 **공개/더미 Excel 함수정보 데이터 파일**(`rules/` 또는 `config/`, 실데이터 0) ② 신규 `ExcelFunctionHelper`(Core: name/부분일치 검색·조회 API) + `ExcelFunctionInfo` record ③ 사용가능 함수 추천(차단 함수 → 대체식·대체 함수 매핑) ④ **수식 삽입 = 사용자 명시 선택 시에만**(자동삽입 0) ⑤ **입력 원문(검색어) 로그 미저장**(평문 미기록; 감사 필요 시 해시만) ⑥ WPF 표면화 = 읽기전용 helper view(기존 패널 재사용).
...
- **수정예상파일**: `Core/Excel/ExcelFunctionHelper.cs`·`ExcelFunctionInfo.cs`(신규), 함수정보 더미 데이터 파일(`rules/excel_function_help.*`), `App/`(helper view 배선), `tests/RiskManagementAI.SmokeTests/`(회귀).
...
- **완료조건**: helper 검색/상세/대체식/추천 + 사용자선택 삽입 + build 0/0 + 기존 보존 + Gate A.
...
- **Claude Review Checklist**: 함수 메타 단일원천(기존 checker/RuleSet 재사용·중복 0) / 상세 완비(설명·인수·리스크예시·수식예시·365여부·대체식·추천) / 수식 삽입 사용자선택만(자동삽입 0) / 입력 원문 로그 미저장(해시 전용) / 외부 API·NuGet 0·실데이터 0 / `CLAUDE.md §6` 정합 / 기존 checker 회귀 / `Total` 보존+신규 / Gate A. **프롬프트**: `prompts/codex/UX-WP-04_excel_function_helper.md`.
