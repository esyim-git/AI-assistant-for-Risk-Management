...
### A.3 ADR-009 필수 항목 — 실측 대상(1단계 PoC에서 채움)
후보 Runtime/Model 명·버전 · 오프라인 기동 여부 · **응답시간(첫토큰/전체)** · **SQL/VBA 한국어 보조 품질** · **규정답변 10단계 품질** · **환각률** · **인용 준수율** · RAM/CPU/GPU 최소·권장 · 배포 크기 · Runtime/Model **Integrity Hash** · 네트워크 격리 증거 · Output→Safety Checker 연계 확인. → **전부 `측정 필요(격리 PoC)`. 지금 값 지어내지 않음.**
> 템플릿: `.claude/skills/risk-llm-approval/model-approval-package-template.md` (1단계 승인 시 이 템플릿을 실측으로 채움).
...
### C.1 권고 — 적재·승인 SOP
1. **위치**: 실 Rule Pack = Prod 권한통제 KB(오프라인, 역할권한·조회로그). repo·release ZIP **미포함**(build/03 원문 스캔이 유입 차단 — 현 가드 유지).
2. **승인 주체**: 준법/리스크관리 문서오너. 계수 출처·시행일·근거조항 명시본만 적재.
3. **로딩 게이트(코드측, 이미 구조 존재)**: Rule Pack 부재 시 계산 차단(SCAFFOLD_ONLY 유지)·Validation SQL 조회전용(`SqlSafetyChecker`)·자동실행 0·답변은 **검토용 초안** 표기(10단계 형식, `docs/10`/§10). 실 계수 적재 후에도 산정 결과는 "검토용 초안"이며 공식 산정 아님.
4. **원문 미포함 불변식**: 적재는 메타+권한통제 본문까지, repo는 **스키마+`(확인 필요)` 경고**까지(현행). `file_hash`·실 version·시행일은 Prod 적재 시 채움.
