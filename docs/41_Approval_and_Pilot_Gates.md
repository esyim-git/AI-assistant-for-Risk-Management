...
## 6. Code-Signing Approval Gate (STAB-WP-05) — **APPROVAL_REQUIRED · STOP**
독립 신뢰 앵커(Authenticode 코드 서명) 도입 전 **반드시** 통과. ADR-008 §결정5의 잔여 3건(콘텐츠 lock-step co-tamper · self-contained 런타임 DLL 미해시 · 폴더 동반 변조)은 manifest만으로 닫히지 않으며, **빌드 시 외부 신뢰 루트 서명 + 런타임 서명 선행 검증**으로만 닫힌다. 인증서·서명 도구 = 외부 신뢰 루트 → **STOP 규칙(§11.5)**. (요건 정의 = `docs/40` ADR-012.)

> **상태: APPROVAL_REQUIRED — 인증서 경로 결정 완료((A) 사내 Enterprise CA, 2026-06-30 사용자 승인).** 핵심 STOP 결정(경로)은 해소. 나머지 §6.2 항목(보관·반입·검증정책·오프라인 폐기·Rollback)과 서명 도구(인박스 `Set-AuthenticodeSignature` 1순위)는 **STAB-WP-05 구현 WP에서 확정**하며, 실 인증서·서명·검증은 **Windows 실 Test PC/운영 환경 의존** → 실 증거 전 VERIFIED/PASS 금지. **v0.7.0은 본 게이트 완료 전 미서명+manifest/Fail-Closed로 출하**(서명은 후속, 릴리스 차단 아님).

### 6.1 인증서 경로 결정 (택1 — 사용자 결정, 승인 필요)
- [x] **(A) 사내 Enterprise CA 코드서명 인증서 — ✅ 선택(2026-06-30, 사용자 승인)** — 오프라인·비용 0·반입 용이, 내부 신뢰 한정(본 제품 성격 적합). 권장 1순위 채택.
- [ ] (B) 상용 OV 코드서명 인증서 — 외부 신뢰, 연 비용·발급 심사, 온라인 폐기/타임스탬프 의존. (미채택)
- [ ] (C) 상용 EV 코드서명 인증서 — HW 토큰 강제·SmartScreen 평판, 비용·운영 최대. (미채택)
- [ ] (D) 자체 서명 + 사내 신뢰 저장소 수동 등록 — 비용 0·신뢰 배포 수동(최소안). (미채택)
...
> 관련: `docs/38`(Train), `docs/39`(WP), `docs/28`(보안 게이트), `docs/34`(릴리스 리허설), `docs/32`(거버넌스), `docs/17`/`docs/08`(RAG/NCR), `docs/40`(ADR-008·ADR-012), `docs/47`(v0.7.0 미서명 출하), **`docs/51`(STOP·승인 게이트 결정 패킷 — §2·§3·§6 승인 요청 정리)**.
