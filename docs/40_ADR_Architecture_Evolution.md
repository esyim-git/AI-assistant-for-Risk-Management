...
- **상태**: 채택(설계) — FEEDBACK-WP-01은 **DONE/VERIFIED(local-gate, #106)**, FEEDBACK-WP-02는 **NEXT UP**. 현재 R5 = **PARTIAL**(승인 Example 본문 ingest·영속·결정적 retrieval까지 완료, Prompt 반영 미구현). 실 Test PC Gate 전 VERIFIED 과대표기 금지.
- **맥락**: R5 현 상태는 `FeedbackLogEntry`(승인 표식) → `FeedbackDraftBodyInput`(non-log 본문 입력) → `ExamplePromotion.PromoteApproved`(승인 필터·중복차단·UserIdHash 검증·ingest 게이트) → `PromotedExampleStore`(`config/promoted_examples.jsonl` append/readAll) → `PromotedExampleRetriever`(keyword/score 결정적 retrieval + hash-only audit) → MainWindow Feedback Center 표시까지다. FEEDBACK-WP-01로 `PromotedExample.ExampleBody`는 ingest 게이트 통과 시에만 저장되고, 검색 경로도 구현됐다. 남은 범위는 검색된 Example을 review 경유 read-only 참고 블록으로 `DraftPipeline`/`DraftRequest.Context`에 반영하는 FEEDBACK-WP-02다.
- **결정**:
  ...
  5. **Prompt 반영(FEEDBACK-WP-02) = review 경유 read-only 주입.** 검색 결과를 `DraftRequest.Context`(또는 신규 옵션 필드)에 **참고 블록으로 결합**하되 원 Context 보존, **자동 무검토 주입 금지**(review gate), 산출은 검토용 초안. 관측은 테스트용 capture `ILocalDraftService`로(=`NoModelDraftService`는 Context 결합 직접 관측 불가). 결합기는 결정적 개수/문자 수 상한, multi-line reference fencing/indentation, 선택된 `ExampleId` 목록의 hash-only audit을 포함해야 한다.
  6. **영속·커밋 가드.** Example 본문 jsonl은 `config/` 샌드박싱(`PromotedExampleStore.ResolveConfigFile` 패턴) + **`.gitignore`에 `config/promoted_examples*.jsonl`(및 smoke fixture) 추가**(STAB-UX-02 `*.local.json` 동형). 본문 영속 파일이 **tracked되지 않음**을 SmokeTest로 고정. revoke(무효화)는 후속 고려하며, FEEDBACK-WP-02 reference 결합 개수/문자 수 상한은 §결정5 범위에 포함한다.
