...
- **결정**:
  1. `CsvReader`가 **UTF-8**(BOM/무BOM) 지원 + Auto 감지(결정적). **CP949는 net8.0에서 인박스 아님** — `Encoding.GetEncoding(949)`는 `CodePagesEncodingProvider` 필요, 이는 **`System.Text.Encoding.CodePages` NuGet 패키지**(MS 1st-party)다. 따라서 CP949 = **DATA_SPEC_REQUIRED 결정 게이트**: (A·권장) **repo 내장 EUC-KR/CP949 디코더**(공개 표준 매핑표 리소스, NuGet 0 유지) 또는 (B) **`System.Text.Encoding.CodePages` 승인**(NuGet-0 불변식 깨짐, 소스/벤더링 필요). **결정 전 패키지 추가 금지.**
