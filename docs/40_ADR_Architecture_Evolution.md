...
- **결정**:
  ...
  3. **Safety/룰 재사용** — SQL/VBA/Excel 차단·허용 판단은 **기존 `SqlSafetyChecker`/`VbaSafetyChecker`/`Excel2021FunctionChecker`+RuleSet** 경유(룰 중복 정의 금지). 입력 시 위험은 `SafetyHint`/`BlockedHint` + 기존 결과 패널 연계.
  4. **Audit = 해시 전용** — accept 시 `SuggestionId·ProviderId·Language·Mode·UserHash·시각`만(입력 원문/삽입 본문 미저장).
