...
**④ 패키징 원문 미포함 스캔 (이번 릴리스 신규)**: `prompts/codex/REL-v0.6-packaging-guard.md`로 `build/03`(및 01)에 **`KbRepositoryGuard` 동등 스캔**을 연결한다 — 패키지에 들어가는 `kb/`·`config/`·`samples/`에서 내부규정/NCR **원문 의심 파일**(allowlist 외)을 발견하면 패키징 **실패**. (현재는 SmokeTest/CI에서만 실행 — docs/41 §2 후속①.)
