...
> **현재 기준선 = v0.6.0 + STAB-WP-01~04 + STAB-UX-01** (main `dd286fa`, PR #68 머지 후). 본 문서는 v0.6.0 다음 단계부터 v1.0 Team Pilot까지의 통합 실행 로드맵이다.
...
- **SmokeTest**: **579 PASS / 0 FAIL** (정본 합계 — STAB-UX-01(#68) local-gate 후 `Total=579`; STAB-WP-04 정본 572 + STAB-UX-01 XAML Contract +7). STAB-WP-02 최초 정본은 `Total=513`이었고 STAB-WP-03b 회귀가 +59(→572) 추가됐다. 과거 484/502는 하니스가 합계를 안 찍던 시절의 미집계 추정치였고, STAB-WP-02가 **합계·도메인별 PASS/FAIL·실행시간**을 출력한다.
...
| **UX** | (병행) v0.7.x | **Smart Assist / Inline Assist** (입력 중 자동완성·snippet·추천 문구·실시간 안전 힌트, **정적·NoModel**) + Resizable Layout | Gate A(보안) | **STAB-UX-01 DONE**(#68) · **UX-WP-01 NEXT**(Smart Assist Core) |
