...
> **현재 기준선 = v0.7.0 (R2 정식 릴리스) + STAB-WP-01~04 + STAB-UX-01/02 + UX-WP-01~11 + R2-WP-01~05 + QA-WP-01~09 + REL-v0.7.0 + KB-WP-01/02 + FEEDBACK-WP-01/02** (**코드/테스트 baseline `7094d91`** — docs-only 머지 #128~#131은 baseline 미변경(관례), current main `git log` 확인(#131 `110e9ee`=제안서 채택) —, 확장 트랙 Wave 3 #124~#127 머지 후; **R2 분석 트랙 v0.7.0 정식 릴리스** 태그 `30c1cfb`·ZIP SHA256 `42C835…`·미서명; 직전 v0.6.0 태그 `3dfa80b`). **KB-WP-01/02 DONE**(#94/#101, local-gate). **UX-WP-04/05/06 DONE**(#102~#104, Excel Function Helper + Smart Assist as-you-type/팝업, local-gate). **FEEDBACK-WP-01 DONE**(#106, 승인 Example ingest 게이트·non-log DTO·결정적 검색·hash audit — RETRIEVAL·학습 아님). **FEEDBACK-WP-02 DONE**(#108, 검색 Example → `DraftRequest.Context` review 경유 read-only 반영 — `DraftReferenceComposer` 결정적·`ReferencesReviewed` 게이트·`effectiveContext` 이중 audit hash·자동주입 0). **cleanup DONE**: UX-WP-07(#110 Smart Assist 표면화 하이진)·R2-WP-05(#109 dead Welford 필드 제거·동작 불변)·UX-WP-08(#111 팝업 포커스 복원, 실 렌더=Gate B)·**UX-WP-09**(#113 A-4 동일 finding 이중 핀 축소 — BlockedHint 유지·finding 미손실·결정적). **확장 트랙 Wave 1 DONE**(#115~#117: QA-WP-01 +15·QA-WP-02 +6·UX-WP-10 +6). **확장 트랙 Wave 2 DONE**(#119~#122: QA-WP-03 Kb +4·QA-WP-04 Report/RISK_VISUAL +5·QA-WP-05 Csv/Xlsx/DataProfile +4·UX-WP-11 Excel 카탈로그(차단 함수 추천 0) +3, adversarial 4축 0 confirmed). **확장 트랙 Wave 3 DONE**(#124~#127: QA-WP-06 Ncr 구조 +4·QA-WP-07 UiContract/레이아웃 영속 +4·QA-WP-08 Audit(해시전용)/Generation(NoModel) +8·QA-WP-09 Mapping/Packaging(무결성 manifest) +7 — 순수 additive 테스트(제품 코드 0)·adversarial 리뷰 0 confirmed; **인박스 SmokeTest 도메인 하드닝 스윕 완결**). **NEXT UP = REL-WP-071**(v0.7.1 출하 정합 릴리스 컷 — 결정지점은 제안서 채택 PR #131 `110e9ee`로 해소, `docs/39` 원장·`prompts/codex/REL-WP-071_release_cut_v0_7_1.md` READY·릴리스 문서 `docs/52`) ∥ **병행(user-driven)** = Gate B/C Round 1(`docs/48 §B″` 런북); 승인 게이트 3건(STAB-WP-05 서명·NCR 실 Pack·R4, APPROVAL_REQUIRED/STOP)=`docs/51` 결정 대기; v0.8 후속 후보=ARCH-WP-01·UI-WP-12(제안서 §8~§10). WP 시퀀스(완료) = KB-WP-01/02→UX-WP-04/05/06→FEEDBACK-WP-01/02→cleanup(UX-WP-07·R2-WP-05·UX-WP-08·UX-WP-09)→Wave 1(QA-WP-01·QA-WP-02·UX-WP-10)→Wave 2(QA-WP-03·QA-WP-04·QA-WP-05·UX-WP-11)→Wave 3(QA-WP-06·QA-WP-07·QA-WP-08·QA-WP-09). **STAB-WP-05 인증서 경로 = A(사내 Enterprise CA) 확정**(나머지 §6.2·서명도구는 구현 WP, 실 증거 전 APPROVAL_REQUIRED 유지). 본 문서는 v0.6.0 다음 단계부터 v1.0 Team Pilot까지의 통합 실행 로드맵이다.
...
- **SmokeTest**: **Total=900 PASS=900 FAIL=0** (정본 합계 — local-gate; `dotnet build` 0/0 + SmokeTest). 이력: … → 714(R2-WP-04 +16) → 747(KB-WP-01 +33) → 768(KB-WP-02 #101 +21) → 778(UX-WP-04 #102 +10) → 788(UX-WP-05 #103 +10) → 792(UX-WP-06 #104 +4) → 807(FEEDBACK-WP-01 #106 +15) → 829(FEEDBACK-WP-02 #108 +13·UX-WP-07 #110 +6·UX-WP-08 #111 +3·R2-WP-05 #109 +0) → 834(UX-WP-09 #113 +5) → 861(QA-WP-01 #115 +15·QA-WP-02 #116 +6·UX-WP-10 #117 +6) → 877(QA-WP-03 #119 +4·QA-WP-04 #120 +5·QA-WP-05 #121 +4·UX-WP-11 #122 +3) → 900(QA-WP-06 #124 +4·QA-WP-07 #125 +4·QA-WP-08 #126 +8·QA-WP-09 #127 +7). 과거 484/502는 하니스가 합계를 안 찍던 시절의 미집계 추정치였고, STAB-WP-02가 **합계·도메인별 PASS/FAIL·실행시간**을 출력한다.
...
| **REL** | **v0.7.1** | **출하 정합 컷**(published `30c1cfb` 이후 머지분 KB/UX/FEEDBACK/QA 반영 — 버전 범프 `0.7.0→0.7.1` 락스텝, 신규 기능 0·`Total=900` 불변) | Gate A · Local-Gate (Gate B/C=별도, published v0.7.1로 B13 봉인 경로 개방) | **READY — NEXT UP**(REL-WP-071, `docs/39`·`docs/52`·프롬프트 발부; 제안서 §10 WP-B 채택 #131) |
...
| R5 | v0.9.x | Feedback Learning (승인 Example 검색·반영) | — | **FEEDBACK-WP-01/02 DONE**(#106/#108, local-gate; WP-01 ingest 게이트·non-log DTO·결정적 검색·hash audit + WP-02 review 경유 read-only Prompt 반영·`effectiveContext` 이중 audit hash·자동주입 0 — RETRIEVAL·학습 아님) |
...
| 승인 Feedback Example 검색·Prompt 반영 | R5 | **FEEDBACK-WP-01/02 VERIFIED**(local-gate #106/#108; WP-01 ingest 게이트·non-log DTO·`PromotedExampleRetriever` 결정적 검색·hash audit + WP-02 `DraftReferenceComposer` review 경유 read-only 반영·`ReferencesReviewed` 게이트·`effectiveContext` 이중 audit hash·자동주입 0 — RETRIEVAL·학습 아님); 인박스·Vector STOP |
...
| C-22 | Smart Assist Core (Engine·Context·Item·Provider 계약·Registry·NoModel) | UX-WP-01 | 결정성·언어 라우팅·개수 상한·SafetyHint pinned·accept 해시 audit(원문 미저장) — `AssistTests`(+23) | A | **VERIFIED**(local-gate, #70. 계약+코어 한정; 실 Provider=C-23·UI=C-24 미구현) · **UX-WP-09 hygiene**(#113 `0f6e1d7` — A-4 동일 finding BlockedHint+SafetyHint 이중 핀 축소, `Findings` collapse 이전 산출로 finding 미손실·결정적·실 provider `SQL_DML_DELETE` 1행 회귀) |
...
| C-14 | Streaming/Perf (대용량 streaming·행/바이트 상한·Welford 온라인 누산·중복 해시·결정적 프로파일) | R2-WP-02 | streaming==기존 결정성·상한 `InvalidDataException`·Welford 누산·legacy-compatible OutlierCount·`DuplicateRowCount` 불변·(선택)벤치 | Data | **VERIFIED**(local-gate, #81 `5280d54`; `Total=680 PASS=680 FAIL=0`, Claude review APPROVE; streaming==in-memory 전필드 동일·OutlierCount 2-pass 정확·CP949 streaming 보존·상한 활성; 실 Test PC Gate B/C BLOCKED) · **R2-WP-05 hygiene**(#109 `e55f00d` — dead Welford `mean`/`m2` 필드 제거, OutlierWork 독립·프로파일 수치 바이트 불변·`Total` 무변) |
...
| C-20 | 승인 Example 검색·Prompt 반영 (RETRIEVAL, 학습 아님) | FEEDBACK-WP-01/02 | 승인 Example만·가중치 불변·ingest 게이트(SQL/VBA Blocker 0 AND ForbiddenTermScanner 0)·본문 non-log DTO 분리·결정적 검색(Score desc·ExampleId Ordinal)·해시 audit·review 경유 주입 | — | **FEEDBACK-WP-01/02 VERIFIED**(local-gate #106 `f8b330a` / #108 `4e82e8f`; WP-01 `PromotedExampleRetriever` 결정적 검색·`ExamplePromotion` ingest 게이트·`ForbiddenTermScanner`·hash-only audit·본문 non-log `FeedbackDraftBodyInput`·가중치 불변·모델파일 0 + WP-02 `DraftReferenceComposer` 검색 Example의 `DraftRequest.Context` review 경유 read-only 반영·`ReferencesReviewed` 게이트·`effectiveContext`를 `DraftRequest`·`DraftPipeline` audit hash 양쪽에 사용·hash-only reflection audit·`PromotedExampleRetriever` 미호출·자동주입 0); 인박스·Vector STOP |
...
| C-30 | Smart Assist 입력중 추천 표면화 / Popup 표시 확장 (정적·NoModel) | UX-WP-05/06 | UX-WP-05: debounced TextChanged 트리거(Core 변경 0·최소 prefix 게이트·재진입 가드·결정 헬퍼)·UX-WP-06: 팝업 Snippet/SafetyNote/Kind 표시·자동삽입 0·입력 원문 미저장 | A·Local-Gate | **VERIFIED**(local-gate #103/#104; Core `CompletionTriggerPolicy`·focus-preserving show(`grabFocus:false`)·TextBox Up/Down·stale close·display formatter·Insertable 게이트/audit 불변; 실시간 LLM=R4; 실 UI 렌더 Gate B BLOCKED) · **UX-WP-07 hygiene**(#110 `246e577` — dedupe Kind 인지·Info/Low 힌트 필터(≥Medium)·`ICompletionProviderWarningSource` allow-fn warning 표면화) · **UX-WP-08 hygiene**(#111 `10030be` — 팝업 Esc/Close 포커스 복원 `Close(restoreFocus)`, 실 포커스 렌더=Gate B) |
