...
> **현재 기준선 = v0.6.0 + STAB-WP-01~04 + STAB-UX-01/02 + UX-WP-01~03 + R2-WP-01** (main `59a752f`, PR #79 머지 후; R2 트랙 v0.7.0 진행 중·미릴리스). 본 문서는 v0.6.0 다음 단계부터 v1.0 Team Pilot까지의 통합 실행 로드맵이다.
...
- **SmokeTest**: **671 PASS / 0 FAIL** (정본 합계 — R2-WP-01(#79) local-gate 후 `Total=671`). 이력: 513(STAB-WP-02) → 572(STAB-WP-03b +59) → 579(STAB-UX-01 +7) → 602(UX-WP-01 +23) → 631(UX-WP-02/03 +29) → 646(STAB-UX-02 +15) → 671(R2-WP-01 +25). 과거 484/502는 하니스가 합계를 안 찍던 시절의 미집계 추정치였고, STAB-WP-02가 **합계·도메인별 PASS/FAIL·실행시간**을 출력한다.
...
| R2 | v0.7.0 | Risk Analytics & Visualization (Semantic Hardening·Streaming·전일대비·차트) | Data Spec Gate | **R2-WP-01 DONE**(#79, local-gate; Semantic Hardening) · R2-WP-02~04 설계 완료(ADR-011 · WP+프롬프트 READY) · **R2-WP-02 NEXT** |
