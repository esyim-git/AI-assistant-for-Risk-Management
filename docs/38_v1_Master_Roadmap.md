...
> **현재 기준선 = v0.7.0 (R2 정식 릴리스) + STAB-WP-01~04 + STAB-UX-01/02 + UX-WP-01~09 + R2-WP-01~05 + REL-v0.7.0 + KB-WP-01/02 + FEEDBACK-WP-01/02** (main `0f6e1d7`, UX-WP-09 #113 머지 후; **R2 분석 트랙 v0.7.0 정식 릴리스** 태그 `30c1cfb`·ZIP SHA256 `42C835…`·미서명; 직전 v0.6.0 태그 `3dfa80b`). **KB-WP-01/02 DONE**(#94/#101, local-gate). **UX-WP-04/05/06 DONE**(#102~#104, Excel Function Helper + Smart Assist as-you-type/팝업, local-gate). **FEEDBACK-WP-01 DONE**(#106, 승인 Example ingest 게이트·non-log DTO·결정적 검색·hash audit — RETRIEVAL·학습 아님). **FEEDBACK-WP-02 DONE**(#108, 검색 Example → `DraftRequest.Context` review 경유 read-only 반영 — `DraftReferenceComposer` 결정적·`ReferencesReviewed` 게이트·`effectiveContext` 이중 audit hash·자동주입 0). **cleanup DONE**: UX-WP-07(#110 Smart Assist 표면화 하이진)·R2-WP-05(#109 dead Welford 필드 제거·동작 불변)·UX-WP-08(#111 팝업 포커스 복원, 실 렌더=Gate B)·**UX-WP-09**(#113 A-4 동일 finding 이중 핀 축소 — BlockedHint 유지·finding 미손실·결정적). **NEXT UP = 확장 트랙 Wave 1**(QA-WP-01 Safety 하드닝·QA-WP-02 Recon/Limit 하드닝·UX-WP-10 seed 확장 — 3 WP 병렬, 프롬프트 READY). 그 외 후보(신규 기능 트랙 설계·STAB-WP-05 승인·**Gate B/C 실 Test PC=user-driven**)는 후속. WP 시퀀스(완료) = KB-WP-01/02→UX-WP-04/05/06→FEEDBACK-WP-01→FEEDBACK-WP-02→cleanup(UX-WP-07·R2-WP-05·UX-WP-08·UX-WP-09). **STAB-WP-05 인증서 경로 = A(사내 Enterprise CA) 확정**(나머지 §6.2·서명도구는 구현 WP, 실 증거 전 APPROVAL_REQUIRED 유지). 본 문서는 v0.6.0 다음 단계부터 v1.0 Team Pilot까지의 통합 실행 로드맵이다.
...
- **SmokeTest**: **Total=834 PASS=834 FAIL=0** (정본 합계 — local-gate; `dotnet build` 0/0 + SmokeTest). 이력: … → 714(R2-WP-04 +16) → 747(KB-WP-01 +33) → 768(KB-WP-02 #101 +21) → 778(UX-WP-04 #102 +10) → 788(UX-WP-05 #103 +10) → 792(UX-WP-06 #104 +4) → 807(FEEDBACK-WP-01 #106 +15) → 829(FEEDBACK-WP-02 #108 +13·UX-WP-07 #110 +6·UX-WP-08 #111 +3·R2-WP-05 #109 +0) → 834(UX-WP-09 #113 +5). 과거 484/502는 하니스가 합계를 안 찍던 시절의 미집계 추정치였고, STAB-WP-02가 **합계·도메인별 PASS/FAIL·실행시간**을 출력한다.
