...
- [x] **모델/인터넷 없이 앱 기동·동작**(NoModelMode)
- [x] 생성 초안은 Safety Checker 통과 + audit log(해시) 기록
- [x] 규정 답변에 "검토용 초안" + 출처 항상 포함, 내부원문 repo 미포함
- [x] Excel 리포트는 2021 호환(365 함수 0건), `reports/`에만 생성
- [x] 승인형 피드백은 예제 승격만(재학습 없음)
- [x] 외부 API/자동실행/telemetry/모델파일 커밋 0건
