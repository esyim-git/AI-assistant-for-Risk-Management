...
| **DM-03** | Excel 리포트 생성 | 산출 수식/함수는 **Excel 2021 호환**만(생성물 자체를 `Excel2021FunctionChecker`로 검사). VBA 자동 실행 없음. **생성 방식(OpenXML SDK NuGet vs Interop vs 템플릿 채우기)은 미정 → 결정 필요**: OpenXML은 NuGet 추가 = AGENTS 규칙상 STOP·승인 대상. 권장 1순위 = 템플릿 xlsx + 최소 치환, NuGet 불가피 시 사유 보고. 쓰기 경로는 `reports/`. |
...
- **출력**: `reports/` 하위 xlsx (또는 CSV+템플릿)
- **완료조건**: 생성물에 365 전용 함수 없음, 오프라인 생성
- **테스트**: 더미 입력 → 리포트 생성, 함수 호환 검사 PASS
- **보안**: 쓰기 경로 `reports/`만. VBA 자동실행 없음. **생성 방식 NuGet 결정(DM-03) 선행**
