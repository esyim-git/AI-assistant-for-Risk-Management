...
**현재 종합 상태 = `PARTIAL`.** v0.7.1 final rebuild/tag/Release, CORR-WP-01, GOV-WP-02는 완료됐지만, (1) 여러 VERIFIED Core 기능이 WPF에서 도달 불가하며, (2) formal Gate B/C 증거와 Team Pilot이 닫히지 않았다. Branch protection/secret scanning은 2026-07-11 REST readback으로 닫혔다.
...
3. **GOV-WP-02 VERIFIED**: hosted green과 실제 check 이름을 정본화하고 Phase A 보호를 REST 적용/재조회했다.
4. **Main hard protection VERIFIED**: PR 필수·strict `test`/`wpf-build`·conversation resolution·linear history·admin enforcement ON, force/deletion OFF. 단일 계정 self-review 교착 때문에 승인 1/Code Owner 강제는 독립 reviewer가 생긴 뒤 켠다.
5. **Secret scanning + push protection VERIFIED**. User-driven published v0.7.1 Gate B/C는 계속 병행한다.

### Phase 1 — v0.8 사용자 가치

1. **NEXT UP = ARCH-WP-01**: MainWindow 행위 불변 partial 분해. MVVM 전면 전환은 하지 않는다. 실행 프롬프트는 `prompts/codex/ARCH-WP-01_mainwindow_partial_decomposition.md`.
...
| Release governance | VERIFIED | published tag/SHA, hosted checks, hard branch protection, secret scanning/push protection 충족 |
