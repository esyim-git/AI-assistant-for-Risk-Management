...
**현재 종합 상태 = `PARTIAL`.** Core 안전·데이터·리포트 기반과 회귀 테스트는 강하고 CORR-WP-01까지 머지됐지만, (1) v0.7.1 final rebuild/tag/Release가 아직 없고, (2) 여러 VERIFIED Core 기능이 WPF에서 도달 불가하며, (3) formal Gate B/C 증거와 Team Pilot이 닫히지 않았고, (4) branch protection/secret scanning이 아직 닫히지 않았다.
...
| 항목 | 실제 상태 | 근거 |
|---|---|---|
| `origin/main` / 코드·테스트 baseline | `4efb8e670ce0306d07683d3fbc5ed7b118844b8b` | PR #135 squash merge, product code/test 변경이므로 baseline 이동 |
...
| v0.7.1 final 후보 | `NOT_IMPLEMENTED` | #135 이후 latest main에서 build/00~03 재실행 필요 |
...
| Prior-Day Analytics | VERIFIED | NOT_IMPLEMENTED: App call site 0 | main 포함, v0.7.1 publish pending | UI-WP-12 |
...
| Clause 검색/발췌 게이트 | VERIFIED | NOT_IMPLEMENTED: `SearchClauses` call site 0 | main 포함, v0.7.1 publish pending | KB-UI-WP-01 + 승인 Pack |
| Feedback 승인 승격 | VERIFIED | VERIFIED | main 포함, v0.7.1 publish pending | formal Gate B |
| 승인 Example 검색/Prompt reflection | VERIFIED | NOT_IMPLEMENTED: Retriever/ReferencesReviewed UI call site 0 | main 포함, v0.7.1 publish pending | FEEDBACK-WP-03 |
...
2. **REL-01: v0.7.1 code cut은 VERIFIED지만 published release가 아님.**
   - Pre-CORR 후보 ZIP은 당시 검증됐지만 #135로 무효다. latest main에서 build/00~03을 다시 수행하고 새 SHA를 정본으로 태깅한다.
...
2. **NEXT UP = latest main에서 build/00~03 재실행, v0.7.1 tag/Release 발행.**
3. **GOV-WP-02**: #135 hosted green 증거를 정본화하고 실제 check 이름으로 보호 설정.
