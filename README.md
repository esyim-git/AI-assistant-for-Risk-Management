...
| 대용량 CSV Streaming / 행·바이트 상한 / Welford 1-pass 프로파일 / 중복행 해시 | VERIFIED | R2-WP-02(#81, local-gate); 기존 결정성·`DuplicateRowCount` 보존, NuGet 0, 실 Test PC Gate B/C BLOCKED |
| 전일 대비 / 차트·Heatmap·TopN·집중도 | NOT_IMPLEMENTED | R2-WP-03(전일대비)·R2-WP-04(시각화) |
...
**SmokeTest**: **Total=680 PASS=680 FAIL=0** (정본 합계 — R2-WP-02(#81) local-gate 후; 631 + 레이아웃 영속화 +15 + R2-WP-01 +25 + R2-WP-02 Streaming/Perf +9). **680은 파생값**(리뷰본 65dca55=678 확인 + 머지 델타 +2)이며 머지본 `5280d54` 로컬 게이트 `dotnet run` 재확인 권장. 과거 484/502/513/572/579/602/631/646/671 등은 이전 베이스라인/중간값이며 현재 정본 수치가 아닙니다.
...
- **R2-WP-02 DONE(#81, local-gate)**: Streaming/Perf — 대용량 CSV streaming 리딩·행(`MaxRowCount=200K`)/바이트(`MaxByteSize=50MB`) 상한·Welford 1-pass 통계(전 값 미보관)·정확 OutlierCount(2차 streaming pass, 기존과 동일)·중복행 SHA256 해시. CP949는 repo 내장 `Cp949Decoder` 재사용(결정성 보존). 인박스 NuGet 0, SmokeTest 671→680(파생). 실 Test PC Gate B/C는 BLOCKED 유지.
- **NEXT UP**: **R2-WP-03**(전일 대비 Prior-Day Analytics — Current/Prev/Δ·TopN movers·4구획 출력 계약, 인박스 NuGet 0). WP+프롬프트 `prompts/codex/R2-WP-03_prior_day_analytics.md` READY. R2 = v0.7.0(분석 트랙).
