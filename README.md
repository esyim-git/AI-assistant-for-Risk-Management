...
| Smart Assist **입력중(as-you-type) 추천 + 팝업 표시 확장 + 표면화 하이진 + 포커스 복원** | VERIFIED | UX-WP-05/06(#103/#104) + UX-WP-07(#110 dedupe Kind·Info/Low 힌트 필터·allow-fn warning)·UX-WP-08(#111 Esc/Close 포커스 복원) local-gate; focus-preserving·debounce·정적 NoModel·자동삽입 0·실시간 LLM=R4 미구현 (실 UI 렌더=Gate B) |
...
**SmokeTest**: **Total=829 PASS=829 FAIL=0** (정본 합계 — local-gate; `dotnet build` 0/0 + SmokeTest. 이력: 747(KB-WP-01) → 768(KB-WP-02 #101 +21) → 778(UX-WP-04 #102 +10) → 788(UX-WP-05 #103 +10) → 792(UX-WP-06 #104 +4) → 807(FEEDBACK-WP-01 #106 +15) → 829(FEEDBACK-WP-02 #108 +13·UX-WP-07 #110 +6·UX-WP-08 #111 +3·R2-WP-05 #109 +0)). 과거 484/502/513/572/579/602/631/646/671/680/698/714/747 등은 이전 베이스라인/중간값이며 현재 정본 수치가 아닙니다.
...
- **cleanup DONE(#109~#111, local-gate)**: UX-WP-07(Smart Assist 표면화 하이진 — dedupe Kind·Info/Low 힌트 필터·allow-fn warning) · R2-WP-05(dead Welford 필드 제거·동작 불변) · UX-WP-08(팝업 Esc/Close 포커스 복원, 실 포커스 렌더=Gate B). 누적 `Total=829`.
- **NEXT UP**: **UX-WP-09**(A-4 동일 finding 이중 핀 정리 — BlockedHint+SafetyHint 중복 표면 축소, 사용자 요청 continuation·reviewable; 프롬프트 READY). 그 외 후보(신규 기능 트랙 설계·STAB-WP-05 승인·**Gate B/C 실 Test PC = user-driven**, `docs/48 §B″` 런북)는 후속. KB-WP-01/02·UX-WP-04~08·R2-WP-05·FEEDBACK-WP-01/02는 **DONE**(#94/#101/#102/#103/#104/#106/#108/#109/#110/#111, local-gate). **STAB-WP-05 인증서 경로 = A(사내 Enterprise CA) 확정**(병행). 실 Test PC Gate B/C = BLOCKED(`docs/48`).
