...
현재 기준선: **v0.6.0** (main `f6b1405`, v0.6.0 정식 릴리스 태그 `3dfa80b`). R1 Data & Limit Foundation(완료, v0.5.0) + R3 **공개 규정 Metadata 기반 RAG 구조/NCR Rule Set 구조**(완료, v0.6.0)가 코드/CI 레벨로 완료된 상태이며, 실 오프라인 Test PC Gate(B/C)와 R2 Risk Analytics 이후가 남아 있습니다. (R3는 공개 catalog/메타데이터 + Keyword/Inverted Index 인용 구조이며 **규정 원문 RAG가 아니고**, NCR은 **Rule Set 8요소 구조일 뿐 실제 NCR 산정이 아닙니다**.)
...
**SmokeTest**: **Total=572 PASS=572 FAIL=0** (정본 합계 — STAB-WP-04 suite 분리 후, 종료부에 `Total=572` + 도메인별 요약 출력, windows-latest CI). 과거 484/502/513 등은 이전 베이스라인/중간값이며 현재 정본 수치가 아닙니다.
...
- **STAB v0.6.1**: WP-01/02/03a/03b/04 완료(#56·#57·#59·#61·#66). 잔여 = STAB-WP-05(Authenticode 코드서명) = **APPROVAL_REQUIRED**.
