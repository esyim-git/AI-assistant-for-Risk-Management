...
**SmokeTest**: **Total=680 PASS=680 FAIL=0** (정본 합계 — R2-WP-02(#81) 후 `5280d54` 직접 local-gate 재확인: `dotnet build` 0/0 + SmokeTest, 2026-06-30; 631 + 레이아웃 영속화 +15 + R2-WP-01 +25 + R2-WP-02 Streaming/Perf +9). 과거 484/502/513/572/579/602/631/646/671 등은 이전 베이스라인/중간값이며 현재 정본 수치가 아닙니다.
...
- **R2-WP-02 DONE(#81, local-gate)**: Streaming/Perf — 대용량 CSV streaming 리딩·행(`MaxRowCount=200K`)/바이트(`MaxByteSize=50MB`) 상한·Welford 누산(전 값 미보관) + 정확 OutlierCount(legacy-compatible 2차 streaming pass, 기존과 동일)·중복행 SHA256 해시. CP949는 repo 내장 `Cp949Decoder` 재사용(결정성 보존). 인박스 NuGet 0, SmokeTest 671→680(직접 local-gate 재확인). 실 Test PC Gate B/C는 BLOCKED 유지.
