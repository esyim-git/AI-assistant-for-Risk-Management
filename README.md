...
- **NEXT UP**: **REL-WP-071 — v0.7.1 출하 정합 릴리스 컷**(결정지점은 저장소 평가 제안서 채택 PR #131 `110e9ee`로 해소 — 채택 로드맵 `docs/proposals/FABLE5_REPO_ASSESSMENT_PROPOSAL_20260706.md`). published `30c1cfb` 이후 머지분(KB clause 검색·Excel Function Helper·Smart Assist as-you-type·승인 Example 반영·QA 하드닝)을 출하본에 반영 — 버전 범프 락스텝만(기능 0·`Total=900` 불변·STOP 비접촉). 원장 `docs/39`·릴리스 문서 `docs/52`·프롬프트 READY. **병행(user-driven)**: Gate B/C Round 1(`docs/48 §B″` 런북 — 증거 민감정보 0, dummy·masking만). 승인 게이트 3건(STAB-WP-05 서명(경로 A 확정)·NCR 실 Pack·R4 LLM)=`docs/51` 결정 대기. KB-WP-01/02·UX-WP-04~11·R2-WP-05·QA-WP-01~09·FEEDBACK-WP-01/02는 **DONE**(#94~#127 계열, local-gate). 실 Test PC Gate B/C = BLOCKED(`docs/48`).
