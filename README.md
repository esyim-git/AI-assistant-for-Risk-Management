...
현재 기준선: **v0.6.0** (main `600d687`, v0.6.0 정식 릴리스 태그 `3dfa80b`). R1 Data & Limit Foundation(완료, v0.5.0) + R3 **공개 규정 Metadata 기반 RAG 구조/NCR Rule Set 구조**(완료, v0.6.0)가 코드/CI 레벨로 완료되고 STAB v0.6.1(WP-01~04 + UX-01 Resizable Layout) + **UX-WP-01 Smart Assist 코어(계약+엔진+해시 audit, 정적·NoModel)** 까지 안정화된 상태이며, 실 오프라인 Test PC Gate(B/C)와 R2 Risk Analytics 이후가 남아 있습니다. (Smart Assist는 **계약+코어만**이며 실 provider 콘텐츠=UX-WP-02·WPF Popup=UX-WP-03은 미구현입니다.) (R3는 공개 catalog/메타데이터 + Keyword/Inverted Index 인용 구조이며 **규정 원문 RAG가 아니고**, NCR은 **Rule Set 8요소 구조일 뿐 실제 NCR 산정이 아닙니다**.)
...
**SmokeTest**: **Total=602 PASS=602 FAIL=0** (정본 합계 — UX-WP-01(#70) local-gate 후; STAB-UX-01 정본 579 + UX-WP-01 `Assist` 도메인 +23, 종료부에 `Total=602` + 도메인별 요약 출력). 과거 484/502/513/572/579 등은 이전 베이스라인/중간값이며 현재 정본 수치가 아닙니다.
...
- **STAB v0.6.1 / UX**: STAB WP-01~04 + UX-01(Resizable Layout) + **UX-WP-01(Smart Assist 코어)** 완료(#56·#57·#59·#61·#66·#68·#70). 잔여 = STAB-WP-05(Authenticode 코드서명) = **APPROVAL_REQUIRED**.
- **NEXT UP**: **UX-WP-02**(정적 SQL/VBA/Excel/Risk Provider — UX-WP-01 계약 위, 기존 Checker/RuleSet 단일원천 재사용, 실데이터 0). WPF Popup=UX-WP-03.
