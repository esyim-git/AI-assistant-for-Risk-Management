---
name: risk-release-verify
description: Verify offline portable release package, SHA256, approved_manifest, forbidden files, package guard, and release evidence.
argument-hint: "[version]"
arguments: [version]
disable-model-invocation: true
allowed-tools: Bash(pwsh:*), Bash(powershell:*), Bash(dotnet build:*), Bash(dotnet run:*), Bash(git status:*), Bash(git diff:*), Read, Grep, Glob
shell: powershell
---
...
## 언제 사용
- **수동 호출 전용** (`/risk-release-verify`). 모델 자동 호출 금지.
...
- **ZIP 해시 = 태그 커밋 기준**: `AssemblyInformationalVersion`에 빌드 커밋 SHA가 박혀 **커밋마다 ZIP SHA256이 달라진다**. 정본은 **태그 대상 커밋에서 빌드한 ZIP의 실제 해시**(Release asset `.sha256` / `docs/47` ReleaseNote 값과 대조). 다른 커밋·dirty 트리에서 빌드한 ZIP 해시를 정본으로 적지 않는다.
...
## 참조
- `docs/24_Release_Packaging_Guide.md`(패키징 절차·산출물) · `docs/28`(게이트 B/C) · `docs/34_Release_Rehearsal_Guide.md`(리허설 런북) · `docs/47_Release_v0.7.0.md`(현 릴리스 ReleaseNote·SHA256 정본) · `deploy/release_checklist.md`(보안·배포·반입 체크) · `build/00..03_*.ps1`(실행 스크립트, 특히 `build/03_verify-package.ps1`).
- `CLAUDE.md §3`(절대 원칙) · `§11.4`(상태 어휘) · `§11.6`(Local-Gate).
- 연계 스킬: `/risk-gate-bc`(게이트 B/C 증거 수집·정리) · `/risk-security-guard`(보안 게이트 A 단독 점검).
