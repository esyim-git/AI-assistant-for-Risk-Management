---
name: risk-security-guard
description: Check repository and release artifacts for secrets, real data, internal regulation originals, NCR official originals, model files, unsafe dependencies, and forbidden automation.
...
## 언제 사용
- **수동 호출 전용**: `/risk-security-guard`. 모델이 자동 호출하지 않는다(`disable-model-invocation: true`).
- commit/push 직전, 또는 Codex 결과를 머지 게이트로 올리기 전 보안 축을 단독으로 돌릴 때.
- 게이트 B/C(릴리스·반입)는 범위 밖이다 — `/risk-gate-bc`로 이어간다.

## 절대 원칙
점검 기준은 `docs/28` 게이트 A + `AGENTS.md §3`(불변). 하나라도 위반이면 **FAIL → 중단**.
- **외부 NuGet PackageReference = 0** · 외부 API 0 · Telemetry 0 · 자동 업데이트 0 (`AGENTS.md §3·§4`). 추가가 보이면 STOP → `/risk-llm-approval` 등 승인 문서 먼저.
...
- 연계 스킬: `/risk-codex-review`(보안 축 포함 4축 리뷰) · `/risk-branch-governance`(머지·브랜치 규약).
