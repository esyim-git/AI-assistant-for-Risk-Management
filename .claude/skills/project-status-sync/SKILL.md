...
description: "프로젝트 현재 상태 동기화 — docs/38 Roadmap·docs/39 Resume Brief(기준선 SHA·NEXT UP·SmokeTest Total)·Traceability를 실제 main과 대조해 정리한다. \"현재 상태/어디까지/다음 뭐/status/where are we\" 류 질문이나 머지 후 상태 점검 시 사용."
...
- 수정이 필요하면 직접 고치지 말고 [/documentation-truth-sync](../documentation-truth-sync/SKILL.md)로 넘긴다.
...
5. **불일치 정리**: 항목별 `문서 위치 → 현재값 vs 실제값 → 제안` 으로 목록화. 수정은 직접 하지 말고 [/documentation-truth-sync](../documentation-truth-sync/SKILL.md)에 위임한다.
6. **NEXT UP 확정**: Resume Brief 기준 NEXT UP 1개를 명시. 구현 착수 시 [/codex-work-package](../codex-work-package/SKILL.md)로 연결.
...
- 관련 스킬: [/documentation-truth-sync](../documentation-truth-sync/SKILL.md)(불일치 실제 수정) · [/codex-work-package](../codex-work-package/SKILL.md)(NEXT UP 구현 착수)
