...
> `/project-status-sync` 보조 체크리스트. **읽기·진단 전용** — 이 스킬은 문서/코드를 수정하지 않는다. 불일치 발견 시 `/documentation-truth-sync`로 위임한다.
...
```
- 불일치가 있으면 항목별로 `/documentation-truth-sync`에 넘긴다(이 스킬은 직접 수정하지 않음).
- 착수할 WP가 정해지면 `/codex-work-package`로 연결한다.
