...
- [ ] 위 항목이 필요해지면 **STOP** → 승인 문서 작성(라이선스·크기·오프라인 동작·보안·반입·무결성) 후에만 진행 → `/local-model-approval`.
...
- E Keyword-only:PASS | STOP(Vector/Embedding 도입 시도 → /local-model-approval)
- F 불변식:      PASS | VIOLATION
```

> 참조: `CLAUDE.md §10·§11.4·§11.5`, `docs/04`, `docs/08`, `docs/17`, `docs/18`, `docs/41 §2`. 연계: `/local-model-approval`, `/security-gate-a`.
