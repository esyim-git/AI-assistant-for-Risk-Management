---
name: rag-ncr-governance
description: "KB/RAG/NCR 거버넌스 — 공개 catalog만·내부규정/NCR 원문 repo 미포함·규정답변 10단계 형식·인용 검증·Ingest Gate·Keyword Index만(Vector/Embedding=STOP). KB/NCR/규정 파일 작업 시 자동 적용."
...
4. **Keyword-only**: 검색 엔진이 Keyword/Inverted Index인지 확인. Vector/Embedding/모델 도입 흔적·요구가 보이면 **STOP** 처리하고 `/local-model-approval`로 승인 경로 안내.
...
- 연계 스킬: `/local-model-approval` (Vector/Embedding/Local LLM 도입 STOP→승인), `/security-gate-a` (커밋 전 보안 게이트·원문/secret 차단)
