---
name: local-model-approval
description: "Local LLM/Vector DB/Embedding/모델파일 STOP+승인 게이트(ADR-009·docs/41 sec3) — Model Approval Package 작성, 승인 전 의존성/런타임/모델 추가 차단(MODEL_APPROVAL_REQUIRED). 수동 호출 전용."
...
## 언제 사용
- **수동 호출 전용** (`/local-model-approval`). 자동 적용 안 함(`disable-model-invocation: true`).
- 외부 라이브러리/NuGet · Vector DB · Embedding Runtime · Local LLM Runtime · 모델파일이 **필요해지는 순간** 호출한다.
- `/rag-ncr-governance`가 Vector/Embedding 도입 흔적을 STOP 처리하며 이 스킬로 승인 경로를 안내할 때.
...
- 연계 스킬: `/rag-ncr-governance` (Vector/Embedding 도입 흔적 → STOP 안내), `/security-gate-a` (커밋 전 보안 게이트·모델파일/secret 차단)
