...
## 목적
문서가 실제 코드/머지 상태를 정확히 반영하고, 정본 상태 어휘만 쓰며, 능력을 과대표기하지 않도록 `docs/38·39·40·54`(현재 Gate B/C 정본, `docs/48/45/44`는 historical)을 정합화한다(증거 기반 최소 수정).
...
- **증거 없으면 PASS/VERIFIED 금지**: 실 오프라인 Test PC 증거 없는 Gate는 `BLOCKED` 유지(현재 정본 `docs/54`, historical `docs/48/45/44`). 머지 Gate는 로컬 build/SmokeTest+리뷰와 활성 hosted `test`/`wpf-build`를 독립 증거로 요구하며 서로 대체하지 않는다(`CLAUDE.md §11`).
...
2. **영향 항목 식별**: `docs/38`(Roadmap·Capability·Traceability) · `docs/39`(WP Backlog·Resume Brief·진행 원장) · `docs/40`(ADR) · `docs/54`(Gate B/C 증거 — 현재 정본; `docs/48/45/44`는 historical)에서 영향받는 항목을 Grep으로 찾는다.
...
## 체크리스트
문서별(38/39/40/54, historical 48/45/44) 정합 점검 항목은 [truth-sync-checklist.md](truth-sync-checklist.md). 상태 어휘 정의·사용 규칙·과대표기 금지 예시는 [status-vocabulary.md](status-vocabulary.md).

## 참조
- `CLAUDE.md §11.1`(Truth Sync 책임) · `§11.4`(상태 어휘·과대표기 금지) · `§11.6`(Local-Gate).
- `docs/38`(Master Roadmap·Capability·Traceability) · `docs/39`(Work Package Backlog) · `docs/40`(ADR) · `docs/54`(Gate B/C current 증거·런북) · `docs/48/45/44`(historical Gate B/C 증거).
