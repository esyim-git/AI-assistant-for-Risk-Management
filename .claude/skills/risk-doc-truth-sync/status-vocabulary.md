...
| `BLOCKED` | 선행(특히 실 오프라인 Test PC 증거) 대기로 진행 불가 | 무엇을 기다리는지 + 증거 문서 PATH(예: 현재 `docs/54`) |
...
- **Gate는 실 오프라인 Test PC 증거 없이 PASS 금지** → `BLOCKED` 유지(현재 `docs/54`, historical `docs/48/45/44`).
...
- 나쁨: "Gate B/C PASS."
  - 좋음: "Gate B/C = `BLOCKED`(실 오프라인 Test PC 증거 대기, 현재 `docs/54`). 실 PC 증거 없이 PASS로 적지 않는다."
- 나쁨: "테스트 다 통과."
