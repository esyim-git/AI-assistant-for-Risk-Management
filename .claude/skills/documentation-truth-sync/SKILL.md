...
description: "문서 ↔ 실제 main Truth-Sync — 상태 어휘(VERIFIED/PARTIAL/SCAFFOLD_ONLY/PLACEHOLDER/BLOCKED/NOT_IMPLEMENTED/APPROVAL_REQUIRED)만 사용, 과대표기 금지, docs/38·39·40·45 정합 유지. 머지 후 문서 갱신·문서 드리프트 의심 시 사용."
allowed-tools: Read, Grep, Glob, Edit, Bash(git log:*)
---

# Documentation Truth-Sync

## 목적
문서가 실제 코드/머지 상태를 정확히 반영하고, 정본 상태 어휘만 쓰며, 능력을 과대표기하지 않도록 `docs/38·39·40·45`를 정합화한다(증거 기반 최소 수정).
...
## 체크리스트
문서별(38/39/40/45) 정합 점검 항목은 [truth-sync-checklist.md](truth-sync-checklist.md). 상태 어휘 정의·사용 규칙·과대표기 금지 예시는 [status-vocabulary.md](status-vocabulary.md).

## 참조
- `CLAUDE.md §11.1`(Truth Sync 책임) · `§11.4`(상태 어휘·과대표기 금지) · `§11.6`(Local-Gate).
- `docs/38`(Master Roadmap·Capability·Traceability) · `docs/39`(Work Package Backlog) · `docs/40`(ADR) · `docs/45`(Gate B/C v0.6.0 증거).
- 연계 스킬: `/project-status-sync`(기준선·Resume Brief 갱신), `/codex-result-review`(Codex 결과 Diff·테스트 검토 후 본 스킬로 문서 정합).
