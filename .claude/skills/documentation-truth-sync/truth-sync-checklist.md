# Truth-Sync 점검 체크리스트 (docs/38·39·40·45)
...
- [ ] historical 문서(예: `docs/44` v0.5)와 정본(`docs/45` v0.6)의 역할 구분이 유지된다.

## 5. 마감
- [ ] 작업은 `planning/*` 브랜치에서 했다(main 직접 수정/병합 0).
- [ ] 정정 요약을 `<문서 §항목>: <기존> → <정정> (근거: <SHA/Total/문서PATH>)` 줄 목록으로 정리했다.
- [ ] 후속: `/project-status-sync`로 기준선/Resume Brief 반영, 필요 시 `/codex-result-review`와 연계.
