---
name: gate-bc-evidence
description: "Gate B(오프라인 Test PC)·Gate C(Excel 2021/반입) 증거 원장(docs/44·docs/45) 작성·봉인 — 실 Test PC 증거 없으면 BLOCKED 유지(과대표기 금지). 수동 호출 전용."
...
## 언제 사용
- **수동 호출 전용 (`/gate-bc-evidence`).** 모델 자동 호출 비활성(`disable-model-invocation: true`).
...
- 연계 스킬: `/release-package-verify` (패키지 컷·SHA·금지파일 검증 → 본 원장 A그룹 선행)
