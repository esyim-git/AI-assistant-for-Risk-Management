...
description: Review branch governance, PR flow, squash merge, soft guard, issue freeze, hard branch protection verification, and release tag policy.
...
- **Local + Hosted Gate**: 로컬 `dotnet build` + SmokeTest **`Total=N PASS / 0 FAIL`** 증거 **＋** Claude 코드리뷰(Diff·보안·문서)는 항상 필요하다. `test`·`wpf-build`는 독립 증거이자 활성 Phase A required checks다 (`CLAUDE.md §11`, `AGENTS.md §7`).
...
5. **머지 후**: `/risk-doc-truth-sync`로 `docs/38·39·40·54` 정합을 맞추고(`docs/48·45·44`는 historical), 병합된 feature 브랜치 삭제를 권장한다.
