...
Diff·신규 파일에 대해 점검. `/security-gate-a` 스킬과 동일 기준.
...
- [ ] 문서 정합 정정이 필요하면 코드 머지와 분리해 `/documentation-truth-sync`로 처리
...
- 머지·브랜치 규약(PR 필수·Squash·main 직접 push 금지·force push 금지)은 `/branch-governance` 참조.

---

## 참조
- `AGENTS.md §1~7` · `docs/28`(게이트 A) · `docs/39`(WP·Review Checklist) · `docs/38`/`docs/40`/`docs/45`.
- 연계 스킬: `/security-gate-a` · `/documentation-truth-sync` · `/branch-governance`.
