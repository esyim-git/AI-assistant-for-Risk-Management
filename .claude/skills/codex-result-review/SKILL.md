---
name: codex-result-review
description: "Codex feature 브랜치 결과 4축 리뷰 — Diff 정확성 · 보안(Gate A) · 테스트(SmokeTest Total 보존+회귀) · 문서 정합. 머지 승인 전 Codex PR/브랜치 검토 시 사용."
...
3. **보안 축(Gate A)**: `docs/28` 게이트 A 항목 + NuGet 0·외부 API 0·secret/실데이터/원문 0·금지 확장자 부재를 Diff에 대해 점검한다. 상세는 [review-dimensions.md](review-dimensions.md). `/security-gate-a` 스킬과 동일 기준.
4. **테스트 축**: SmokeTest **이전 Total 보존 + 신규 회귀 추가** 여부, 단언 약화/삭제 0, 보고에 `Total=N PASS / 0 FAIL` 합계 줄이 포함됐는지 확인. 총수 감소면 사유·매핑 필수.
5. **문서 축**: 해당 WP의 **Claude Review Checklist**(`docs/39`)와 `docs/38·40·45` 정합, 상태 어휘 적정성을 확인한다. 문서 정합 정정은 `/documentation-truth-sync`로 이어간다.
...
- 연계 스킬: `/security-gate-a`(보안 축 단독) · `/documentation-truth-sync`(문서 정합 후속) · `/branch-governance`(머지·브랜치 규약).
