...
- [ ] 구현 PR은 지정 단일 WP만, planning PR은 선언한 audit/truth-sync package만 다루는가
...
- [ ] 머지 게이트 증거 = 로컬 `dotnet build`(Release) + SmokeTest + 활성 hosted `test`/`wpf-build` exact-head 결과
- [ ] hosted queued/skipped/not-run/red 또는 deprecation/security annotation을 success로 간주하지 않음
...
## 참조
- `AGENTS.md §1~7` · `docs/28`(게이트 A) · `docs/39`(WP·Review Checklist) · `docs/38`/`docs/40`/`docs/54`(current Gate) · `docs/48/45/44`(historical Gate).
