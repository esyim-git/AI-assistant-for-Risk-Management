...
description: "브랜치/PR 거버넌스 — feature/<WP-ID>-* 작업 브랜치, Claude 계획은 planning/*, squash 머지, Commit Subject에 (#PR), force push·hard reset·main 직접 push 금지, Local-Gate 머지(로컬 Total=N PASS/0 FAIL + Claude 리뷰). git 브랜치/PR/머지 작업 시 사용."
allowed-tools: Read, Grep, Bash(git status:*), Bash(git branch:*), Bash(git log:*)
---
...
- **머지 정책**: **Squash merge만** 허용(merge commit·rebase merge OFF). Commit Subject 끝에 **`(#<PR번호>)`** 포함(soft guard가 PR 표식을 확인) (`docs/32 §4·§2.1`, `docs/35`).
- **Local-Gate 머지 게이트**: 로컬 `dotnet build` + SmokeTest **`Total=N PASS / 0 FAIL`** 증거 **＋** Claude 코드리뷰(Diff·보안·문서). **GitHub CI green을 머지 전제로 요구하지 않는다**(분 가용 시 보조망) (`CLAUDE.md §11.6`, `AGENTS.md §5`).
...
3. **머지 전 게이트**: `/security-gate-a`(보안 축)와 `/codex-result-review`(4축 리뷰) 통과 + 로컬 **`Total=N PASS / 0 FAIL`** 증거를 확인한다. 하나라도 없으면 머지 불가.
4. **Squash·(#PR) 규약**: 머지는 squash로, Commit Subject에 `(#<PR번호>)`가 들어가는지 확인한다(soft guard 신호). custom subject도 `(#N)` 필수.
5. **머지 후**: `/documentation-truth-sync`로 `docs/38·39·40·45` 정합을 맞추고, 병합된 feature 브랜치 삭제를 권장한다.

## 산출물/보고
- **거버넌스 점검표**: `브랜치명 / 금지조작(force·hard reset) / main 직접 변경 / Squash·(#PR) / Local-Gate 증거` 각각 `PASS` 또는 `위반`.
- 위반은 `항목 — 근거(브랜치명·커밋·증거 누락) — 정정 조치` 줄 목록.
- 머지 증거 줄: 인용된 로컬 **`Total=N PASS / 0 FAIL`** + Claude 리뷰 결과(GitHub CI green은 전제 아님).
...
## 참조
- `docs/32_Branch_Governance.md`(브랜치 모델·보호·리뷰·머지) · `docs/35_Private_Free_Soft_Guard.md`(private Free soft guard·squash only·`(#PR)`).
...
- 연계 스킬: `/security-gate-a`(보안 축) · `/codex-result-review`(머지 전 4축 리뷰) · `/documentation-truth-sync`(머지 후 문서 정합).
