...
## 절대 원칙
- **실 오프라인 Test PC 증거가 없으면 항목 `BLOCKED` 유지** — `PASS`로 적지 않는다. user-reported(증거 미첨부)는 봉인 PASS가 아니라 `user-reported`로만 기록한다(`docs/48 §B′`). (CLAUDE.md §11.4)
- 상태 어휘는 정본만: `VERIFIED`/`PARTIAL`/`SCAFFOLD_ONLY`/`PLACEHOLDER`/`BLOCKED`/`NOT_IMPLEMENTED`/`APPROVAL_REQUIRED`. **과대표기 금지.**
- 전체 `PASS` 조건: **모든 항목 `PASS`**(문서에 명시된 `N/A`/`ACCEPTED_RISK` 예외 수용 포함). 하나라도 누락/불일치면 해당 항목 + 전체 `BLOCKED` 유지. (`docs/48 §판정 규칙`)
...
- **라운드 분리 규칙**(`docs/48 §B″·§5a`): published 출하 ZIP 라운드(R1)와 test-only 로컬 빌드 라운드(R2)는 증거를 분리 기입하며, **test-only 빌드로 출하본 게이트를 봉인(PASS)하지 않는다** — test-only 증거는 "코드가 실 UI에서 동작함"의 회귀 근거일 뿐, 출하 봉인은 해당 기능을 담은 공개 컷에서만.
- 이 스킬은 **프로세스/체크리스트 가이드**다. 코드·산출물 동작을 바꾸지 않으며, 원장 `.md`만 `Edit`한다.

## 절차
1. 대상 릴리스 Gate 시트 식별: **현재 정본 = `docs/48_GateBC_v0.7.0_Evidence.md`**(v0.7.0 R1+R3+R2 — §B″ 턴키 런북·라운드별 증거 워크시트 포함). `docs/45`(v0.6.0)·`docs/44`(v0.5.0)는 historical. 신규 릴리스 컷 시 `docs/48` 양식을 복제해 새 원장을 만든다(초기 상태 BLOCKED).
...
## 참조
- **`docs/48_GateBC_v0.7.0_Evidence.md` (현재 정본 원장 — §B″ 런북·증거 워크시트·라운드 규칙)** · `docs/45`(v0.6.0)·`docs/44`(v0.5.0) historical
- `docs/41_Approval_and_Pilot_Gates.md §4` (Pilot Gate B/C 실행계획·결과양식), `docs/28_Security_Review_Checklist.md`
- `docs/47 §3` (v0.7.0 Gate B 체크), CLAUDE.md §11.4 (상태 어휘·과대표기 금지)
