...
정본 원장: `docs/54_GateBC_v0.7.1_Evidence.md`. `docs/48`(v0.7.0)·`docs/45`(v0.6.0)·`docs/44`(v0.5.0)는 historical이다.
...
| B5 | 대사 9종 + 원천합계=분석합계 | 화면/로그 | 적용 대상 PASS; optional 통화/단위 컬럼 부재 시 해당 2종 N/A 허용; 원천=분석 수치 일치 |
| B6 | 화면=리포트 동일 수치(`LIMIT_MONITORING`/Dashboard/Report) | 캡처 2장 이상 | 동일 케이스 수치 일치 |
| B7 | Streaming/Welford/대용량 경계 | 릴리스 노트·테스트 결과 | v0.7.1 WPF call site 0 -> `N/A (local-gate only)` |
| B8 | Prior-Day Analytics | 릴리스 노트·테스트 결과 | v0.7.1 WPF call site 0 -> `N/A (local-gate only)` |
...
## 회신 → 판정
운영자가 표를 채워 회신하면 항목 단위 재판정 후 현재 Gate 시트(`docs/54` 또는 릴리스별 successor) 상단 상태를 갱신한다. 적용 대상 전부 `PASS`이고 명시 N/A가 수용되면 해당 릴리스 Gate B를 `PASS`로 봉인한다.

> 관련: `docs/41 §4`, `docs/54`; `docs/48`은 historical. Gate C는 [gate-c-template.md](gate-c-template.md).
